...
private void prosesCari() {
    ...
    try {
        ...
        htmlContent.append("<tr class='head'>"
            ...
            + "<td colspan='3' bgcolor='#F7F7F7' align='center'>Jumlah Kasus Baru</td>"
        ...
        // --- KASUS BARU ---
        String queryBaru = "SELECT dp.kd_penyakit, SUBSTRING(pyk.nm_penyakit,1,80) AS nm_penyakit, p.jk " +
        ...
        while (rs.next()) {
            ...
            if (kode.matches("O80|O82") || kode.matches("^[RVWXYZ].*")) {
                continue;
            }
        ...
        // --- JUMLAH KUNJUNGAN ---
        String queryKunjungan = "SELECT dp.kd_penyakit, p.jk FROM diagnosa_pasien dp " +
                                "INNER JOIN reg_periksa rp ON dp.no_rawat = rp.no_rawat " +
                                "INNER JOIN pasien p ON rp.no_rkm_medis = p.no_rkm_medis " +
                                "WHERE dp.status = 'Ralan' AND dp.prioritas = 1 " +
                                "AND rp.tgl_registrasi BETWEEN ? AND ?";
        ...
        while (rs.next()) {
            ...
            // pastikan kode tetap masuk ke map
            namaPenyakitMap.putIfAbsent(kode, "-");
        }
        rs.close();
        ps.close();

        // --- SORTING BERDASARKAN TOTAL KUNJUNGAN ---
        ...
        for (String kode : kodeList.stream().limit(10).collect(Collectors.toList())) {
            ...
            htmlContent.append("<tr class='isi'><td align='center'>").append(no++).append("</td>")
                ...
                .append("<td align='center'>").append(totalKunj).append("</td></tr>");
        }

        htmlContent.append("</table></body></html>");
        LoadHTML.setText(htmlContent.toString());
    } catch (Exception e) {
        System.out.println("Notifikasi prosesCariRL53: " + e);
    }
    this.setCursor(Cursor.getDefaultCursor());
}
