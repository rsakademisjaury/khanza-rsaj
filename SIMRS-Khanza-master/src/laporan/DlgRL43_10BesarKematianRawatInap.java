...
private void prosesCari() {
    ...
    try {
        ...
        String query = "SELECT dp.kd_penyakit, pyk.nm_penyakit, p.jk, " +
        ...
        Map<String, String> namaKelompokMap = new HashMap<>();
        ...
        while (rs.next()) {
            ...
            if (kode.length() >= 3) {
                String kelompokKode = kode.substring(0, 3);
                String namaPenyakit = rs.getString("nm_penyakit");

                // Simpan hanya nama pertama yang ditemukan (opsional)
                namaKelompokMap.putIfAbsent(kelompokKode, namaPenyakit);

                if (jk.equals("L")) {
                    totalLakiMap.put(kelompokKode, totalLakiMap.getOrDefault(kelompokKode, 0) + 1);
                    if (meninggal) {
                        meninggalLakiMap.put(kelompokKode, meninggalLakiMap.getOrDefault(kelompokKode, 0) + 1);
                    }
                } else {
                    totalPerempuanMap.put(kelompokKode, totalPerempuanMap.getOrDefault(kelompokKode, 0) + 1);
                    if (meninggal) {
                        meninggalPerempuanMap.put(kelompokKode, meninggalPerempuanMap.getOrDefault(kelompokKode, 0) + 1);
                    }
                }
            }
        }
        ...
        // Urutkan berdasarkan total kematian tertinggi
        List<String> kodeList = new ArrayList<>(namaKelompokMap.keySet());
        kodeList.sort((k1, k2) -> {
            int totalMati1 = meninggalLakiMap.getOrDefault(k1, 0) + meninggalPerempuanMap.getOrDefault(k1, 0);
            int totalMati2 = meninggalLakiMap.getOrDefault(k2, 0) + meninggalPerempuanMap.getOrDefault(k2, 0);
            return Integer.compare(totalMati2, totalMati1);
        });

        int noUrut = 1;
        for (String kode : kodeList.stream().limit(10).collect(Collectors.toList())) {
            String nama = namaKelompokMap.get(kode);
            int hidupL = totalLakiMap.getOrDefault(kode, 0);
            int hidupP = totalPerempuanMap.getOrDefault(kode, 0);
            int totalHidup = hidupL + hidupP;
            ...
            htmlContent.append("<tr class='isi'><td align='center'>").append(noUrut++).append("</td>")
                       .append("<td align='center'>").append(kode).append("</td>")
                       .append("<td>").append(nama).append("</td>")
                       .append("<td align='center'>").append(hidupL).append("</td>")
                       .append("<td align='center'>").append(hidupP).append("</td>")
                       .append("<td align='center'>").append(totalHidup).append("</td>")
        ...
        System.out.println("Notifikasi prosesCari RL4.3: " + e);
    }
    this.setCursor(Cursor.getDefaultCursor());
}
