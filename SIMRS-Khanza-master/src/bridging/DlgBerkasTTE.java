 ...
 * @author perpustakaan
 */
public class DlgBerkasTTE extends javax.swing.JDialog {
    ...
    public DlgBerkasTTE(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        initComponents2();
    }
            ...
            public void run() {
                ...
                engine.titleProperty().addListener((ObservableValue<? extends String> observable, String oldValue, final String newValue) -> {
                    SwingUtilities.invokeLater(() -> {
                        DlgBerkasTTE.this.setTitle(newValue);
                    });
                });
                    ...
                    public void changed(ObservableValue ov, State oldState, State newState) {
                        if (newState == State.SUCCEEDED) {
                            try {
                                ...
                                if(engine.getLocation().replaceAll("http://"+koneksiDB.HOSTHYBRIDWEB()+":"+prop.getProperty("PORTWEB")+"/"+prop.getProperty("HYBRIDWEB")+"/","").contains("berkastte/pages")){
                                    setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                                    Valid.panggilUrl(engine.getLocation().replaceAll("http://"+koneksiDB.HOSTHYBRIDWEB()+":"+prop.getProperty("PORTWEB")+"/"+prop.getProperty("HYBRIDWEB")+"/berkastte/pages/upload/","berkastte/").replaceAll("http://"+koneksiDB.HOSTHYBRIDWEB()+"/"+prop.getProperty("HYBRIDWEB")+"/berkastte/pages/upload/","berkastte/"));
                                    ...
                                    ps=koneksi.prepareStatement("SELECT berkas_tte.lokasi_file "+
                                                  "from berkas_tte inner join master_berkas_tte "+
                                                  "on berkas_tte.kode=master_berkas_tte.kode "+
                                                  "where berkas_tte.no_rawat=? ORDER BY master_berkas_tte.nama ASC ");
                                    try {
                                        ...
                                        rs=ps.executeQuery();
                                        while(rs.next()){
                                            url = new URL("http://"+koneksiDB.HOSTHYBRIDWEB()+":"+prop.getProperty("PORTWEB")+"/"+prop.getProperty("HYBRIDWEB")+"/berkastte/"+rs.getString("lokasi_file"));
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgBerkasTTE dialog = new DlgBerkasTTE(new javax.swing.JFrame(), true);
