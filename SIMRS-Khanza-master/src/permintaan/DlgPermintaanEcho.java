 ...
 * @author dosen
 */
public final class DlgPermintaanEcho extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        InformasiTambahan.setBounds(252, 102, 380, 23);

        jLabel5.setText("Informasi Tambahan Permintaan Echo dll  :");
        jLabel5.setName("jLabel5"); // NOI18N
        PanelInput.add(jLabel5);
        jLabel5.setBounds(9, 102, 230, 23);
    ...
    */
    ...
    try {
        ...
        pspemeriksaan = koneksi.prepareStatement(
            ...
            "jns_perawatan_radiologi.kd_pj, jns_perawatan_radiologi.kelas " +
            "from jns_perawatan_radiologi where jns_perawatan_radiologi.status='1' " +
        ...
        if (cara_bayar_radiologi.equals("Yes") && kelas_radiologi.equals("No")) {
            if (response.isArray()) {
                for (JsonNode list : response) {
                    String namaPemeriksaan = list.path("NamaPemeriksaan").asText().toLowerCase();
                    if ((namaPemeriksaan.contains("echo") || namaPemeriksaan.contains("usg doppler")) &&
                        (list.path("KodePeriksa").asText().toLowerCase().contains(TCariPeriksa.getText().toLowerCase()) || 
                         namaPemeriksaan.contains(TCariPeriksa.getText().toLowerCase()))) {
            ...
            if (response.isArray()) {
                for (JsonNode list : response) {
                    ...
                    if ((list.path("Kelas").asText().equals(kelas.trim()) || list.path("Kelas").asText().equals("-")) &&
                        (list.path("KodePJ").asText().equals(Penjab.getText()) || list.path("KodePJ").asText().equals("-")) &&
                        (namaPemeriksaan.contains("echo") || namaPemeriksaan.contains("usg doppler")) &&
                        (list.path("KodePeriksa").asText().toLowerCase().contains(TCariPeriksa.getText().toLowerCase()) || 
                         namaPemeriksaan.contains(TCariPeriksa.getText().toLowerCase()))) {
                        tabMode.addRow(new Object[] {
                            false, list.path("KodePeriksa").asText(), list.path("NamaPemeriksaan").asText()
...
//        autoNomor();
    ...
    public void isCek(){        
        ...
        Tanggal.setEnabled(akses.getadmin());
        CmbJam.setEnabled(akses.getadmin());
        CmbMenit.setEnabled(akses.getadmin());
        CmbDetik.setEnabled(akses.getadmin());
...
// MODIF MUSTAFA
    ...
    if (reply == JOptionPane.YES_OPTION) {
        ...
        try {
            ...
            // Cek jika TNoPermintaan kosong, maka generate nomor permintaan
            if (TNoPermintaan.getText().trim().isEmpty()) {
                Valid.autoNomer3(
                    "select ifnull(MAX(CONVERT(RIGHT(permintaan_radiologi.noorder,4),signed)),0) from permintaan_radiologi where permintaan_radiologi.tgl_permintaan='" + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                    "PR" + Valid.SetTgl(Tanggal.getSelectedItem() + "").replaceAll("-", ""), 4, TNoPermintaan
                );
            }

            // Simpan data ke tabel permintaan_radiologi
            if (Sequel.menyimpantf2("permintaan_radiologi", "?,?,?,?,?,?,?,?,?,?,?,?", "No.Permintaan", 12, new String[] {
                    TNoPermintaan.getText(), TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    CmbJam.getSelectedItem() + ":" + CmbMenit.getSelectedItem() + ":" + CmbDetik.getSelectedItem(),
                    "0000-00-00", "00:00:00", "0000-00-00", "00:00:00", KodePerujuk.getText(), status.replaceAll("R", "r"),
                ...
                // Simpan data pemeriksaan radiologi
                for (i = 0; i < tbPemeriksaan.getRowCount(); i++) {
                    if (tbPemeriksaan.getValueAt(i, 0).toString().equals("true")) {
                        Sequel.menyimpan2("permintaan_pemeriksaan_radiologi", "?,?,?", "pemeriksaan lab", 3, new String[] {
                            TNoPermintaan.getText(), tbPemeriksaan.getValueAt(i, 1).toString(), "Belum"
                ...
                // Jika proses simpan gagal, coba lagi dengan generate nomor dan simpan data                
                if (TNoPermintaan.getText().trim().isEmpty()) {
                    Valid.autoNomer3(
                        "select ifnull(MAX(CONVERT(RIGHT(permintaan_radiologi.noorder,4),signed)),0) from permintaan_radiologi where permintaan_radiologi.tgl_permintaan='" + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                        "PR" + Valid.SetTgl(Tanggal.getSelectedItem() + "").replaceAll("-", ""), 4, TNoPermintaan
                ...
                if (Sequel.menyimpantf2("permintaan_radiologi", "?,?,?,?,?,?,?,?,?,?,?,?", "No.Permintaan", 12, new String[] {
                    ...
                    // Coba lagi dengan generate nomor dan simpan data
                    if (TNoPermintaan.getText().trim().isEmpty()) {
                    Valid.autoNomer3(
                            "select ifnull(MAX(CONVERT(RIGHT(permintaan_radiologi.noorder,4),signed)),0) from permintaan_radiologi where permintaan_radiologi.tgl_permintaan='" + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                            "PR" + Valid.SetTgl(Tanggal.getSelectedItem() + "").replaceAll("-", ""), 4, TNoPermintaan
