...
//                    iddokter = cekViaSatuSehat.tampilIDParktisi(tbObat.getValueAt(i, 12).toString());
                    ...
                    try {
                        ...
                        if (!response.asText().equals("")) {
                            Sequel.menyimpan(
                            "satu_sehat_episode_of_care(no_rawat,kd_penyakit,status,id_episode_of_care)",
                            "?,?,?,?",
                            "EpisodeOfCare",
                            4,
                            new String[]{
...
//        try {
//            String queryRalan = "select DISTINCT reg_periksa.tgl_registrasi,reg_periksa.jam_reg,reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.no_ktp,"
...
//
//            String queryRanap = "select DISTINCT reg_periksa.tgl_registrasi,reg_periksa.jam_reg,reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.no_ktp,"
...
private void tampil() {
    Valid.tabelKosong(tabMode);
    try {
        String filterCari = TCari.getText().equals("") ? "" :
                "and (reg_periksa.no_rawat like ? or reg_periksa.no_rkm_medis like ? or "
                + "pasien.nm_pasien like ? or pasien.no_ktp like ? or d.kd_penyakit like ? or penyakit.nm_penyakit like ? or "
                + "reg_periksa.stts like ? or reg_periksa.status_lanjut like ?) ";

        String queryRalan = "select distinct reg_periksa.tgl_registrasi,reg_periksa.jam_reg,reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.no_ktp,"
                + "reg_periksa.stts,reg_periksa.status_lanjut,date_format(pr.waktu_perawatan,'%Y-%m-%dT%H:%i:%s+07:00') as pulang,satu_sehat_encounter.id_encounter, "
                + "d.kd_penyakit,penyakit.nm_penyakit,ifnull(sse.id_episode_of_care,'') as id_episode_of_care "
                + "from reg_periksa "
                + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                + "inner join ( "
                + "    select no_rawat,max(concat(tgl_perawatan,' ',jam_rawat)) as waktu_perawatan "
                + "    from pemeriksaan_ralan group by no_rawat "
                + ") pr on pr.no_rawat=reg_periksa.no_rawat "
                + "inner join satu_sehat_encounter on satu_sehat_encounter.no_rawat=reg_periksa.no_rawat "
                + "inner join ( "
                + "    select distinct no_rawat,kd_penyakit,status "
                + "    from diagnosa_pasien "
                + "    where kd_penyakit like '%O%' "
                + ") d on d.no_rawat=reg_periksa.no_rawat "
                + "inner join penyakit on d.kd_penyakit=penyakit.kd_penyakit "
                + "left join ( "
                + "    select no_rawat,kd_penyakit,status,max(id_episode_of_care) as id_episode_of_care "
                + "    from satu_sehat_episode_of_care "
                + "    group by no_rawat,kd_penyakit,status "
                + ") sse on sse.no_rawat=d.no_rawat and sse.kd_penyakit=d.kd_penyakit and sse.status=d.status "
                + "where date(pr.waktu_perawatan) between ? and ? "
                + filterCari;

        String queryRanap = "select distinct reg_periksa.tgl_registrasi,reg_periksa.jam_reg,reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.no_ktp,"
                + "reg_periksa.stts,reg_periksa.status_lanjut,date_format(ki.waktu_keluar,'%Y-%m-%dT%H:%i:%s+07:00') as pulang,satu_sehat_encounter.id_encounter, "
                + "d.kd_penyakit,penyakit.nm_penyakit,ifnull(sse.id_episode_of_care,'') as id_episode_of_care "
                + "from reg_periksa "
                + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                + "inner join ( "
                + "    select no_rawat,max(concat(tgl_keluar,' ',jam_keluar)) as waktu_keluar "
                + "    from kamar_inap "
                ...
                + "    group by no_rawat,kd_penyakit,status "
                + ") sse on sse.no_rawat=d.no_rawat and sse.kd_penyakit=d.kd_penyakit and sse.status=d.status "
                + "where date(ki.waktu_keluar) between ? and ? "
                + filterCari;

        if (cmbStatus.getSelectedItem().toString().equals("Terkirim")) {
            queryRalan += " and sse.id_episode_of_care IS NOT NULL";
            queryRanap += " and sse.id_episode_of_care IS NOT NULL";
        }

        if (cmbStatus.getSelectedItem().toString().equals("Belum Terkirim")) {
            queryRalan += " and sse.id_episode_of_care IS NULL";
            queryRanap += " and sse.id_episode_of_care IS NULL";
        }

        queryRalan += " order by reg_periksa.tgl_registrasi,reg_periksa.jam_reg,reg_periksa.no_rawat,d.kd_penyakit";
        queryRanap += " order by reg_periksa.tgl_registrasi,reg_periksa.jam_reg,reg_periksa.no_rawat,d.kd_penyakit";
        ...
        try {
            ...
            while (rs.next()) {
                tabMode.addRow(new Object[]{
                    ...
                    rs.getString("id_episode_of_care")
                });
            }
        } catch (Exception e) {
