/*
 * PATCH RSAJ - Popup mandiri cek rujukan BPJS berdasarkan No.Kartu hasil pencarian NIK.
 * Versi UI diperbarui agar lebih fresh dan elegan, tanpa bergantung pada form BPJSCekRujukanKartuRS.
 */
package bridging;
...
public final class BPJSCekRujukanKartuRSPopup extends JDialog {
    ...
    private final JLabel LblRingkas = new JLabel("Belum ada data rujukan ditampilkan");
    ...
    public BPJSCekRujukanKartuRSPopup(Frame parent, boolean modal) {
        super(parent, modal);
        setTitle("Cek Rujukan BPJS Berdasarkan NIK");
    ...
    private JPanel buildHeaderPanel() {
        ...
        JLabel lblTitle = new JLabel("Cek Rujukan BPJS Berdasarkan NIK");
        lblTitle.setFont(FONT_JUDUL);
        lblTitle.setForeground(WARNA_AKSESEN_GELAP);

        JLabel lblDesc = new JLabel("Menampilkan data rujukan peserta BPJS secara ringkas dan rapi");
        lblDesc.setFont(FONT_SUBJUDUL);
        lblDesc.setForeground(WARNA_TEXT_SEKUNDER);

        kiri.add(lblTitle);
        kiri.add(Box.createVerticalStrut(4));
        kiri.add(lblDesc);

        JLabel chip = new JLabel("BPJS ONLINE", SwingConstants.CENTER);
        chip.setOpaque(true);
        ...
        JButton BtnCari = buatTombol("Cek Ulang", WARNA_AKSESEN, Color.WHITE);
        ...
        JButton BtnTutup = buatTombol("Tutup", new Color(241, 245, 249), WARNA_TEXT);
        BtnTutup.setBorder(BorderFactory.createLineBorder(new Color(203, 213, 225)));
        BtnTutup.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                dispose();
            }
        });
        formPanel.add(BtnTutup);

        LblRingkas.setFont(FONT_SUBJUDUL);
        LblRingkas.setForeground(WARNA_TEXT_SEKUNDER);
        LblRingkas.setBorder(new EmptyBorder(2, 0, 0, 2));

        searchCard.add(formPanel, BorderLayout.NORTH);
        searchCard.add(LblRingkas, BorderLayout.SOUTH);
    ...
    public void tampil(String nomorKartu) {
        ...
        try {
            ...
            LblRingkas.setText("Memproses data rujukan untuk No. Kartu " + nomorKartu.trim());

            String URL = link + "/Rujukan/RS/Peserta/" + nomorKartu.trim();
            ...
            if ("200".equals(metaData.path("code").asText())) {
                ...
                LblRingkas.setText("Peserta: " + nama + " | Poli Rujukan: " + poli);
                ...
                LblRingkas.setText("Rujukan untuk No. Kartu " + nomorKartu.trim() + " tidak ditemukan");
                LblStatus.setText(metaData.path("message").asText());
                JOptionPane.showMessageDialog(this, metaData.path("message").asText());
            }
        } catch (Exception ex) {
            System.out.println("Notifikasi Popup Rujukan Peserta : " + ex);
            tambah("Error", ex.toString());
            LblRingkas.setText("Terjadi kendala saat memproses permintaan");
