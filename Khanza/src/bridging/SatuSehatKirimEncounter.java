...
public final class SatuSehatKirimEncounter extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        jLabel18.setText("Status Encounter :");
        ...
        CmbStatusKirim.setPreferredSize(new java.awt.Dimension(130, 23));
        CmbStatusKirim.addItemListener(new java.awt.event.ItemListener() {
            public void itemStateChanged(java.awt.event.ItemEvent evt) {
                CmbStatusKirimItemStateChanged(evt);
            }
        });
    ...
    private void CmbStatusKirimItemStateChanged(java.awt.event.ItemEvent evt) {
        ...
        for(i=0;i<tbObat.getRowCount();i++){
            if(tbObat.getValueAt(i,0).toString().equals("true")&&(!tbObat.getValueAt(i,5).toString().equals(""))&&(!tbObat.getValueAt(i,8).toString().equals(""))&&tbObat.getValueAt(i,15).toString().equals("")){
                try {
                    ...
                    String[] dataPasienSatuSehat=ambilDataPasienUntukSatuSehat(tbObat.getValueAt(i,3).toString());
                    idpasien=cariIDPasienSatuSehat(tbObat.getValueAt(i,5).toString(),tbObat.getValueAt(i,4).toString(),dataPasienSatuSehat[0],dataPasienSatuSehat[1]);
                    if(iddokter==null||iddokter.trim().equals("")){
                        System.out.println("Notifikasi Bridging : Practitioner tidak ditemukan untuk NIK dokter "+tbObat.getValueAt(i,8).toString()+", Encounter "+tbObat.getValueAt(i,2).toString()+" tidak dikirim.");
                        continue;
                    }
                    if(idpasien==null||idpasien.trim().equals("")){
                        System.out.println("Notifikasi Bridging : Patient tidak ditemukan di SATUSEHAT untuk NIK "+tbObat.getValueAt(i,5).toString()+" / nama "+tbObat.getValueAt(i,4).toString()+" / tgl lahir "+dataPasienSatuSehat[0]+" / gender "+dataPasienSatuSehat[1]+", Encounter "+tbObat.getValueAt(i,2).toString()+" tidak dikirim.");
                        continue;
                    }
                    try{
    ...
    */
    ...
    private String[] ambilDataPasienUntukSatuSehat(String noRkmMedis){
        String[] data=new String[]{"",""};
        PreparedStatement psPasien=null;
        ResultSet rsPasien=null;
        try{
            psPasien=koneksi.prepareStatement("select tgl_lahir,jk from pasien where no_rkm_medis=?");
            psPasien.setString(1,noRkmMedis);
            rsPasien=psPasien.executeQuery();
            if(rsPasien.next()){
                data[0]=rsPasien.getString("tgl_lahir")==null?"":rsPasien.getString("tgl_lahir");
                data[1]=rsPasien.getString("jk")==null?"":rsPasien.getString("jk");
            }
        }catch(Exception e){
            System.out.println("Notifikasi Ambil Data Pasien SATUSEHAT : "+e);
        }finally{
            try{
                if(rsPasien!=null){
                    rsPasien.close();
                ...
                System.out.println("Notifikasi Tutup Data Pasien SATUSEHAT : "+e);
            }
        }
        return data;
    }

    private String cariIDPasienSatuSehat(String nikPasien,String namaPasien,String tglLahir,String jk){
        String idCari="";
        try{
            idCari=cekViaSatuSehat.tampilIDPasien(nikPasien);
        }catch(Exception e){
            System.out.println("Notifikasi Cek Patient By NIK : "+e);
        }

        if(idCari!=null&&!idCari.trim().equals("")){
            return idCari.trim();
        }

        try{
            String gender=konversiGenderSatuSehat(jk);
            if(namaPasien==null||namaPasien.trim().equals("")||tglLahir==null||tglLahir.trim().equals("")||gender.equals("")){
                System.out.println("Notifikasi Fallback Patient dibatalkan karena data kurang lengkap. NIK="+nikPasien+", Nama="+namaPasien+", TglLahir="+tglLahir+", JK="+jk);
                return "";
            }
            ...
            String urlFallback=link+"/Patient?name="+urlEncodeSatuSehat(namaPasien.trim())+"&birthdate="+urlEncodeSatuSehat(tglLahir.trim())+"&gender="+urlEncodeSatuSehat(gender);
            URI uriFallback=URI.create(urlFallback);
            System.out.println("URL Fallback Patient : "+uriFallback.toString());

            requestEntity = new HttpEntity(headers);
            String hasilFallback=api.getRest().exchange(uriFallback, HttpMethod.GET, requestEntity, String.class).getBody();
            System.out.println("JSON Fallback Patient : "+hasilFallback);

            JsonNode rootFallback=mapper.readTree(hasilFallback);
            if(rootFallback.path("total").asInt()>0&&rootFallback.path("entry").isArray()){
                for(int idx=0;idx<rootFallback.path("entry").size();idx++){
                    String idFallback=rootFallback.path("entry").get(idx).path("resource").path("id").asText();
                    if(idFallback!=null&&!idFallback.trim().equals("")){
                        System.out.println("Patient ditemukan via fallback name+birthdate+gender : "+idFallback);
                        return idFallback.trim();
                    }
                }
            }
        }catch(HttpClientErrorException e){
            System.out.println("Notifikasi Fallback Patient : "+e);
            System.out.println("Response Body Fallback Patient : "+e.getResponseBodyAsString());
        }catch(Exception e){
            System.out.println("Notifikasi Fallback Patient : "+e);
        }
    ...
    private String konversiGenderSatuSehat(String jk){
        if(jk==null){
            return "";
        }
        String jkUpper=jk.trim().toUpperCase();
        if(jkUpper.equals("L")||jkUpper.equals("LAKI-LAKI")||jkUpper.equals("LAKI LAKI")||jkUpper.equals("MALE")){
            return "male";
        }else if(jkUpper.equals("P")||jkUpper.equals("PEREMPUAN")||jkUpper.equals("FEMALE")){
            return "female";
        }
        return "";
    }

    // Catatan: URL hasil encode harus dikirim memakai exchange(URI, ...), bukan exchange(String, ...), agar %20 tidak berubah menjadi %2520.
    private String urlEncodeSatuSehat(String value) throws Exception{
        return URLEncoder.encode(value==null?"":value,"UTF-8").replace("+","%20");
    }

    private String getKriteriaStatusEncounter(){
        if(CmbStatusKirim==null||CmbStatusKirim.getSelectedItem()==null){
            return "";
        }
        String status=CmbStatusKirim.getSelectedItem().toString();
        if(status.equals("Belum Terkirim")){
            return "and trim(ifnull(satu_sehat_encounter.id_encounter,''))='' ";
        }else if(status.equals("Sudah Terkirim")){
            return "and trim(ifnull(satu_sehat_encounter.id_encounter,''))<>'' ";
        }
        return "";
    }
    ...
    private void tampil() {
        ...
        try{
            ps=koneksi.prepareStatement(
                   ...
                   "where nota_jalan.tanggal between ? and ? "+getKriteriaStatusEncounter()+
            ...
            ps=koneksi.prepareStatement(
                   ...
                   "where nota_inap.tanggal between ? and ? "+getKriteriaStatusEncounter()+
                   (TCari.getText().equals("")?"":"and (reg_periksa.no_rawat like ? or reg_periksa.no_rkm_medis like ? or "+
                   "pasien.nm_pasien like ? or pasien.no_ktp like ? or pegawai.nama like ? or poliklinik.nm_poli like ? or "+
                   "reg_periksa.stts like ? or reg_periksa.status_lanjut like ?)"));
            try {
