...
public class ApiBPJSSisrute {        
    ...
    public ApiBPJSSisrute(){
        try {
            Key = koneksiDB.SECRETKEYAPIBPJSRUJUKANSISRUTE();
            Consid = koneksiDB.CONSIDAPIBPJSRUJUKANSISRUTE();
        } catch (Exception ex) {
...
//    final javax.net.ssl.SSLContext ctx = javax.net.ssl.SSLContext.getInstance("TLSv1.2");
...
//                https.setHostnameVerifier((h, s) -> true); // dev only
//            }
...
//}
    ...
    SimpleClientHttpRequestFactory f = new SimpleClientHttpRequestFactory() {
        ...
        protected void prepareConnection(java.net.HttpURLConnection conn, String method)
            ...
            conn.setConnectTimeout(15000);
            conn.setReadTimeout(60000);
            conn.setRequestProperty("Connection", "close");   // <-- BARIS BARU
            if (conn instanceof javax.net.ssl.HttpsURLConnection) {
                javax.net.ssl.HttpsURLConnection https = (javax.net.ssl.HttpsURLConnection) conn;
                https.setSSLSocketFactory(sf);
                https.setHostnameVerifier((h, s) -> true); // dev only
