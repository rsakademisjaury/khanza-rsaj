...
//                    "dokter.nm_dokter,reg_periksa.no_rkm_medis,pasien.nm_pasien,poliklinik.nm_poli,penjab.png_jawab,bridging_sep.no_sep,"+
...
//                    "left join bridging_sep on periksa_radiologi.no_rawat=bridging_sep.no_rawat inner join kabupaten inner join kecamatan inner join kelurahan "+
...
//                    "group by reg_periksa.no_rawat,periksa_radiologi.tgl_periksa,periksa_radiologi.jam order by periksa_radiologi.tgl_periksa,reg_periksa.jam_reg");
                ...
                "concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab) as almt_pj," +
                "pasien.jk,concat(reg_periksa.umurdaftar,' ',reg_periksa.sttsumur) as umur, " +
                "pasien.no_ktp, pasien.no_tlp, pasien.tgl_lahir, " +                
                "ifnull(sum(case when periksa_radiologi.no_rawat=reg_periksa.no_rawat and periksa_radiologi.kd_jenis_prw is not null then periksa_radiologi.biaya else 0 end),0) as total_bayar " +
                "from periksa_radiologi inner join reg_periksa inner join dokter inner join pasien inner join poliklinik inner join penjab " +
                "left join bridging_sep on periksa_radiologi.no_rawat=bridging_sep.no_rawat inner join kabupaten inner join kecamatan inner join kelurahan "+
                "on periksa_radiologi.dokter_perujuk=dokter.kd_dokter and periksa_radiologi.no_rawat=reg_periksa.no_rawat " +
                "and reg_periksa.no_rkm_medis=pasien.no_rkm_medis and reg_periksa.kd_pj=penjab.kd_pj " +
            ...
            try {
                ...
                rs=ps.executeQuery();
                i=1;   
                laki=0;per=0;jmldiagnosa=0;jmlnolab=0; double totalSemuaBiaya = 0;
                while(rs.next()){
                    umurlk="";
                    umurpr="";
                    ...
                    totalSemuaBiaya += totalBayar; // Menjumlahkan total bayar setiap baris
                    ...
                    tabMode.addRow(new Object[]{
                        i,//0
                        rs.getString("tgl_periksa")+" "+rs.getString("jam"),//1
                        no_lab,//2
                        rs.getString("no_rkm_medis"),//3
                        rs.getString("nm_pasien"),//4
                        rs.getString("no_ktp"),//5
                        rs.getString("no_tlp"),//6
                        rs.getString("tgl_lahir"), //7                       
                        umurlk,//8
                        umurpr,//9
                        rs.getString("png_jawab"),//10
                        rs.getString("no_sep"),//11
                        rs.getString("almt_pj"),//12
                        kddiangnosa,//13
                        diagnosa,//14
                        tindakan,//15
                        rs.getString("nm_dokter"),//16
                        rs.getString("nm_poli"),//17                   
                        rs.getDouble("total_bayar") //18                           
                    });                
                    i++;
                }
                if(i>1){
                    tabMode.addRow(new Object[]{
                        ">>","Total : ",jmlnolab,(i-1),"","","","",laki,per,"","","",jmldiagnosa,"","","","",totalSemuaBiaya
                    });
                }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
