...
public class DlgRL53_BesarKunjunganPenyakitRawatJalan extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgRL53_BesarKunjunganPenyakitRawatJalan(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        // PATCH SIRS 6.3: form ini hanya menggunakan satu tab sesuai juknis.
        // Komponen tab kedua tetap dibiarkan agar source hasil NetBeans tidak banyak berubah.
        TabRawat.remove(Scroll1);
        TabRawat.setTitleAt(0, "RL 5.3 Rawat Jalan");
    ...
    private void initComponents() {
        ...
        jLabel1.setText("RL. 5.3 10 Besar Kunjungan Penyakit Rawat Jalan");
                            ...
                            "<table width='100%' border='0' align='center' cellpadding='3px' cellspacing='0' class='tbl_form'>"+
                                "<tr class='isi2'>"+
                                    "<td valign='top' align='center'>"+
                                        ...
                                        "<font size='2' face='Tahoma'>RL 5.3 10 BESAR KUNJUNGAN PENYAKIT RAWAT JALAN<br>PERIODE "+Tgl1.getSelectedItem()+" s.d. "+Tgl2.getSelectedItem()+"<br><br></font>"+        
                                    "</td>"+
                               "</tr>"+
                            "</table>")
                );
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgRL53_BesarKunjunganPenyakitRawatJalan dialog = new DlgRL53_BesarKunjunganPenyakitRawatJalan(new javax.swing.JFrame(), true);
...
private void prosesCari() {
    ...
    try {
        ...
        htmlContent.append("<tr class='head'>"
            + "<td rowspan='2' bgcolor='#F7F7F7' width='4%' align='center'>No.</td>"
            + "<td rowspan='2' bgcolor='#F7F7F7' width='8%' align='center'>Kelompok ICD-10</td>"
            + "<td rowspan='2' bgcolor='#F7F7F7' align='center'>Kelompok Diagnosis Penyakit</td>"
            + "<td colspan='3' bgcolor='#F7F7F7' align='center'>Jumlah Kasus Baru Menurut Jenis Kelamin</td>"
            + "<td colspan='3' bgcolor='#F7F7F7' align='center'>Jumlah Kunjungan</td></tr>");
        htmlContent.append("<tr class='head'>"
            + "<td bgcolor='#F7F7F7' width='7%' align='center'>Laki-Laki</td>"
            + "<td bgcolor='#F7F7F7' width='7%' align='center'>Perempuan</td>"
            + "<td bgcolor='#F7F7F7' width='7%' align='center'>Total</td>"
            + "<td bgcolor='#F7F7F7' width='7%' align='center'>Laki-Laki</td>"
            + "<td bgcolor='#F7F7F7' width='7%' align='center'>Perempuan</td>"
            + "<td bgcolor='#F7F7F7' width='7%' align='center'>Total</td></tr>");

        String tgl1 = Valid.SetTgl(Tgl1.getSelectedItem() + "");
        String tgl2 = Valid.SetTgl(Tgl2.getSelectedItem() + "");

        String queryBaru = "SELECT LEFT(dp.kd_penyakit,3) AS kelompok_icd, SUBSTRING(pyk.nm_penyakit,1,80) AS nm_penyakit, p.jk " +
                           ...
                           "WHERE dp.status = 'Ralan' AND dp.prioritas = 1 AND rp.stts <> 'Batal' " +
        ...
        ps = koneksi.prepareStatement(queryBaru);
        ps.setString(1, tgl1);
        ps.setString(2, tgl2);
        rs = ps.executeQuery();

        Map<String, Integer> kasusBaruL = new HashMap<>();
        Map<String, Integer> kasusBaruP = new HashMap<>();
        Map<String, String> namaPenyakitMap = new HashMap<>();

        while (rs.next()) {
            String kode = rs.getString("kelompok_icd");
            String nama = rs.getString("nm_penyakit");
            String jk = rs.getString("jk");
            namaPenyakitMap.putIfAbsent(kode, nama);
            if ("L".equalsIgnoreCase(jk)) {
                kasusBaruL.put(kode, kasusBaruL.getOrDefault(kode, 0) + 1);
            } else if ("P".equalsIgnoreCase(jk)) {
                kasusBaruP.put(kode, kasusBaruP.getOrDefault(kode, 0) + 1);
            }
        }
        rs.close(); ps.close();

        String queryKunjungan = "SELECT LEFT(dp.kd_penyakit,3) AS kelompok_icd, SUBSTRING(pyk.nm_penyakit,1,80) AS nm_penyakit, p.jk " +
                                "FROM diagnosa_pasien dp " +
                                "INNER JOIN penyakit pyk ON dp.kd_penyakit = pyk.kd_penyakit " +
                                "INNER JOIN reg_periksa rp ON dp.no_rawat = rp.no_rawat " +
                                "INNER JOIN pasien p ON rp.no_rkm_medis = p.no_rkm_medis " +
        ...
        Map<String, Integer> kunjunganL = new HashMap<>();
        Map<String, Integer> kunjunganP = new HashMap<>();
        ...
        while (rs.next()) {
            ...
            String jk = rs.getString("jk");
            if ("L".equalsIgnoreCase(jk)) {
                kunjunganL.put(kode, kunjunganL.getOrDefault(kode, 0) + 1);
            } else if ("P".equalsIgnoreCase(jk)) {
                kunjunganP.put(kode, kunjunganP.getOrDefault(kode, 0) + 1);
            }
        }

        List<String> kodeList = new ArrayList<>(namaPenyakitMap.keySet());
        kodeList.sort((a, b) -> {
            int totalB = kunjunganL.getOrDefault(b, 0) + kunjunganP.getOrDefault(b, 0);
            int totalA = kunjunganL.getOrDefault(a, 0) + kunjunganP.getOrDefault(a, 0);
            return Integer.compare(totalB, totalA);
        });

        int no = 1;
        for (String kode : kodeList.stream().limit(10).collect(Collectors.toList())) {
            int baruL = kasusBaruL.getOrDefault(kode, 0);
            int baruP = kasusBaruP.getOrDefault(kode, 0);
            int totalBaru = baruL + baruP;
            int kunjL = kunjunganL.getOrDefault(kode, 0);
            int kunjP = kunjunganP.getOrDefault(kode, 0);
            int totalKunjungan = kunjL + kunjP;
            htmlContent.append("<tr class='isi'><td align='center'>").append(no++).append("</td>")
                .append("<td align='center'>").append(kode).append("</td>")
                .append("<td>").append(namaPenyakitMap.get(kode)).append("</td>")
                .append("<td align='center'>").append(baruL).append("</td>")
                .append("<td align='center'>").append(baruP).append("</td>")
                .append("<td align='center'>").append(totalBaru).append("</td>")
                .append("<td align='center'>").append(kunjL).append("</td>")
                .append("<td align='center'>").append(kunjP).append("</td>")
                .append("<td align='center'>").append(totalKunjungan).append("</td></tr>");
        }

        htmlContent.append("</table></body></html>");
        LoadHTML.setText(htmlContent.toString());
    } catch (Exception e) {
        System.out.println("Notifikasi prosesCariRL53 SIRS 6.3: " + e);
    } finally {
