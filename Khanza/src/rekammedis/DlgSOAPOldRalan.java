...
public class DlgSOAPOldRalan extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgSOAPOldRalan(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
         setSize(1285,674);
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgSOAPOldRalan dialog = new DlgSOAPOldRalan(new javax.swing.JFrame(), true);
...
//                                 System.out.println("Notif Rekening : "+rs2.getString("nik"));
                    ...
                    try {
                        rs2=koneksi.prepareStatement(
                                "select pemeriksaan_ralan.tgl_perawatan,pemeriksaan_ralan.jam_rawat,pemeriksaan_ralan.keluhan,"+
                                "pemeriksaan_ralan.pemeriksaan,pemeriksaan_ralan.rtl,pemeriksaan_ralan.penilaian,pemeriksaan_ralan.nip,pegawai.nama,departemen.nama, "+
                                "validasi_pemeriksaan_ralan.nik_validator,namavalidator.nama as namavalidator,validasi_pemeriksaan_ralan.tgl_validasi,validasi_pemeriksaan_ralan.jam_validasi,validasi_pemeriksaan_ralan.status_validasi "+ 
                                "from pemeriksaan_ralan LEFT JOIN validasi_pemeriksaan_ralan ON validasi_pemeriksaan_ralan.no_rawat = pemeriksaan_ralan.no_rawat AND validasi_pemeriksaan_ralan.tgl_perawatan = pemeriksaan_ralan.tgl_perawatan AND validasi_pemeriksaan_ralan.jam_rawat = pemeriksaan_ralan.jam_rawat inner join pegawai on pemeriksaan_ralan.nip=pegawai.nik inner join departemen on pegawai.departemen=departemen.dep_id LEFT JOIN pegawai AS namavalidator ON validasi_pemeriksaan_ralan.nik_validator=namavalidator.nik where pemeriksaan_ralan.no_rawat='"+rs.getString("no_rawat")+"' "+
                                "order by pemeriksaan_ralan.tgl_perawatan,pemeriksaan_ralan.jam_rawat").executeQuery();
                        if(rs2.next()){
                            htmlContent.append(
                                    "<tr class='isi'>"+
                            ...
                            while(rs2.next()){
                                ...
                                if(Sequel.cariInteger("select count(validasi_pemeriksaan_ralan.nik_validator) " +
                                        "from pemeriksaan_ralan LEFT JOIN validasi_pemeriksaan_ralan ON validasi_pemeriksaan_ralan.no_rawat = pemeriksaan_ralan.no_rawat AND validasi_pemeriksaan_ralan.tgl_perawatan = pemeriksaan_ralan.tgl_perawatan AND validasi_pemeriksaan_ralan.jam_rawat = pemeriksaan_ralan.jam_rawat inner join pegawai on pemeriksaan_ralan.nip=pegawai.nik inner join departemen on pegawai.departemen=departemen.dep_id LEFT JOIN pegawai AS namavalidator ON validasi_pemeriksaan_ralan.nik_validator=namavalidator.nik where pemeriksaan_ralan.no_rawat='"+rs.getString("no_rawat")+"' AND validasi_pemeriksaan_ralan.nik_validator='"+rs2.getString("nik_validator")+"'" +
                                        "order by pemeriksaan_ralan.tgl_perawatan,pemeriksaan_ralan.jam_rawat")>0){
