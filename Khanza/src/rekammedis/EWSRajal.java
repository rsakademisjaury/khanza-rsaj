 ...
 * @author perpustakaan
 */
public final class EWSRajal extends javax.swing.JDialog {
...
//    private SimpleDateFormat tanggalNow = new SimpleDateFormat("dd-MM-yyyy");
     ...
     * @param parent
     * @param modal */
    public EWSRajal(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
...
//        isMenu();
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TNoRM.getText().trim().equals("")){
            ...
            if(Sequel.menyimpantf("ews_rajal","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",38,new String[]{
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if(tbObat.getSelectedRow()>-1){
            if(Sequel.queryu2tf("delete from ews_rajal where jam=?",1,new String[]{
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            EWSRajal dialog = new EWSRajal(new javax.swing.JFrame(), true);
    ...
    public void tampil() {
        ...
        try{
            if(TCari.getText().equals("")){
                ps=koneksi.prepareStatement(
                        "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,if(pasien.jk='L','Laki-Laki','Perempuan') as jk,pasien.tgl_lahir,ews_rajal.tanggal,ews_rajal.jam,ews_rajal.nik,pegawai.nama,"+
                        "ews_rajal.pernafasan,ews_rajal.score_pernafasan,ews_rajal.saturasi,ews_rajal.score_saturasi,ews_rajal.alat,ews_rajal.score_alat,ews_rajal.suhu,ews_rajal.score_suhu,"+
                        "ews_rajal.denyut,ews_rajal.score_denyut,ews_rajal.tekanan,ews_rajal.score_tekanan,ews_rajal.kesadaran,ews_rajal.score_kesadaran,ews_rajal.total_score,ews_rajal.klasifikasi,ews_rajal.respon,"+
                        "ews_rajal.tindakan,ews_rajal.frekuensi,ews_rajal.skala_nyeri,ews_rajal.bb,ews_rajal.tb,ews_rajal.lk,ews_rajal.lp,ews_rajal.masuk1,ews_rajal.masuk2,ews_rajal.jumlahmasuk,"+
                        "ews_rajal.keluar1,ews_rajal.keluar2,ews_rajal.keluar3,ews_rajal.keluar4,ews_rajal.keluar5,ews_rajal.jumlahkeluar,ews_rajal.bc "+
                        "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                        "inner join ews_rajal on reg_periksa.no_rawat=ews_rajal.no_rawat "+
                        "inner join pegawai on ews_rajal.nik=pegawai.nik "+
                        "inner join bahasa_pasien on bahasa_pasien.id=pasien.bahasa_pasien "+
                        "inner join cacat_fisik on cacat_fisik.id=pasien.cacat_fisik where "+
                        "ews_rajal.tanggal between ? and ? order by ews_rajal.tanggal");
            }else{
                ps=koneksi.prepareStatement(
                        "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,if(pasien.jk='L','Laki-Laki','Perempuan') as jk,pasien.tgl_lahir,ews_rajal.tanggal,ews_rajal.jam,ews_rajal.nik,pegawai.nama,"+
                        "ews_rajal.pernafasan,ews_rajal.score_pernafasan,ews_rajal.saturasi,ews_rajal.score_saturasi,ews_rajal.alat,ews_rajal.score_alat,ews_rajal.suhu,ews_rajal.score_suhu,"+
                        ...
                        "inner join bahasa_pasien on bahasa_pasien.id=pasien.bahasa_pasien "+
                        "inner join cacat_fisik on cacat_fisik.id=pasien.cacat_fisik where "+
                        "ews_rajal.tanggal between ? and ? and reg_periksa.no_rawat like ? or "+
                        "ews_rajal.tanggal between ? and ? and pasien.no_rkm_medis like ? or "+
                        "ews_rajal.tanggal between ? and ? and pasien.nm_pasien like ? or "+
                        "ews_rajal.tanggal between ? and ? and ews_rajal.nik like ? or "+
                        "ews_rajal.tanggal between ? and ? and pegawai.nama like ? order by ews_rajal.tanggal");
            }
