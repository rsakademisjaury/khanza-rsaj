...
public final class DlgRujuk extends javax.swing.JDialog {
    ...
    private String diagnosa="",diagnosa2="",keluar="",status="",tgl="",sql="",finger="",FileName = "", kodeberkas = "";
    ...
    private void initComponents() {
        ...
        DTPRujuk.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "22-05-2025" }));
...
private void TDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_TDokterKeyPressed
    ...
    private void MnSuratRujukanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnSuratRujukanActionPerformed
        if(TPasien.getText().trim().equals("")){
            ...
            Valid.MyReportqry2("rptSuratRujukan_updated.jasper", "report", "::[ Surat Rujukan ]::",
                ...
                "reg_periksa.almt_pj,pasien.umur,reg_periksa.tgl_registrasi,DATE_FORMAT( rujuk.tgl_rujuk, '%d %M %Y' ) AS tgl_rujuk "+
                "from reg_periksa inner join pasien inner join rujuk inner join dokter on "+
                "reg_periksa.no_rkm_medis=pasien.no_rkm_medis and reg_periksa.no_rawat=rujuk.no_rawat and "+
                "rujuk.kd_dokter=dokter.kd_dokter where reg_periksa.no_rawat='"+TNoRw.getText()+"'", param);
    ...
    private void BtnUploadActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnUploadActionPerformed
        ...
        FileName = timestamp + "_" + tbObat.getValueAt(tbObat.getSelectedRow(), 1).toString().replace("/", "") + "_Surat_Rujuk_Antar_RS";
...
//            Valid.MyReportqry("rptSuratRujukan.jasper","report","::[ Surat Rujukan ]::",
            Valid.MyReportPDFqryUpload("rptSuratRujukan_updated.jasper", "report", "::[ Surat Rujukan ]::",
                "select rujuk.rujuk_ke,rujuk.no_rujuk,reg_periksa.no_rawat,pasien.alamat,dokter.nm_dokter, "+
                "reg_periksa.no_rkm_medis,pasien.jk,pasien.keluarga,pasien.namakeluarga,pasien.tgl_lahir,pasien.nm_pasien,"+
                "reg_periksa.almt_pj,pasien.umur,reg_periksa.tgl_registrasi,DATE_FORMAT( rujuk.tgl_rujuk, '%d %M %Y' ) AS tgl_rujuk "+
                "from reg_periksa inner join pasien inner join rujuk inner join dokter on "+
                "reg_periksa.no_rkm_medis=pasien.no_rkm_medis and reg_periksa.no_rawat=rujuk.no_rawat and "+
                "rujuk.kd_dokter=dokter.kd_dokter where reg_periksa.no_rawat='"+TNoRw.getText()+"'",FileName,param);
            this.setCursor(Cursor.getDefaultCursor());
        }
}
