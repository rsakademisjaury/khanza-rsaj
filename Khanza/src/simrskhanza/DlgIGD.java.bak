...
import bridging.SatuSehatRujukanForm;
...
public final class DlgIGD extends javax.swing.JDialog {
    ...
    // Dibuat saat benar-benar dipakai agar pembukaan IGD tidak menunggu form pendukung.
    ...
    // PATCH RSAJ - autocomplete IGD memakai cache lokal per form.
    // Data dokter, jenis bayar, dan asal rujukan dimuat sekali saat diperlukan.
    private static final class CacheDokterIgd {
        private final String kode, nama;
        private CacheDokterIgd(String kode, String nama) {
            this.kode = kode == null ? "" : kode;
            this.nama = nama == null ? "" : nama;
        }
    }
    private static final class CachePenjabIgd {
        private final String kode, nama;
        private CachePenjabIgd(String kode, String nama) {
            this.kode = kode == null ? "" : kode;
            this.nama = nama == null ? "" : nama;
        }
    }
    private static final class CachePerujukIgd {
        private final String nama, alamat;
        private CachePerujukIgd(String nama, String alamat) {
            this.nama = nama == null ? "" : nama;
            this.alamat = alamat == null ? "" : alamat;
        }
    }
    private final java.util.List<CacheDokterIgd> cacheDokterIgd = new java.util.ArrayList<CacheDokterIgd>();
    private final java.util.List<CachePenjabIgd> cachePenjabIgd = new java.util.ArrayList<CachePenjabIgd>();
    private final java.util.List<CachePerujukIgd> cachePerujukIgd = new java.util.ArrayList<CachePerujukIgd>();
    private boolean cacheDokterIgdSiap = false;
    private boolean cachePenjabIgdSiap = false;
    private boolean cachePerujukIgdSiap = false;

    private final DefaultTableModel modelDokterIgd = new DefaultTableModel(new Object[]{"Kode", "Dokter IGD"}, 0) {
        @Override public boolean isCellEditable(int row, int column) { return false; }
    };
    private final DefaultTableModel modelPenjabIgd = new DefaultTableModel(new Object[]{"Kode", "Jenis Bayar"}, 0) {
        @Override public boolean isCellEditable(int row, int column) { return false; }
    };
    private final DefaultTableModel modelPerujukIgd = new DefaultTableModel(new Object[]{"Asal Rujukan", "Alamat"}, 0) {
        @Override public boolean isCellEditable(int row, int column) { return false; }
    };

    private javax.swing.JPopupMenu popupDokterIgd, popupPenjabIgd, popupPerujukIgd;
    private JTable tabelDokterIgd, tabelPenjabIgd, tabelPerujukIgd;
    private Timer timerDokterIgd, timerPenjabIgd, timerPerujukIgd;
    private javax.swing.JLabel petunjukDokterIgd, petunjukPenjabIgd, petunjukPerujukIgd;
    private boolean sedangIsiAutocompleteIgd = false;
    private boolean sedangIsiJenisBayarOtomatisIgd = false;

    private static final int MIN_AUTOCOMPLETE_IGD = 2;
    private static final int BARIS_AUTOCOMPLETE_IGD = 30;
    private static final int HEADER_AUTOCOMPLETE_IGD = 26;
    private static final int MAX_BARIS_AUTOCOMPLETE_IGD = 11;
    private static final Color PILIH_AUTOCOMPLETE_IGD = new Color(0,112,192);
    private static final Color TEKS_PILIH_AUTOCOMPLETE_IGD = Color.WHITE;
    private static final Color LATAR_GANJIL_AUTOCOMPLETE_IGD = new Color(248,250,252);
    private static final Color LATAR_GENAP_AUTOCOMPLETE_IGD = Color.WHITE;
    private static final Color TEKS_NORMAL_AUTOCOMPLETE_IGD = new Color(31,41,55);
    private static final Color GARIS_AUTOCOMPLETE_IGD = new Color(220,228,236);
    private static final Color HEADER_AUTOCOMPLETE_BG_IGD = new Color(236,244,252);
    private static final Color HEADER_AUTOCOMPLETE_FG_IGD = new Color(30,78,121);
    private static final Color PETUNJUK_AUTOCOMPLETE_IGD = new Color(148,163,184);

    // PATCH RSAJ - Bio/Finger BPJS dan Foto KTP pada DlgIGD.
    private static final int IDX_IGD_KD_PJ = 19;
    private static final int IDX_IGD_NO_SEP = 23;
    private static final int IDX_IGD_FINGER = 24;
    private static final int INTERVAL_PANTAU_FINGER_IGD = 15000;
    private Timer timerPantauFingerIGD;

    // Foto KTP memakai rasio kartu elektronik 85,60 x 53,98 mm.
    private static final int FOTO_KTP_LEBAR = 250;
    private static final int FOTO_KTP_TINGGI = 158;
    private static final int DELAY_MUAT_FOTO_KTP = 400;
    private static final int TIMEOUT_KONEKSI_FOTO_KTP = 3000;
    private static final int TIMEOUT_BACA_FOTO_KTP = 5000;
    private ImageIcon ikonKameraKTP, ikonZoomKTP;
    private BufferedImage fotoKTPAsli;
    private String noRMFotoKTP = "";
    private String noRMFotoKTPTerakhir = "";
    private String noRMFotoKTPDiminta = "";
    private boolean fotoKTPTersediaTerakhir = false;
    private long nomorProsesFotoKTP = 0L;
    private Timer timerMuatFotoKTP;
    private SwingWorker<BufferedImage, Void> workerMuatFotoKTP;
    private SwingWorker<BufferedImage, Void> workerMuatFotoKTPBesar;
    private SwingWorker<BufferedImage, Void> workerCetakFotoKTP;
    private char ESC = 27;
    // ganti kertas
    ...
    /*
     * Validasi nama pasien sebelum simpan.
     * Penghapusan otomatis hanya untuk sapaan/penanda yang berdiri sendiri
     * di awal atau akhir nama: Tn., Ny., Nn., dan An. Gelar akademis hanya
     * menjadi pengingat dan tidak pernah dihapus otomatis.
     */
    private static final Pattern POLA_SAPAAN_AWAL_NAMA_PASIEN = Pattern.compile(
            "(?i)^\\s*(?:tn|ny|nn|an)\\s*\\.?\\s+(?=\\S)"
    );
    private static final Pattern POLA_SAPAAN_AKHIR_NAMA_PASIEN = Pattern.compile(
            "(?i)(?:\\s*,\\s*|\\s+)(?:tn|ny|nn|an)\\s*\\.?\\s*$"
    );
    private static final Pattern POLA_GELAR_AKADEMIS_NAMA_PASIEN = Pattern.compile(
            "(?i)(^|[\\s,])(?:dr|drs|dra|prof|ir|h|hj|kh|ust|ustadz|"
            + "s\\s*\\.\\s*[a-z]{1,10}(?:\\s*\\.\\s*[a-z]{1,5})?|"
            + "m\\s*\\.\\s*[a-z]{1,10}(?:\\s*\\.\\s*[a-z]{1,5})?|"
            + "sp\\s*\\.\\s*[a-z]{1,10}|ph\\s*\\.\\s*d|mba|mm)\\.?($|[\\s,])"
    );
    private static final int AKSI_VALIDASI_NAMA_BATAL = 0;
    private static final int AKSI_VALIDASI_NAMA_HAPUS_SAPAAN = 1;
    private static final int AKSI_VALIDASI_NAMA_LANJUT = 2;
    private static final int AKSI_VALIDASI_NAMA_EDIT_MANUAL = 3;

    /*
     * Menjaga alur simpan setelah petugas menyimpan Edit Nama Manual.
     * Nilai bypass hanya berlaku sekali, khusus untuk pemanggilan ulang
     * tombol Simpan yang dijadwalkan setelah nama pasien sukses diperbarui.
     */
    private boolean lewatiValidasiNamaPasienSekali = false;
    private boolean simpanLanjutanSetelahEditNamaPasienDijadwalkan = false;
    ...
    public DlgIGD(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        BtnZoomFotoKTP.setIcon(buatIkonZoomKTP());
        initIGD();
        initPopupVisumEtRepertum();
        initTombolHasilPenunjangIGD();
        ...
        Object[] row={"P","Reg","No.Rawat","Tanggal","Jam","Kd.Dokter","Dokter Dituju","Nomer RM",
            ...
            "Biaya Regristrasi","Status","Jenis Bayar","Stts Rawat","Kd PJ","Status Bayar","SO","Riwayat Terakhir",
            "No. SEP","Finger Pasien"};
        ...
        // Renderer asli IGD tetap dipakai untuk seluruh tabel agar seluruh warna/status
        // bawaan DlgIGD tetap utuh. Khusus kolom "Reg" (index 1), bila Finger pasien
        // sudah tercatat maka dipakai renderer indikator sendiri. Jangan mendelegasikan
        // warna hijaunya ke WarnaTableIGD karena komponen renderer tersebut dapat dipakai
        // ulang dan warna pilihan/warna status IGD dapat menimpa indikator Finger.
        final WarnaTableIGD rendererIGD = new WarnaTableIGD(koneksi);
        tbPetugas.setDefaultRenderer(Object.class, rendererIGD);
        tbPetugas.getColumnModel().getColumn(1).setCellRenderer(new javax.swing.table.DefaultTableCellRenderer() {
            @Override
            public Component getTableCellRendererComponent(javax.swing.JTable table, Object value, boolean isSelected,
                ...
                if (row >= 0 && table.getModel().getColumnCount() > IDX_IGD_FINGER) {
                    int barisModel = table.convertRowIndexToModel(row);
                    Object nilaiFinger = table.getModel().getValueAt(barisModel, IDX_IGD_FINGER);
                    String statusFinger = nilaiFinger == null ? "" : nilaiFinger.toString().trim();
                    sudahFinger = !statusFinger.equals("") &&
                            !statusFinger.equals("-") &&
                            !statusFinger.equalsIgnoreCase("Belum Sidik Jari");
                }

                // Bila SUDAH Finger, gambar sel Reg secara mandiri supaya warna tetap terlihat,
                // termasuk ketika baris sedang dipilih oleh user.
                if (sudahFinger) {
                    ...
                    label.setToolTipText("Sudah Bio / Finger");
                    return label;
                }

                // Selain yang sudah Finger, kembalikan sepenuhnya ke renderer asli IGD
                // agar penanda/status warna lama tidak berubah sedikit pun.
                return rendererIGD.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
            }
        });
        ...
        nmpnj.setDocument(new batasInput((byte)100).getKata(nmpnj));
        ...
        // Dipasang terakhir agar listener Foto KTP menempel pada Document TNoRM yang final.
        initBioFingerDanFotoKTPIGD();
    }
    
    

    /**
     * Dialog pencarian pendukung dibuat secara lazy. Sebelumnya tiga dialog
     * besar dibuat bersamaan saat menu IGD diklik sehingga frame IGD belum
     * sempat tampil.
     */
    private DlgPasien getPasien(){
        if(pasien==null){
    ...
    private DlgCariDokter getDokter(){
        if(dokter==null){
            dokter=new DlgCariDokter(null,false);
            pasangListenerDokter();
        }
        return dokter;
    }

    private DlgRujukMasuk getRujukMasuk(){
        if(rujukmasuk==null){
            rujukmasuk=new DlgRujukMasuk(null,false);
            pasangListenerRujukMasuk();
        }
        return rujukmasuk;
    }

    private void pasangListenerPasien(){
        getPasien().addWindowListener(new WindowListener() {
            @Override
            ...
            public void windowClosed(WindowEvent e) {
                if(akses.getform().equals("DlgIGD")){
                    if(getPasien().getTable().getSelectedRow()!= -1){ 
                        TNoRM.setText(getPasien().getTable().getValueAt(getPasien().getTable().getSelectedRow(),1).toString());
                        isPas();
                        isNumber();                                        
                    }  
                    if(getPasien().getTable2().getSelectedRow()!= -1){ 
                        TNoRM.setText(getPasien().getTable2().getValueAt(getPasien().getTable2().getSelectedRow(),1).toString());
                        isPas();
                        isNumber();                                        
                    } 
                    if(getPasien().getTable3().getSelectedRow()!= -1){ 
                        TNoRM.setText(getPasien().getTable3().getValueAt(getPasien().getTable3().getSelectedRow(),1).toString());
                        isPas();
                        isNumber();                                        
                    }  
                    TNoRM.requestFocus();
        ...
        getPasien().getTable().addKeyListener(new KeyListener() {
            @Override
        ...
        getPasien().getTable2().addKeyListener(new KeyListener() {
            @Override
            public void keyTyped(KeyEvent e) {}
            @Override
            public void keyPressed(KeyEvent e) {
        ...
        getPasien().getTable3().addKeyListener(new KeyListener() {
            @Override
            public void keyTyped(KeyEvent e) {}
            @Override
            public void keyPressed(KeyEvent e) {
        ...
        getPasien().kab.addWindowListener(new WindowListener() {
            @Override
            public void windowOpened(WindowEvent e) {}
            @Override
            public void windowClosing(WindowEvent e) {}
            ...
            public void windowClosed(WindowEvent e) {
                if(akses.getform().equals("DlgIGD")){
                    if(getPasien().kab.getTable().getSelectedRow()!= -1){                   
                        Kabupaten2.setText(getPasien().kab.getTable().getValueAt(getPasien().kab.getTable().getSelectedRow(),0).toString());
                    }     
        ...
        getPasien().kec.addWindowListener(new WindowListener() {
            @Override
            public void windowOpened(WindowEvent e) {}
            @Override
            public void windowClosing(WindowEvent e) {}
            ...
            public void windowClosed(WindowEvent e) {
                if(akses.getform().equals("DlgIGD")){
                    if(getPasien().kec.getTable().getSelectedRow()!= -1){                   
                        Kecamatan2.setText(getPasien().kec.getTable().getValueAt(getPasien().kec.getTable().getSelectedRow(),0).toString());
                    }                
        ...
        getPasien().kel.addWindowListener(new WindowListener() {
            @Override
            public void windowOpened(WindowEvent e) {}
            @Override
            public void windowClosing(WindowEvent e) {}
            ...
            public void windowClosed(WindowEvent e) {
                if(akses.getform().equals("DlgIGD")){
                    if(getPasien().kel.getTable().getSelectedRow()!= -1){                   
                        Kelurahan2.setText(getPasien().kel.getTable().getValueAt(getPasien().kel.getTable().getSelectedRow(),0).toString());
                    }  
        ...
        getPasien().penjab.addWindowListener(new WindowListener() {
            @Override
            public void windowOpened(WindowEvent e) {}
            @Override
            public void windowClosing(WindowEvent e) {}
            ...
            public void windowClosed(WindowEvent e) {
                if(akses.getform().equals("DlgIGD")){
                    if(getPasien().penjab.getTable().getSelectedRow()!= -1){
                        isiJenisBayarOtomatisIgd(
                                getPasien().penjab.getTable().getValueAt(getPasien().penjab.getTable().getSelectedRow(),1).toString(),
                                getPasien().penjab.getTable().getValueAt(getPasien().penjab.getTable().getSelectedRow(),2).toString());
                    }    
                    kdpnj.requestFocus();
                }
            }
        ...
        getPasien().penjab.getTable().addKeyListener(new KeyListener() {
            @Override
            public void keyTyped(KeyEvent e) {}
            @Override
            public void keyPressed(KeyEvent e) {
    ...
    private void pasangListenerDokter(){
        getDokter().addWindowListener(new WindowListener() {
            @Override
            public void windowOpened(WindowEvent e) {;}
            @Override
            public void windowClosing(WindowEvent e) {}
            ...
            public void windowClosed(WindowEvent e) {
                if(akses.getform().equals("DlgIGD")){
                    if(getDokter().getTable().getSelectedRow()!= -1){    
                            isiDokterOtomatisIgd(
                                    getDokter().getTable().getValueAt(getDokter().getTable().getSelectedRow(),0).toString(),
                                    getDokter().getTable().getValueAt(getDokter().getTable().getSelectedRow(),1).toString());
                            isNumber();
    ...
    private void pasangListenerRujukMasuk(){
            ...
            public void windowClosed(WindowEvent e) {
                if(akses.getform().equals("DlgIGD")){
                    if(rujukmasuk.tbPerujuk.getSelectedRow()!= -1){
                        isiPerujukOtomatisIgd(
                                rujukmasuk.tbPerujuk.getValueAt(rujukmasuk.tbPerujuk.getSelectedRow(),0).toString(),
                                rujukmasuk.tbPerujuk.getValueAt(rujukmasuk.tbPerujuk.getSelectedRow(),1).toString());
                    }    
    ...
    private void initComponents() {
        ...
        MnCetakSuratKeteranganSakit = new javax.swing.JMenuItem();
        MnCetakSuratKeteranganBerbadanSehat = new javax.swing.JMenuItem();
        ...
        MnSisruteSatuSehat = new javax.swing.JMenuItem();
        ...
        BtnPantauFingerBPJS = new widget.Button();
        PanelInput = new javax.swing.JPanel();
        FormInput = new widget.PanelBiasa();
        LblFotoKTP = new widget.Label();
        BtnZoomFotoKTP = new widget.Button();
        BtnUnduhFotoKTP = new widget.Button();
        BtnAmbilFotoKTP = new widget.Button();
        BtnUnggahFotoKTP = new widget.Button();
        BtnRefreshFotoKTP = new widget.Button();
        ...
        MnRujukSatuSehat = new widget.Button();
        ...
        MnCetakSuratKeteranganSakit.setBackground(new java.awt.Color(255, 255, 254));
        MnCetakSuratKeteranganSakit.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        MnCetakSuratKeteranganSakit.setForeground(new java.awt.Color(50, 50, 50));
        MnCetakSuratKeteranganSakit.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        MnCetakSuratKeteranganSakit.setText("Surat Keterangan Sakit");
        MnCetakSuratKeteranganSakit.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        MnCetakSuratKeteranganSakit.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        MnCetakSuratKeteranganSakit.setName("MnCetakSuratKeteranganSakit"); // NOI18N
        MnCetakSuratKeteranganSakit.setPreferredSize(new java.awt.Dimension(250, 26));
        MnCetakSuratKeteranganSakit.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MnCetakSuratKeteranganSakitActionPerformed(evt);
            }
        });
        MnSuratSurat.add(MnCetakSuratKeteranganSakit);

        MnCetakSuratKeteranganBerbadanSehat.setBackground(new java.awt.Color(255, 255, 254));
        MnCetakSuratKeteranganBerbadanSehat.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        MnCetakSuratKeteranganBerbadanSehat.setForeground(new java.awt.Color(50, 50, 50));
        MnCetakSuratKeteranganBerbadanSehat.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        MnCetakSuratKeteranganBerbadanSehat.setText("Surat Ket Berbadan Sehat");
        MnCetakSuratKeteranganBerbadanSehat.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        MnCetakSuratKeteranganBerbadanSehat.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        MnCetakSuratKeteranganBerbadanSehat.setName("MnCetakSuratKeteranganBerbadanSehat"); // NOI18N
        MnCetakSuratKeteranganBerbadanSehat.setPreferredSize(new java.awt.Dimension(250, 26));
        MnCetakSuratKeteranganBerbadanSehat.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MnCetakSuratKeteranganBerbadanSehatActionPerformed(evt);
            }
        });
        MnSuratSurat.add(MnCetakSuratKeteranganBerbadanSehat);
        ...
        MnSisruteSatuSehat.setBackground(new java.awt.Color(255, 255, 254));
        MnSisruteSatuSehat.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        MnSisruteSatuSehat.setForeground(new java.awt.Color(50, 50, 50));
        MnSisruteSatuSehat.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        MnSisruteSatuSehat.setText("Rujukan IGD Satu Sehat");
        MnSisruteSatuSehat.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        MnSisruteSatuSehat.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        MnSisruteSatuSehat.setName("MnSisruteSatuSehat"); // NOI18N
        MnSisruteSatuSehat.setPreferredSize(new java.awt.Dimension(160, 26));
        MnSisruteSatuSehat.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MnSisruteSatuSehatActionPerformed(evt);
            }
        });
        MnBridging.add(MnSisruteSatuSehat);
        ...
        DTPCari1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-05-2026" }));
        ...
        DTPCari2.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-05-2026" }));
        ...
        BtnPantauFingerBPJS.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/face-recognition.png"))); // NOI18N
        BtnPantauFingerBPJS.setText("Sudah Bio / Finger");
        BtnPantauFingerBPJS.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        BtnPantauFingerBPJS.setName("BtnPantauFingerBPJS"); // NOI18N
        BtnPantauFingerBPJS.setPreferredSize(new java.awt.Dimension(140, 30));
        BtnPantauFingerBPJS.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnPantauFingerBPJSActionPerformed(evt);
            }
        });
        panelGlass7.add(BtnPantauFingerBPJS);
        ...
        FormInput.setMinimumSize(new java.awt.Dimension(1420, 167));
        FormInput.setPreferredSize(new java.awt.Dimension(1420, 167));
        ...
        jLabel4.setBounds(160, 10, 60, 23);

        TDokter.setEditable(true);
        TDokter.setName("TDokter"); // NOI18N
        FormInput.add(TDokter);
        TDokter.setBounds(183, 70, 200, 23);
        ...
        FormInput.add(TNoRw);
        TNoRw.setBounds(220, 10, 120, 23);

        jLabel8.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        jLabel8.setText("Tgl. Registrasi");
        jLabel8.setName("jLabel8"); // NOI18N
        FormInput.add(jLabel8);
        jLabel8.setBounds(10, 40, 80, 23);

        jLabel13.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        jLabel13.setText("Dokter IGD");
        ...
        jLabel9.setBounds(190, 40, 36, 23);

        DTPReg.setForeground(new java.awt.Color(50, 70, 50));
        DTPReg.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-05-2026" }));
        ...
        jLabel21.setText("Alamat Png Jawab");
        ...
        TPngJwb.setBounds(620, 40, 340, 23);
        ...
        CmbMenit.setBounds(287, 40, 50, 23);
        ...
        CmbDetik.setBounds(342, 40, 50, 23);

        jLabel7.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        jLabel7.setText("Nomor Rekam Medik");
        jLabel7.setName("jLabel7"); // NOI18N
        FormInput.add(jLabel7);
        jLabel7.setBounds(510, 10, 100, 23);
        ...
        BtnPasien.setBounds(960, 10, 120, 23);

        TPasien.setEditable(false);
        TPasien.setHighlighter(null);
        TPasien.setName("TPasien"); // NOI18N
        FormInput.add(TPasien);
        TPasien.setBounds(683, 10, 277, 23);

        jLabel22.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        jLabel22.setText("Hubungan");
        jLabel22.setName("jLabel22"); // NOI18N
        FormInput.add(jLabel22);
        jLabel22.setBounds(970, 40, 60, 23);
        ...
        THbngn.setBounds(1030, 40, 80, 23);

        ChkJln.setBackground(new java.awt.Color(255, 255, 255));
        ChkJln.setBorder(null);
        ChkJln.setSelected(true);
        ChkJln.setText("Jam Otomatis");
        ChkJln.setFont(new java.awt.Font("Tahoma", 0, 10)); // NOI18N
        ChkJln.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        ...
        ChkJln.setBounds(400, 40, 130, 23);
        ...
        KdDokter.setBounds(100, 70, 80, 23);
        ...
        FormInput.add(kdpnj);
        kdpnj.setBounds(620, 100, 70, 23);

        nmpnj.setEditable(true);
        nmpnj.setName("nmpnj"); // NOI18N
        FormInput.add(nmpnj);
        nmpnj.setBounds(693, 100, 265, 23);

        btnPenjab.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/190.png"))); // NOI18N
        btnPenjab.setMnemonic('2');
        btnPenjab.setText("Pilih Jenis Bayar");
        btnPenjab.setToolTipText("ALt+2");
        btnPenjab.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        btnPenjab.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        ...
        btnPenjab.setBounds(960, 100, 140, 23);
        ...
        AsalRujukan.setBounds(620, 130, 340, 23);

        jLabel23.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        jLabel23.setText("Asal Rujukan");
        jLabel23.setName("jLabel23"); // NOI18N
        FormInput.add(jLabel23);
        jLabel23.setBounds(510, 130, 90, 23);

        btnPenjab1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/190.png"))); // NOI18N
        btnPenjab1.setMnemonic('2');
        btnPenjab1.setText("Pilih Asal Rujukan");
        btnPenjab1.setToolTipText("ALt+2");
        btnPenjab1.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        btnPenjab1.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        ...
        btnPenjab1.setBounds(960, 130, 150, 23);

        ChkTracker.setBackground(new java.awt.Color(255, 255, 255));
        ...
        ChkTracker.setBounds(70, 10, 23, 23);

        MnRujukSatuSehat.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/190.png"))); // NOI18N
        MnRujukSatuSehat.setMnemonic('2');
        MnRujukSatuSehat.setText("Sistem Rujukan");
        MnRujukSatuSehat.setToolTipText("ALt+2");
        MnRujukSatuSehat.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        MnRujukSatuSehat.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        MnRujukSatuSehat.setName("MnRujukSatuSehat"); // NOI18N
        MnRujukSatuSehat.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MnRujukSatuSehatActionPerformed(evt);
            }
        });
        FormInput.add(MnRujukSatuSehat);
        MnRujukSatuSehat.setBounds(10, 130, 140, 23);

        LblFotoKTP.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        LblFotoKTP.setName("LblFotoKTP"); // NOI18N
        LblFotoKTP.setOpaque(true);
        FormInput.add(LblFotoKTP);
        LblFotoKTP.setBounds(1120, 5, 250, 158);

        BtnZoomFotoKTP.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/zoom-in.png"))); // NOI18N
        BtnZoomFotoKTP.setToolTipText("Perbesar Foto KTP Pasien");
        BtnZoomFotoKTP.setEnabled(false);
        BtnZoomFotoKTP.setName("BtnZoomFotoKTP"); // NOI18N
        BtnZoomFotoKTP.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnZoomFotoKTPActionPerformed(evt);
            }
        });
        FormInput.add(BtnZoomFotoKTP);
        BtnZoomFotoKTP.setBounds(1375, 101, 33, 30);

        BtnUnduhFotoKTP.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/printer_hitam.png"))); // NOI18N
        BtnUnduhFotoKTP.setToolTipText("Cetak Foto KTP pada kertas F4/Folio");
        BtnUnduhFotoKTP.setEnabled(false);
        BtnUnduhFotoKTP.setName("BtnUnduhFotoKTP"); // NOI18N
        BtnUnduhFotoKTP.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnUnduhFotoKTPActionPerformed(evt);
            }
        });
        FormInput.add(BtnUnduhFotoKTP);
        BtnUnduhFotoKTP.setBounds(1375, 133, 33, 30);

        BtnAmbilFotoKTP.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/dslr-camera.png"))); // NOI18N
        BtnAmbilFotoKTP.setToolTipText("Ambil Foto KTP Pasien menggunakan kamera");
        BtnAmbilFotoKTP.setName("BtnAmbilFotoKTP"); // NOI18N
        BtnAmbilFotoKTP.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnAmbilFotoKTPActionPerformed(evt);
            }
        });
        FormInput.add(BtnAmbilFotoKTP);
        BtnAmbilFotoKTP.setBounds(1375, 5, 33, 30);

        BtnUnggahFotoKTP.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/upload-big-arrow.png"))); // NOI18N
        BtnUnggahFotoKTP.setToolTipText("Unggah hasil scan Foto KTP dari komputer (JPG, JPEG, PNG)");
        BtnUnggahFotoKTP.setName("BtnUnggahFotoKTP"); // NOI18N
        BtnUnggahFotoKTP.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnUnggahFotoKTPActionPerformed(evt);
            }
        });
        FormInput.add(BtnUnggahFotoKTP);
        BtnUnggahFotoKTP.setBounds(1375, 37, 33, 30);

        BtnRefreshFotoKTP.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/camera_1.png"))); // NOI18N
        BtnRefreshFotoKTP.setToolTipText("Muat ulang Foto KTP Pasien");
        BtnRefreshFotoKTP.setName("BtnRefreshFotoKTP"); // NOI18N
        BtnRefreshFotoKTP.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnRefreshFotoKTPActionPerformed(evt);
            }
        });
        FormInput.add(BtnRefreshFotoKTP);
        BtnRefreshFotoKTP.setBounds(1375, 69, 33, 30);
    ...
    private void BtnPantauFingerBPJSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPantauFingerBPJSActionPerformed
        pantauFingerBPJSIGD();
    }//GEN-LAST:event_BtnPantauFingerBPJSActionPerformed

    private void BtnAmbilFotoKTPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnAmbilFotoKTPActionPerformed
        ambilFotoKTP();
    }//GEN-LAST:event_BtnAmbilFotoKTPActionPerformed

    private void BtnUnggahFotoKTPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnUnggahFotoKTPActionPerformed
        unggahFotoKTP();
    }//GEN-LAST:event_BtnUnggahFotoKTPActionPerformed

    private void BtnRefreshFotoKTPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnRefreshFotoKTPActionPerformed
        panggilFotoKTP(TNoRM.getText(), true);
    }//GEN-LAST:event_BtnRefreshFotoKTPActionPerformed

    private void BtnZoomFotoKTPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnZoomFotoKTPActionPerformed
        tampilkanFotoKTPBesar();
    }//GEN-LAST:event_BtnZoomFotoKTPActionPerformed

    private void BtnUnduhFotoKTPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnUnduhFotoKTPActionPerformed
        cetakFotoKTP();
    }//GEN-LAST:event_BtnUnduhFotoKTPActionPerformed
    ...
    /*
     * Validasi nama pasien sebelum registrasi/IGD disimpan.
     * Dialog hanya tampil bila benar-benar ditemukan sapaan/penanda nama
     * atau gelar akademis. Normalisasi spasi tidak lagi dijadikan dasar
     * deteksi agar nama biasa tidak memunculkan dialog validasi.
     */
    private boolean memilikiSapaanNamaPasien(String namaPasien){
        String nama = namaPasien == null ? "" : namaPasien.trim();
        return POLA_SAPAAN_AWAL_NAMA_PASIEN.matcher(nama).find()
                || POLA_SAPAAN_AKHIR_NAMA_PASIEN.matcher(nama).find();
    }

    private String hapusSapaanNamaPasien(String namaPasien){
        String hasil = namaPasien == null ? "" : namaPasien.trim();
        String sebelum;

        do{
            sebelum = hasil;
            hasil = POLA_SAPAAN_AWAL_NAMA_PASIEN.matcher(hasil).replaceFirst("");
            hasil = POLA_SAPAAN_AKHIR_NAMA_PASIEN.matcher(hasil).replaceFirst("");
            hasil = hasil.replaceAll("\\s{2,}", " ").trim();
        }while(!sebelum.equals(hasil));

        return hasil.replaceAll("\\s+,", ",").replaceAll(",\\s*$", "").trim();
    }

    private boolean validasiNamaPasienSebelumSimpan(){
        /*
         * Setelah Edit Nama Manual berhasil, proses simpan dipanggil ulang
         * sekali melalui event queue. Jangan tampilkan dialog validasi untuk
         * pemanggilan ulang itu karena petugas baru saja mengonfirmasi edit.
         */
        if(lewatiValidasiNamaPasienSekali){
            lewatiValidasiNamaPasienSekali = false;
            return true;
        }

        String noRM = TNoRM.getText().trim();
        String namaLama = Sequel.cariIsi(
        ...
        namaLama = namaLama.trim();

        if(namaLama.equals("")){
            JOptionPane.showMessageDialog(
                    rootPane,
                    "Nama master pasien tidak ditemukan. Silakan pilih pasien kembali sebelum menyimpan.",
        ...
        /*
         * Penting: deteksi sapaan menggunakan kecocokan pola, bukan dengan
         * membandingkan hasil normalisasi nama. Dengan demikian nama biasa
         * yang hanya memiliki spasi ganda/tanda baca tidak akan memunculkan
         * dialog validasi.
         */
        boolean adaSapaan = memilikiSapaanNamaPasien(namaLama);
        boolean adaGelarAkademis = POLA_GELAR_AKADEMIS_NAMA_PASIEN.matcher(namaLama).find();

        if(!adaSapaan && !adaGelarAkademis){
            return true;
        }

        String namaTanpaSapaan = adaSapaan ? hapusSapaanNamaPasien(namaLama) : namaLama;
        int aksi = tampilDialogValidasiNamaPasien(
                namaLama, namaTanpaSapaan, adaSapaan, adaGelarAkademis
        );

        if(aksi == AKSI_VALIDASI_NAMA_HAPUS_SAPAAN){
            if(namaTanpaSapaan.equals("")){
                JOptionPane.showMessageDialog(
                        rootPane,
                        "Nama hasil pembersihan kosong. Silakan periksa nama pasien terlebih dahulu.",
                        "Validasi Nama Pasien",
                        JOptionPane.WARNING_MESSAGE
                );
                return false;
            }
            return simpanNamaPasienDariValidasi(namaTanpaSapaan);
        }else if(aksi == AKSI_VALIDASI_NAMA_LANJUT){
            return true;
        }else if(aksi == AKSI_VALIDASI_NAMA_EDIT_MANUAL){
            /*
             * Dialog edit bersifat modal. Setelah nama berhasil disimpan,
             * jadwalkan ulang aksi Simpan agar seluruh proses registrasi/IGD
             * yang semula sedang berjalan diteruskan otomatis. Pemanggilan
             * awal dihentikan di sini untuk mencegah proses simpan ganda.
             */
            if(tampilDialogEditNamaPasien(namaLama)){
                jadwalkanSimpanSetelahEditNamaManual();
            }
            return false;
        }
    ...
    private void jadwalkanSimpanSetelahEditNamaManual(){
        if(simpanLanjutanSetelahEditNamaPasienDijadwalkan){
            return;
        }

        simpanLanjutanSetelahEditNamaPasienDijadwalkan = true;
        javax.swing.SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                try{
                    /*
                     * Panggil kembali alur simpan yang sama, sehingga semua
                     * validasi registrasi/IGD lain tetap berjalan normal.
                     * Yang dilewati hanya popup nama ini, sekali saja.
                     */
                    lewatiValidasiNamaPasienSekali = true;
                    BtnSimpanActionPerformed(null);
                }finally{
                    lewatiValidasiNamaPasienSekali = false;
                    simpanLanjutanSetelahEditNamaPasienDijadwalkan = false;
                }
            }
        });
    }

    private int tampilDialogValidasiNamaPasien(
            String namaLama, String namaTanpaSapaan,
            boolean adaSapaan, boolean adaGelarAkademis
    ){
        final int[] aksi = {AKSI_VALIDASI_NAMA_BATAL};
        final javax.swing.JDialog dialog = new javax.swing.JDialog(
                this, "Verifikasi Nama Pasien", true
        );
        dialog.setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
        dialog.setResizable(false);

        javax.swing.JPanel utama = new javax.swing.JPanel(new java.awt.BorderLayout());
        utama.setBackground(java.awt.Color.WHITE);
        utama.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(211, 222, 234)));

        javax.swing.JPanel header = new javax.swing.JPanel(new java.awt.BorderLayout(14, 0));
        header.setBackground(new java.awt.Color(18, 92, 162));
        header.setBorder(javax.swing.BorderFactory.createEmptyBorder(16, 20, 16, 20));
        javax.swing.JLabel ikon = new javax.swing.JLabel(javax.swing.UIManager.getIcon("OptionPane.warningIcon"));
        ikon.setVerticalAlignment(javax.swing.SwingConstants.TOP);
        header.add(ikon, java.awt.BorderLayout.WEST);

        javax.swing.JPanel judulHeader = new javax.swing.JPanel();
        judulHeader.setOpaque(false);
        judulHeader.setLayout(new javax.swing.BoxLayout(judulHeader, javax.swing.BoxLayout.Y_AXIS));
        javax.swing.JLabel judul = new javax.swing.JLabel("Verifikasi Nama Pasien");
        judul.setForeground(java.awt.Color.WHITE);
        judul.setFont(new java.awt.Font("Segoe UI", java.awt.Font.BOLD, 18));
        javax.swing.JLabel subJudul = new javax.swing.JLabel("Pastikan identitas pasien sesuai KTP / Dukcapil sebelum pendaftaran disimpan");
        subJudul.setForeground(new java.awt.Color(225, 239, 252));
        subJudul.setFont(new java.awt.Font("Segoe UI", java.awt.Font.PLAIN, 12));
        judulHeader.add(judul);
        judulHeader.add(javax.swing.Box.createVerticalStrut(4));
        judulHeader.add(subJudul);
        header.add(judulHeader, java.awt.BorderLayout.CENTER);
        utama.add(header, java.awt.BorderLayout.NORTH);

        javax.swing.JPanel isi = new javax.swing.JPanel();
        isi.setOpaque(true);
        isi.setBackground(java.awt.Color.WHITE);
        isi.setLayout(new javax.swing.BoxLayout(isi, javax.swing.BoxLayout.Y_AXIS));
        isi.setBorder(javax.swing.BorderFactory.createEmptyBorder(18, 20, 12, 20));

        String keterangan;
        if(adaSapaan && adaGelarAkademis){
            keterangan = "Sistem menemukan sapaan/penanda nama dan gelar tambahan pada data pasien.";
        }else if(adaSapaan){
            keterangan = "Sistem menemukan sapaan/penanda nama pada data pasien.";
        }else{
            keterangan = "Sistem menemukan gelar tambahan pada data pasien.";
        }
        javax.swing.JTextArea informasi = buatAreaInformasiNama(keterangan, 13, new java.awt.Color(61, 73, 87));
        isi.add(informasi);
        isi.add(javax.swing.Box.createVerticalStrut(12));
        isi.add(buatKartuInformasiNama("NAMA SAAT INI", namaLama, new java.awt.Color(92, 105, 121)));

        if(adaSapaan){
            isi.add(javax.swing.Box.createVerticalStrut(10));
            isi.add(buatKartuInformasiNama("REKOMENDASI NAMA", namaTanpaSapaan, new java.awt.Color(23, 128, 181)));
        }

        isi.add(javax.swing.Box.createVerticalStrut(12));
        String catatan = adaSapaan
                ? "Penghapusan otomatis hanya berlaku untuk Tn., Ny., Nn., atau An. yang berdiri sendiri di awal/akhir nama."
                : "Gelar akademis tidak dihapus otomatis. Gunakan Edit Nama Manual bila perlu disesuaikan dengan KTP/Dukcapil.";
        if(adaGelarAkademis && adaSapaan){
            catatan += " Gelar akademis tetap dipertahankan dan dapat diperbaiki melalui Edit Nama Manual.";
        }
        javax.swing.JPanel catatanPanel = new javax.swing.JPanel(new java.awt.BorderLayout(9, 0));
        catatanPanel.setMaximumSize(new java.awt.Dimension(720, 58));
        catatanPanel.setBackground(new java.awt.Color(242, 247, 252));
        catatanPanel.setBorder(javax.swing.BorderFactory.createCompoundBorder(
                javax.swing.BorderFactory.createLineBorder(new java.awt.Color(205, 223, 239)),
                javax.swing.BorderFactory.createEmptyBorder(9, 11, 9, 11)
        ));
        javax.swing.JLabel ikonInfo = new javax.swing.JLabel(javax.swing.UIManager.getIcon("OptionPane.informationIcon"));
        catatanPanel.add(ikonInfo, java.awt.BorderLayout.WEST);
        catatanPanel.add(buatAreaInformasiNama(catatan, 12, new java.awt.Color(57, 78, 98)), java.awt.BorderLayout.CENTER);
        isi.add(catatanPanel);
        utama.add(isi, java.awt.BorderLayout.CENTER);

        javax.swing.JPanel footer = new javax.swing.JPanel(new java.awt.FlowLayout(java.awt.FlowLayout.RIGHT, 8, 11));
        footer.setBackground(new java.awt.Color(248, 250, 252));
        footer.setBorder(javax.swing.BorderFactory.createMatteBorder(1, 0, 0, 0, new java.awt.Color(222, 229, 236)));

        /*
         * Tombol menggunakan warna solid agar tetap terbaca pada semua
         * LookAndFeel Java. Saat mouse diarahkan ke tombol, warna dibuat
         * sedikit lebih gelap sebagai penanda interaksi.
         */
        final javax.swing.JButton btnBatal = buatTombolValidasiNama(
                "Batal",
        ...
        btnBatal.addActionListener(new java.awt.event.ActionListener() {
            ...
            public void actionPerformed(java.awt.event.ActionEvent e) {
                aksi[0] = AKSI_VALIDASI_NAMA_BATAL;
                dialog.dispose();
            }
        });

        final javax.swing.JButton btnEdit = buatTombolValidasiNama(
                "Edit Nama Manual",
                new java.awt.Color(249, 115, 22),
                new java.awt.Color(194, 65, 12)
        );
        btnEdit.addActionListener(new java.awt.event.ActionListener() {
            @Override
            public void actionPerformed(java.awt.event.ActionEvent e) {
                aksi[0] = AKSI_VALIDASI_NAMA_EDIT_MANUAL;
                dialog.dispose();
            }
        });

        final javax.swing.JButton btnLanjut = buatTombolValidasiNama(
                "Lanjut Simpan, data sesuai KTP",
        ...
        btnLanjut.addActionListener(new java.awt.event.ActionListener() {
            @Override
            public void actionPerformed(java.awt.event.ActionEvent e) {
                aksi[0] = AKSI_VALIDASI_NAMA_LANJUT;
                dialog.dispose();
            }
        });

        javax.swing.JButton btnHapusSapaan = null;
        if(adaSapaan){
            btnHapusSapaan = buatTombolValidasiNama(
                    "Hilangkan Sapaan & Simpan",
                    new java.awt.Color(22, 163, 74),
                    new java.awt.Color(21, 128, 61)
            );
            btnHapusSapaan.addActionListener(new java.awt.event.ActionListener() {
                @Override
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    aksi[0] = AKSI_VALIDASI_NAMA_HAPUS_SAPAAN;
                    dialog.dispose();
                }
            });
            footer.add(btnHapusSapaan);
        }
        footer.add(btnLanjut);
        footer.add(btnEdit);
        footer.add(btnBatal);
        utama.add(footer, java.awt.BorderLayout.SOUTH);

        /*
         * Saat sapaan ditemukan, tombol hapus menjadi tombol utama: fokus
         * awal langsung berada di tombol itu dan Enter menjalankan aksi yang
         * sama. Untuk kasus gelar akademis tanpa sapaan, tombol utama adalah
         * Lanjut Simpan karena tidak ada penghapusan otomatis.
         */
        final javax.swing.JButton tombolFokusUtama =
                (btnHapusSapaan != null) ? btnHapusSapaan : btnLanjut;

        dialog.setContentPane(utama);
        dialog.getRootPane().setDefaultButton(tombolFokusUtama);
        dialog.addWindowListener(new java.awt.event.WindowAdapter() {
            @Override
            public void windowOpened(java.awt.event.WindowEvent e) {
                tombolFokusUtama.requestFocusInWindow();
            }
        });
        ...
        dialog.pack();
        dialog.setSize(760, dialog.getHeight());
        dialog.setLocationRelativeTo(this);
        dialog.setVisible(true);
        return aksi[0];
    }

    private javax.swing.JPanel buatKartuInformasiNama(String judul, String isi, java.awt.Color warnaAksen){
        javax.swing.JPanel kartu = new javax.swing.JPanel(new java.awt.BorderLayout(0, 5));
        kartu.setMaximumSize(new java.awt.Dimension(720, 82));
        kartu.setBackground(java.awt.Color.WHITE);
        kartu.setBorder(javax.swing.BorderFactory.createCompoundBorder(
                javax.swing.BorderFactory.createMatteBorder(0, 4, 0, 0, warnaAksen),
                javax.swing.BorderFactory.createCompoundBorder(
                        javax.swing.BorderFactory.createLineBorder(new java.awt.Color(222, 229, 236)),
                        javax.swing.BorderFactory.createEmptyBorder(10, 12, 10, 12)
                )
        ));
        javax.swing.JLabel labelJudul = new javax.swing.JLabel(judul);
        labelJudul.setForeground(warnaAksen);
        labelJudul.setFont(new java.awt.Font("Segoe UI", java.awt.Font.BOLD, 11));
        kartu.add(labelJudul, java.awt.BorderLayout.NORTH);
        kartu.add(buatAreaInformasiNama(isi, 15, new java.awt.Color(35, 48, 62)), java.awt.BorderLayout.CENTER);
        return kartu;
    }

    private javax.swing.JTextArea buatAreaInformasiNama(String teks, int ukuran, java.awt.Color warna){
        javax.swing.JTextArea area = new javax.swing.JTextArea(teks == null ? "" : teks);
        area.setEditable(false);
        area.setFocusable(false);
        area.setOpaque(false);
        area.setLineWrap(true);
        area.setWrapStyleWord(true);
        area.setForeground(warna);
        area.setFont(new java.awt.Font("Segoe UI", java.awt.Font.PLAIN, ukuran));
        area.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        return area;
    }

    private javax.swing.JButton buatTombolValidasiNama(
            String teks, final java.awt.Color latar, final java.awt.Color latarHover
    ){
        /*
         * Background dilukis sendiri agar warna tombol tidak ditimpa oleh
         * LookAndFeel bawaan Windows/Nimbus. Dengan cara ini teks putih dan
         * warna tombol selalu terbaca jelas.
         */
        final javax.swing.JButton tombol = new javax.swing.JButton(teks) {
            @Override
            ...
            protected void paintBorder(java.awt.Graphics g) {
                // Border bawaan dinonaktifkan karena tombol memakai sudut membulat.
            }
        };
        tombol.setFont(new java.awt.Font("Segoe UI", java.awt.Font.BOLD, 12));
        tombol.setForeground(java.awt.Color.WHITE);
        tombol.setBackground(latar);
        tombol.setOpaque(false);
        tombol.setContentAreaFilled(false);
        tombol.setBorderPainted(false);
        tombol.setFocusPainted(false);
        tombol.setRolloverEnabled(false);
        ...
        tombol.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseEntered(java.awt.event.MouseEvent e) {
                if(tombol.isEnabled()){
                    tombol.setBackground(latarHover);
                    tombol.repaint();
                }
            }
    ...
    private boolean tampilDialogEditNamaPasien(final String namaLama){
        final boolean[] berhasil = {false};
        final javax.swing.JDialog dialog = new javax.swing.JDialog(
                this, "Edit Nama Pasien", true
        );
        dialog.setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
        dialog.setResizable(false);
        ...
        header.setBackground(new java.awt.Color(18, 92, 162));
        header.setBorder(javax.swing.BorderFactory.createEmptyBorder(15, 18, 15, 18));
        header.add(new javax.swing.JLabel(javax.swing.UIManager.getIcon("OptionPane.informationIcon")), java.awt.BorderLayout.WEST);
        javax.swing.JPanel judulHeader = new javax.swing.JPanel();
        judulHeader.setOpaque(false);
        judulHeader.setLayout(new javax.swing.BoxLayout(judulHeader, javax.swing.BoxLayout.Y_AXIS));
        javax.swing.JLabel judul = new javax.swing.JLabel("Edit Nama Pasien");
        judul.setForeground(java.awt.Color.WHITE);
        judul.setFont(new java.awt.Font("Segoe UI", java.awt.Font.BOLD, 17));
        javax.swing.JLabel subJudul = new javax.swing.JLabel("Perbarui nama sesuai KTP / Dukcapil, kemudian simpan perubahan");
        subJudul.setForeground(new java.awt.Color(225, 239, 252));
        subJudul.setFont(new java.awt.Font("Segoe UI", java.awt.Font.PLAIN, 12));
        judulHeader.add(judul);
        judulHeader.add(javax.swing.Box.createVerticalStrut(3));
        judulHeader.add(subJudul);
        header.add(judulHeader, java.awt.BorderLayout.CENTER);
        utama.add(header, java.awt.BorderLayout.NORTH);
        ...
        isi.setBackground(java.awt.Color.WHITE);
        isi.setLayout(new javax.swing.BoxLayout(isi, javax.swing.BoxLayout.Y_AXIS));
        isi.setBorder(javax.swing.BorderFactory.createEmptyBorder(18, 20, 16, 20));
        javax.swing.JLabel label = new javax.swing.JLabel("NAMA PASIEN");
        label.setForeground(new java.awt.Color(65, 84, 102));
        label.setFont(new java.awt.Font("Segoe UI", java.awt.Font.BOLD, 11));
        isi.add(label);
        isi.add(javax.swing.Box.createVerticalStrut(6));
        final javax.swing.JTextField txtNama = new javax.swing.JTextField(namaLama);
        txtNama.setFont(new java.awt.Font("Segoe UI", java.awt.Font.PLAIN, 15));
        txtNama.setBorder(javax.swing.BorderFactory.createCompoundBorder(
                javax.swing.BorderFactory.createLineBorder(new java.awt.Color(147, 185, 222)),
                javax.swing.BorderFactory.createEmptyBorder(9, 10, 9, 10)
        ));
        txtNama.setMaximumSize(new java.awt.Dimension(560, 42));
        isi.add(txtNama);
        isi.add(javax.swing.Box.createVerticalStrut(10));
        javax.swing.JTextArea petunjuk = buatAreaInformasiNama(
                "Perubahan ini langsung memperbarui kolom pasien.nm_pasien untuk nomor RM yang sedang dipilih.",
                12, new java.awt.Color(84, 98, 113)
        );
        isi.add(petunjuk);
        utama.add(isi, java.awt.BorderLayout.CENTER);
        ...
        footer.setBackground(new java.awt.Color(248, 250, 252));
        footer.setBorder(javax.swing.BorderFactory.createMatteBorder(1, 0, 0, 0, new java.awt.Color(222, 229, 236)));
        final javax.swing.JButton btnBatal = buatTombolValidasiNama("Batal", new java.awt.Color(220, 38, 38), new java.awt.Color(185, 28, 28));
        btnBatal.addActionListener(new java.awt.event.ActionListener() {
            @Override
            public void actionPerformed(java.awt.event.ActionEvent e) {
                dialog.dispose();
        ...
        final javax.swing.JButton btnSimpan = buatTombolValidasiNama("Simpan Perubahan", new java.awt.Color(20, 126, 94), new java.awt.Color(16, 102, 76));
        btnSimpan.addActionListener(new java.awt.event.ActionListener() {
            @Override
            public void actionPerformed(java.awt.event.ActionEvent e) {
                String namaBaru = txtNama.getText() == null ? "" : txtNama.getText().trim();
                if(namaBaru.equals("")){
                    JOptionPane.showMessageDialog(
                            dialog,
                ...
                if(namaBaru.equals(namaLama) || simpanNamaPasienDariValidasi(namaBaru)){
                    berhasil[0] = true;
                    dialog.dispose();
                }
            }
        ...
        dialog.setContentPane(utama);
        dialog.getRootPane().setDefaultButton(btnSimpan);
        dialog.getRootPane().registerKeyboardAction(
                new java.awt.event.ActionListener() {
                    @Override
                    public void actionPerformed(java.awt.event.ActionEvent e) {
    ...
    private boolean simpanNamaPasienDariValidasi(String namaBaru){
        PreparedStatement psNamaPasien = null;

        try{
            psNamaPasien = koneksi.prepareStatement(
                    "update pasien set nm_pasien=? where no_rkm_medis=?"
            );
            psNamaPasien.setString(1, namaBaru);
            psNamaPasien.setString(2, TNoRM.getText().trim());

            if(psNamaPasien.executeUpdate() > 0){
                // Memuat ulang data form agar nama yang tampil sama dengan master pasien.
                isCekPasien();
                return true;
            }
            ...
            JOptionPane.showMessageDialog(
                    rootPane,
                    "Nama pasien tidak dapat diperbarui. Silakan pilih pasien kembali dan ulangi proses simpan.",
                    "Validasi Nama Pasien",
                    JOptionPane.ERROR_MESSAGE
            );
        }catch(Exception e){
            System.out.println("Notif Validasi Nama Pasien : "+e);
            JOptionPane.showMessageDialog(
                    rootPane,
                    "Terjadi kesalahan saat memperbarui nama pasien.\n"
                    +"Pendaftaran dibatalkan agar data tidak tersimpan sebagian.",
                    "Validasi Nama Pasien",
                    JOptionPane.ERROR_MESSAGE
            );
        }finally{
            if(psNamaPasien != null){
                try{
                    psNamaPasien.close();
                }catch(Exception e){
                    System.out.println("Notif Validasi Nama Pasien : "+e);
...
////            String nik = Sequel.cariIsi("SELECT no_ktp FROM pasien WHERE no_rkm_medis=?", noRM);
////            String noPeserta = Sequel.cariIsi("SELECT no_peserta FROM pasien WHERE no_rkm_medis=?", noRM);
////            boolean isBlokir = Sequel.cariInteger(
////                "SELECT COUNT(*) FROM blokir_pasien WHERE no_rkm_medis=? OR nik=? OR no_peserta=?", 
////                noRM, nik, noPeserta
////            ) > 0;
////
////            if (isBlokir) {
////                JOptionPane.showMessageDialog(null, "Maaf, pasien blacklist karena alasan tertentu. Silakan rujuk ke rumah sakit lain.");
...
//            }                    
    ...
    if(TNoReg.getText().trim().equals("")){
        ...
        Valid.textKosong(kdpnj,"Jenis Bayar");
    }else if(!validasiDokterAutocompleteIgd()){
        return;
    }else if(!validasiJenisBayarAutocompleteIgd()){
        return;
    }else if(!validasiNamaPasienSebelumSimpan()){
        return;
    }else if(Sequel.cariInteger("select count(pasien.no_rkm_medis) from pasien inner join reg_periksa inner join kamar_inap "+
             "on reg_periksa.no_rkm_medis=pasien.no_rkm_medis and reg_periksa.no_rawat=kamar_inap.no_rawat "+
             "where kamar_inap.stts_pulang='-' and pasien.no_rkm_medis=?",TNoRM.getText())>0){
        ...
        // =========================
        // ✅ Validasi BPJS 7 HARI
        // =========================
        try {
            final String noRM      = TNoRM.getText();
            final String tglRegStr = Valid.SetTgl(DTPReg.getSelectedItem()+""); // yyyy-MM-dd
            final java.time.LocalDate tglReg = java.time.LocalDate.parse(tglRegStr);
            final java.time.format.DateTimeFormatter dmy = java.time.format.DateTimeFormatter.ofPattern("dd/MM/yyyy");

            // ---- Cek Ranap (ambil tgl_pulang terakhir ≤ DTPReg) ----
            String sqlRanap =
                "SELECT DATE_FORMAT(MAX(k.tgl_keluar),'%Y-%m-%d') " +
                "FROM reg_periksa r INNER JOIN kamar_inap k ON k.no_rawat=r.no_rawat " +
                "WHERE r.no_rkm_medis='" + noRM + "' AND r.status_lanjut='Ranap' " +
                "AND k.stts_pulang NOT IN ('-','Pindah Kamar') " +
                "AND k.tgl_keluar IS NOT NULL AND k.tgl_keluar<>'0000-00-00 00:00:00' " +
                "AND DATE(k.tgl_keluar) < '" + tglRegStr + "'";

            String lastPulangStr = Sequel.cariIsi(sqlRanap);

            if (lastPulangStr != null && !lastPulangStr.trim().equals("")) {
                java.time.LocalDate tglPulangTerakhir = java.time.LocalDate.parse(lastPulangStr);
                long selisihHariRanap = java.time.temporal.ChronoUnit.DAYS.between(tglPulangTerakhir, tglReg);
                if (selisihHariRanap <= 7L) {
                    String pesanRanap =
                        "Maaf, pasien ini telah dirawat inap " + selisihHariRanap + " hari yang lalu, " +                        
                        " pulang tanggal " + tglPulangTerakhir.format(dmy) + ".\n" +                        
                        "Klik OK untuk lanjut menyimpan.";
                    int ok = JOptionPane.showConfirmDialog(
                        null, pesanRanap, "Konfirmasi BPJS 7 Hari",
                        JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE
                    );
                    if (ok != JOptionPane.OK_OPTION) return; // batalkan simpan
                }
            }

            // ---- Cek Ralan (ambil tgl_registrasi terakhir ≤ DTPReg) ----
            String sqlRalan =
                "SELECT DATE_FORMAT(MAX(r.tgl_registrasi),'%Y-%m-%d') " +
                "FROM reg_periksa r " +
                "WHERE r.no_rkm_medis='" + noRM + "' AND r.status_lanjut='Ralan' " +
                "AND DATE(r.tgl_registrasi) < '" + tglRegStr + "'";

            String lastRalanStr = Sequel.cariIsi(sqlRalan);

            if (lastRalanStr != null && !lastRalanStr.trim().equals("")) {
                java.time.LocalDate tglRalanTerakhir = java.time.LocalDate.parse(lastRalanStr);
                long selisihHariRalan = java.time.temporal.ChronoUnit.DAYS.between(tglRalanTerakhir, tglReg);
                if (selisihHariRalan <= 7L) {
                    String pesanRalan =
                        "Maaf, pasien ini telah diobservasi " + selisihHariRalan + " hari yang lalu " +                        
                        "pada tanggal " + tglRalanTerakhir.format(dmy) + ".\n" +                        
                        "Klik OK untuk lanjut menyimpan.";
                    int ok = JOptionPane.showConfirmDialog(
                        null, pesanRalan, "Konfirmasi BPJS 7 Hari",
                        JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE
                    );
                    if (ok != JOptionPane.OK_OPTION) return; // batalkan simpan
                }
            ...
            System.out.println("Warning Validasi 7 Hari BPJS: " + ex);
            // jika error validasi, lanjut proses (jangan menghalangi simpan)
        }
    ...
    private void BtnKeluarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnKeluarActionPerformed
        ...
        DlgCatatan.dispose();
        hentikanAutoRefreshPantauFingerIGD();
        if(timerMuatFotoKTP != null){ timerMuatFotoKTP.stop(); }
        batalkanPemuatanFotoKTP();
        ...
        if(tabMode.getRowCount()!=0){
            ...
            if(evt.getClickCount()==1){
                ...
                if(i==1){
                    ...
                    boolean isBPJ = tbPetugas.getValueAt(tbPetugas.getSelectedRow(), 17).toString().equals("BPJ");
                    boolean isAdminUtama = akses.getkode().equals("Admin Utama");                    
                    
                    if(akses.gettindakan_ralan()==true){
                        MnRawatJalanActionPerformed(null);
                        
                    if (!isAdminUtama && isBPJ) {
                        
                        // Periksa apakah kode dokter yang login sama dengan kode dokter yang tertera di kddokter
//                        if (kddokter.getText().equals(akses.getkode())) {
                    
                            MnRiwayatPerawatanICareNoKartuActionPerformed(null); // Panggil method baru hanya jika kddokter sesuai dengan kode yang login
                        }
                    }                                
                }else if(i==3){
...
//                        if (kddokter.getText().equals(akses.getkode())) {
    ...
    /**
     * Autocomplete IGD: cache hanya hidup selama DlgIGD ini terbuka.
     * Dokter, jenis bayar, dan asal rujukan tidak lagi query database per ketikan.
     */
    private void pasangAutocompleteIGD(){
        popupDokterIgd = new javax.swing.JPopupMenu();
        popupPenjabIgd = new javax.swing.JPopupMenu();
        popupPerujukIgd = new javax.swing.JPopupMenu();

        tabelDokterIgd = buatTabelAutocompleteIgd(modelDokterIgd);
        tabelPenjabIgd = buatTabelAutocompleteIgd(modelPenjabIgd);
        tabelPerujukIgd = buatTabelAutocompleteIgd(modelPerujukIgd);

        sembunyikanKolomAutocompleteIgd(tabelDokterIgd, 0);
        sembunyikanKolomAutocompleteIgd(tabelPenjabIgd, 0);
        tabelDokterIgd.getColumnModel().getColumn(1).setPreferredWidth(Math.max(1, TDokter.getWidth()));
        tabelPenjabIgd.getColumnModel().getColumn(1).setPreferredWidth(Math.max(1, nmpnj.getWidth()));
        tabelPerujukIgd.getColumnModel().getColumn(0).setPreferredWidth(210);
        tabelPerujukIgd.getColumnModel().getColumn(1).setPreferredWidth(130);

        popupDokterIgd.add(buatScrollAutocompleteIgd(tabelDokterIgd));
        popupPenjabIgd.add(buatScrollAutocompleteIgd(tabelPenjabIgd));
        popupPerujukIgd.add(buatScrollAutocompleteIgd(tabelPerujukIgd));

        timerDokterIgd = new Timer(250, new ActionListener() {
            @Override public void actionPerformed(ActionEvent evt) {
                tampilkanPilihanDokterIgd(TDokter.getText());
            }
        });
        timerDokterIgd.setRepeats(false);
        timerPenjabIgd = new Timer(250, new ActionListener() {
            @Override public void actionPerformed(ActionEvent evt) {
                tampilkanPilihanPenjabIgd(nmpnj.getText());
            }
        });
        timerPenjabIgd.setRepeats(false);
        timerPerujukIgd = new Timer(250, new ActionListener() {
            @Override public void actionPerformed(ActionEvent evt) {
                tampilkanPilihanPerujukIgd(AsalRujukan.getText());
            }
        });
        timerPerujukIgd.setRepeats(false);

        TDokter.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            @Override public void insertUpdate(DocumentEvent evt) { perubahanTeksDokterIgd(); }
            @Override public void removeUpdate(DocumentEvent evt) { perubahanTeksDokterIgd(); }
            @Override public void changedUpdate(DocumentEvent evt) { perubahanTeksDokterIgd(); }
        });
        nmpnj.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            @Override public void insertUpdate(DocumentEvent evt) { perubahanTeksPenjabIgd(); }
            @Override public void removeUpdate(DocumentEvent evt) { perubahanTeksPenjabIgd(); }
            @Override public void changedUpdate(DocumentEvent evt) { perubahanTeksPenjabIgd(); }
        });
        AsalRujukan.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            @Override public void insertUpdate(DocumentEvent evt) { perubahanTeksPerujukIgd(); }
            @Override public void removeUpdate(DocumentEvent evt) { perubahanTeksPerujukIgd(); }
            @Override public void changedUpdate(DocumentEvent evt) { perubahanTeksPerujukIgd(); }
        });

        TDokter.addKeyListener(new java.awt.event.KeyAdapter() {
            @Override public void keyPressed(KeyEvent evt) {
                navigasiAutocompleteIgd(evt, tabelDokterIgd, popupDokterIgd, 1);
            }
        });
        nmpnj.addKeyListener(new java.awt.event.KeyAdapter() {
            @Override public void keyPressed(KeyEvent evt) {
                navigasiAutocompleteIgd(evt, tabelPenjabIgd, popupPenjabIgd, 2);
            }
        });

        tabelDokterIgd.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override public void mouseClicked(java.awt.event.MouseEvent evt) {
                if (evt.getClickCount() >= 1) pilihDokterAutocompleteIgd();
            }
        });
        tabelPenjabIgd.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override public void mouseClicked(java.awt.event.MouseEvent evt) {
                if (evt.getClickCount() >= 1) pilihPenjabAutocompleteIgd();
            }
        });
        tabelPerujukIgd.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override public void mouseClicked(java.awt.event.MouseEvent evt) {
                if (evt.getClickCount() >= 1) pilihPerujukAutocompleteIgd();
            }
        });

        pasangSembunyiPopupIgd(TDokter, popupDokterIgd);
        pasangSembunyiPopupIgd(nmpnj, popupPenjabIgd);
        pasangSembunyiPopupIgd(AsalRujukan, popupPerujukIgd);

        petunjukDokterIgd = pasangPetunjukIgd(TDokter, "ketik pencarian dokter di sini");
        petunjukPenjabIgd = pasangPetunjukIgd(nmpnj, "ketik pencarian jenis bayar di sini");
        petunjukPerujukIgd = pasangPetunjukIgd(AsalRujukan, "ketik asal rujukan di sini");
    }

    private JTable buatTabelAutocompleteIgd(final DefaultTableModel model){
        JTable tabel = new JTable(model);
        tabel.setRowHeight(BARIS_AUTOCOMPLETE_IGD);
        tabel.setFont(new java.awt.Font("Tahoma", java.awt.Font.PLAIN, 11));
        tabel.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        tabel.setShowGrid(true);
        tabel.setGridColor(GARIS_AUTOCOMPLETE_IGD);
        tabel.setAutoResizeMode(JTable.AUTO_RESIZE_LAST_COLUMN);
        tabel.getTableHeader().setReorderingAllowed(false);
        tabel.getTableHeader().setResizingAllowed(false);
        tabel.getTableHeader().setPreferredSize(new Dimension(1, HEADER_AUTOCOMPLETE_IGD));
        tabel.getTableHeader().setBackground(HEADER_AUTOCOMPLETE_BG_IGD);
        tabel.getTableHeader().setForeground(HEADER_AUTOCOMPLETE_FG_IGD);
        tabel.getTableHeader().setFont(new java.awt.Font("Tahoma", java.awt.Font.BOLD, 11));
        tabel.setDefaultRenderer(Object.class, new javax.swing.table.DefaultTableCellRenderer() {
            @Override public java.awt.Component getTableCellRendererComponent(
                ...
                label.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 7, 0, 7));
                if (selected) {
                    label.setBackground(PILIH_AUTOCOMPLETE_IGD);
                    label.setForeground(TEKS_PILIH_AUTOCOMPLETE_IGD);
                } else {
                    label.setBackground((row % 2 == 0) ? LATAR_GENAP_AUTOCOMPLETE_IGD : LATAR_GANJIL_AUTOCOMPLETE_IGD);
                    label.setForeground(TEKS_NORMAL_AUTOCOMPLETE_IGD);
                }
                return label;
            }
        });
    ...
    private javax.swing.JScrollPane buatScrollAutocompleteIgd(JTable tabel){
        javax.swing.JScrollPane scroll = new javax.swing.JScrollPane(tabel);
        ...
        scroll.setColumnHeaderView(tabel.getTableHeader());
        scroll.getVerticalScrollBar().setUnitIncrement(BARIS_AUTOCOMPLETE_IGD);
        return scroll;
    }

    private void sembunyikanKolomAutocompleteIgd(JTable tabel, int kolom){
        TableColumn column = tabel.getColumnModel().getColumn(kolom);
        column.setMinWidth(0);
        column.setMaxWidth(0);
        column.setPreferredWidth(0);
    }

    private javax.swing.JLabel pasangPetunjukIgd(final JTextField field, final String teks){
        final javax.swing.JLabel label = new javax.swing.JLabel(teks);
        java.awt.Font font = field.getFont();
        if (font == null) font = new java.awt.Font("Tahoma", java.awt.Font.PLAIN, 11);
        label.setFont(font.deriveFont(java.awt.Font.ITALIC));
        label.setForeground(PETUNJUK_AUTOCOMPLETE_IGD);
        label.setOpaque(false);
        label.setFocusable(false);
        label.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        label.setVerticalAlignment(javax.swing.SwingConstants.CENTER);
        aturPosisiPetunjukIgd(field, label);
        label.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override public void mousePressed(java.awt.event.MouseEvent evt) { field.requestFocusInWindow(); }
        });
        field.addFocusListener(new java.awt.event.FocusAdapter() {
            @Override public void focusGained(java.awt.event.FocusEvent evt) { perbaruiPetunjukIgd(field, label); }
            @Override public void focusLost(java.awt.event.FocusEvent evt) { perbaruiPetunjukIgd(field, label); }
        });
        field.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            @Override public void insertUpdate(DocumentEvent evt) { perbaruiPetunjukIgd(field, label); }
            @Override public void removeUpdate(DocumentEvent evt) { perbaruiPetunjukIgd(field, label); }
            @Override public void changedUpdate(DocumentEvent evt) { perbaruiPetunjukIgd(field, label); }
        });
        field.addComponentListener(new java.awt.event.ComponentAdapter() {
            @Override public void componentResized(java.awt.event.ComponentEvent evt) { aturPosisiPetunjukIgd(field, label); }
            @Override public void componentMoved(java.awt.event.ComponentEvent evt) { aturPosisiPetunjukIgd(field, label); }
        });
        FormInput.add(label);
        FormInput.setComponentZOrder(label, 0);
        perbaruiPetunjukIgd(field, label);
        FormInput.revalidate();
        FormInput.repaint();
        return label;
    }

    private void aturPosisiPetunjukIgd(JTextField field, javax.swing.JLabel label){
        label.setBounds(field.getX() + 8, field.getY() + 1, Math.max(1, field.getWidth() - 16), Math.max(1, field.getHeight() - 2));
    }

    private void perbaruiPetunjukIgd(JTextField field, javax.swing.JLabel label){
        if (label == null) return;
        aturPosisiPetunjukIgd(field, label);
        label.setVisible(field.getText().trim().equals("") && !field.hasFocus());
        label.repaint();
    }

    /*
     * Popup autocomplete adalah lightweight popup. Pada beberapa komputer/tema Java,
     * pemanggilan popup.show() dapat memicu focusLost sesaat pada TextBox. Patch awal
     * langsung menutup popup 180 ms setelah focusLost sehingga hasil pencarian terlihat
     * seperti berkedip. Timer di bawah hanya menutup popup bila fokus benar-benar sudah
     * berpindah ke komponen lain, bukan ketika fokus masih di field atau berada di tabel
     * popup.
     */
    private void pasangSembunyiPopupIgd(final JTextField field, final javax.swing.JPopupMenu popup){
        final Timer[] timerSembunyi = new Timer[1];
        field.addFocusListener(new java.awt.event.FocusAdapter() {
            @Override public void focusGained(java.awt.event.FocusEvent evt) {
                if (timerSembunyi[0] != null) timerSembunyi[0].stop();
            }

            @Override public void focusLost(java.awt.event.FocusEvent evt) {
                if (timerSembunyi[0] != null) timerSembunyi[0].stop();
                timerSembunyi[0] = new Timer(300, new ActionListener() {
                    @Override public void actionPerformed(ActionEvent event) {
                        java.awt.Component pemilikFokus = java.awt.KeyboardFocusManager
                                .getCurrentKeyboardFocusManager().getFocusOwner();
                        boolean fokusMasihDiAutocomplete = field.isFocusOwner()
                                || (pemilikFokus != null
                                    && (pemilikFokus == field
                                        || javax.swing.SwingUtilities.isDescendingFrom(pemilikFokus, popup)));
                        if (!fokusMasihDiAutocomplete) {
                            popup.setVisible(false);
                        }
                    }
                });
                timerSembunyi[0].setRepeats(false);
                timerSembunyi[0].start();
            }
        });
    }

    private void perubahanTeksDokterIgd(){
        if (sedangIsiAutocompleteIgd) return;
        resetDokterIgdJikaTidakSesuai();
        String kata = TDokter.getText().trim();
        if (kata.length() < MIN_AUTOCOMPLETE_IGD) {
            timerDokterIgd.stop();
            modelDokterIgd.setRowCount(0);
            popupDokterIgd.setVisible(false);
            return;
        }
        timerDokterIgd.restart();
    }

    private void perubahanTeksPenjabIgd(){
        // Popup jenis bayar hanya berasal dari ketikan manual petugas.
        if (sedangIsiAutocompleteIgd || sedangIsiJenisBayarOtomatisIgd) return;
        resetPenjabIgdJikaTidakSesuai();
        String kata = nmpnj.getText().trim();
        if (kata.length() < MIN_AUTOCOMPLETE_IGD) {
            timerPenjabIgd.stop();
            modelPenjabIgd.setRowCount(0);
            popupPenjabIgd.setVisible(false);
            return;
        }
        timerPenjabIgd.restart();
    }

    private void perubahanTeksPerujukIgd(){
        if (sedangIsiAutocompleteIgd) return;
        alamatperujuk = "-";
        String kata = AsalRujukan.getText().trim();
        if (kata.length() < MIN_AUTOCOMPLETE_IGD) {
            timerPerujukIgd.stop();
            modelPerujukIgd.setRowCount(0);
            popupPerujukIgd.setVisible(false);
            return;
        }
        timerPerujukIgd.restart();
    }

    private void resetDokterIgdJikaTidakSesuai(){
        String kode = KdDokter.getText().trim();
        String nama = namaDokterIgdBerdasarkanKode(kode);
        if (kode.equals("") || nama.equals("") || !nama.equalsIgnoreCase(TDokter.getText().trim())) {
            boolean awal = sedangIsiAutocompleteIgd;
            sedangIsiAutocompleteIgd = true;
            try { KdDokter.setText(""); } finally { sedangIsiAutocompleteIgd = awal; }
        }
    }

    private void resetPenjabIgdJikaTidakSesuai(){
        String kode = kdpnj.getText().trim();
        String nama = namaPenjabIgdBerdasarkanKode(kode);
        if (kode.equals("") || nama.equals("") || !nama.equalsIgnoreCase(nmpnj.getText().trim())) {
            boolean awal = sedangIsiAutocompleteIgd;
            sedangIsiAutocompleteIgd = true;
            try { kdpnj.setText(""); } finally { sedangIsiAutocompleteIgd = awal; }
        }
    }

    /** Cache autocomplete IGD dibatasi untuk dokter dengan spesialisasi IGD (kd_sps=SP001). */
    private void muatCacheDokterIgdJikaPerlu(){
        if (cacheDokterIgdSiap) return;
        cacheDokterIgd.clear();
        try (PreparedStatement psCache = koneksi.prepareStatement(
                "select kd_dokter,nm_dokter from dokter where kd_sps='SP001' order by nm_dokter")) {
            try (ResultSet rsCache = psCache.executeQuery()) {
                while (rsCache.next()) {
                    cacheDokterIgd.add(new CacheDokterIgd(
                            rsCache.getString("kd_dokter"), rsCache.getString("nm_dokter")));
                }
            }
            cacheDokterIgdSiap = true;
        } catch (Exception e) {
            cacheDokterIgdSiap = false;
            System.out.println("Autocomplete IGD, muat cache dokter: " + e);
        }
    }

    private void muatCachePenjabIgdJikaPerlu(){
        if (cachePenjabIgdSiap) return;
        cachePenjabIgd.clear();
        try (PreparedStatement psCache = koneksi.prepareStatement(
                "select kd_pj,png_jawab from penjab order by png_jawab")) {
            try (ResultSet rsCache = psCache.executeQuery()) {
                while (rsCache.next()) {
                    cachePenjabIgd.add(new CachePenjabIgd(
                            rsCache.getString("kd_pj"), rsCache.getString("png_jawab")));
                }
            }
            cachePenjabIgdSiap = true;
        } catch (Exception e) {
            cachePenjabIgdSiap = false;
            System.out.println("Autocomplete IGD, muat cache jenis bayar: " + e);
        }
    }

    private void muatCachePerujukIgdJikaPerlu(){
        if (cachePerujukIgdSiap) return;
        cachePerujukIgd.clear();
        try (PreparedStatement psCache = koneksi.prepareStatement(
                "select rujuk_masuk.perujuk,ifnull(rujuk_masuk.alamat,'') as alamat " +
                "from rujuk_masuk where ifnull(trim(rujuk_masuk.perujuk),'')<>'' " +
                "group by rujuk_masuk.perujuk order by rujuk_masuk.perujuk")) {
            try (ResultSet rsCache = psCache.executeQuery()) {
                while (rsCache.next()) {
                    cachePerujukIgd.add(new CachePerujukIgd(
                            rsCache.getString("perujuk"), rsCache.getString("alamat")));
                }
            }
            cachePerujukIgdSiap = true;
        } catch (Exception e) {
            cachePerujukIgdSiap = false;
            System.out.println("Autocomplete IGD, muat cache asal rujukan: " + e);
        }
    }

    private void tampilkanPilihanDokterIgd(String kataKunci){
        if (sedangIsiAutocompleteIgd) return;
        String kata = kataKunci == null ? "" : kataKunci.trim();
        if (kata.length() < MIN_AUTOCOMPLETE_IGD) { popupDokterIgd.setVisible(false); return; }
        muatCacheDokterIgdJikaPerlu();
        modelDokterIgd.setRowCount(0);
        for (CacheDokterIgd item : cacheDokterIgd) {
            if (cocokAutocompleteIgd(kata, item.kode, item.nama)) {
                modelDokterIgd.addRow(new Object[]{item.kode, item.nama});
            }
        }
        tampilkanPopupIgd(TDokter, popupDokterIgd, tabelDokterIgd, modelDokterIgd.getRowCount());
    }

    private void tampilkanPilihanPenjabIgd(String kataKunci){
        if (sedangIsiAutocompleteIgd || sedangIsiJenisBayarOtomatisIgd) return;
        String kata = kataKunci == null ? "" : kataKunci.trim();
        if (kata.length() < MIN_AUTOCOMPLETE_IGD) { popupPenjabIgd.setVisible(false); return; }
        muatCachePenjabIgdJikaPerlu();
        modelPenjabIgd.setRowCount(0);
        for (CachePenjabIgd item : cachePenjabIgd) {
            if (cocokAutocompleteIgd(kata, item.kode, item.nama)) {
                modelPenjabIgd.addRow(new Object[]{item.kode, item.nama});
            }
        }
        tampilkanPopupIgd(nmpnj, popupPenjabIgd, tabelPenjabIgd, modelPenjabIgd.getRowCount());
    }

    private void tampilkanPilihanPerujukIgd(String kataKunci){
        if (sedangIsiAutocompleteIgd) return;
        String kata = kataKunci == null ? "" : kataKunci.trim();
        if (kata.length() < MIN_AUTOCOMPLETE_IGD) { popupPerujukIgd.setVisible(false); return; }
        muatCachePerujukIgdJikaPerlu();
        modelPerujukIgd.setRowCount(0);
        for (CachePerujukIgd item : cachePerujukIgd) {
            if (cocokAutocompleteIgd(kata, "", item.nama) || cocokAutocompleteIgd(kata, "", item.alamat)) {
                modelPerujukIgd.addRow(new Object[]{item.nama, item.alamat});
            }
        }
        tampilkanPopupIgd(AsalRujukan, popupPerujukIgd, tabelPerujukIgd, modelPerujukIgd.getRowCount());
    }

    private String namaDokterIgdBerdasarkanKode(String kode){
        if (kode == null || kode.trim().equals("")) return "";
        muatCacheDokterIgdJikaPerlu();
        for (CacheDokterIgd item : cacheDokterIgd) if (kode.trim().equals(item.kode)) return item.nama.trim();
        return "";
    }

    private String namaPenjabIgdBerdasarkanKode(String kode){
        if (kode == null || kode.trim().equals("")) return "";
        muatCachePenjabIgdJikaPerlu();
        for (CachePenjabIgd item : cachePenjabIgd) if (kode.trim().equals(item.kode)) return item.nama.trim();
        return "";
    }

    private boolean cocokAutocompleteIgd(String kata, String kode, String nama){
        String kunci = kata == null ? "" : kata.trim().toLowerCase(java.util.Locale.ROOT);
        String nilaiKode = kode == null ? "" : kode.toLowerCase(java.util.Locale.ROOT);
        String nilaiNama = nama == null ? "" : nama.toLowerCase(java.util.Locale.ROOT);
        return nilaiKode.contains(kunci) || nilaiNama.contains(kunci);
    }

    private String nilaiTabelAutocompleteIgd(JTable tabel, int row, int column){
        try {
            Object nilai = tabel.getValueAt(row, column);
    ...
    /** Popup mengikuti jumlah hasil 1 sampai 11 baris; selebihnya memakai scroll. */
    private void tampilkanPopupIgd(JTextField field, javax.swing.JPopupMenu popup, JTable tabel, int jumlah){
        if (jumlah <= 0) { popup.setVisible(false); return; }
        int lebar = Math.max(1, field.getWidth());
        int baris = Math.min(jumlah, MAX_BARIS_AUTOCOMPLETE_IGD);
        int tinggiData = Math.max(1, baris) * tabel.getRowHeight();
        int tinggiHeader = Math.max(HEADER_AUTOCOMPLETE_IGD, tabel.getTableHeader().getPreferredSize().height);
        int tinggi = tinggiHeader + tinggiData + 2;
        for (java.awt.Component component : popup.getComponents()) {
            if (component instanceof javax.swing.JScrollPane) {
                javax.swing.JScrollPane scroll = (javax.swing.JScrollPane) component;
                int lebarViewport = jumlah > MAX_BARIS_AUTOCOMPLETE_IGD
                        ? Math.max(1, lebar - scroll.getVerticalScrollBar().getPreferredSize().width) : lebar;
                Dimension viewport = new Dimension(lebarViewport, tinggiData);
                tabel.setPreferredScrollableViewportSize(viewport);
                scroll.getViewport().setPreferredSize(viewport);
                scroll.setPreferredSize(new Dimension(lebar, tinggi));
                scroll.setMinimumSize(new Dimension(lebar, tinggi));
                scroll.setSize(lebar, tinggi);
                scroll.revalidate();
                scroll.doLayout();
                break;
            }
        ...
        Dimension ukuran = new Dimension(lebar, tinggi + insets.top + insets.bottom);
        popup.setPreferredSize(ukuran);
        popup.setPopupSize(ukuran);
        popup.setSize(ukuran);
        popup.revalidate();
        tabel.setRowSelectionInterval(0, 0);
        popup.show(field, 0, field.getHeight());
        popup.setPopupSize(ukuran);
        // Jaga fokus tetap di kolom input agar popup tidak dianggap kehilangan fokus
        // segera setelah tampil. Ini juga membuat panah/Enter tetap bekerja dari field.
        javax.swing.SwingUtilities.invokeLater(new Runnable() {
            @Override public void run() {
                if (field.isShowing() && popup.isVisible() && !field.hasFocus()) {
    ...
    /** jenis: 1=dokter, 2=jenis bayar, 3=asal rujukan. */
    private boolean navigasiAutocompleteIgd(KeyEvent evt, JTable tabel, javax.swing.JPopupMenu popup, int jenis){
        if (popup == null || tabel == null || !popup.isVisible()) return false;
        int selected = tabel.getSelectedRow();
        if (evt.getKeyCode() == KeyEvent.VK_DOWN) {
            int next = Math.min(tabel.getRowCount() - 1, Math.max(0, selected + 1));
            tabel.setRowSelectionInterval(next, next);
            tabel.scrollRectToVisible(tabel.getCellRect(next, 0, true));
        } else if (evt.getKeyCode() == KeyEvent.VK_UP) {
            int previous = Math.max(0, selected - 1);
            tabel.setRowSelectionInterval(previous, previous);
            tabel.scrollRectToVisible(tabel.getCellRect(previous, 0, true));
        } else if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
            if (jenis == 1) pilihDokterAutocompleteIgd();
            else if (jenis == 2) pilihPenjabAutocompleteIgd();
            else pilihPerujukAutocompleteIgd();
    ...
    private void pilihDokterAutocompleteIgd(){
        int row = tabelDokterIgd.getSelectedRow();
        if (row < 0) return;
        isiDokterOtomatisIgd(nilaiTabelAutocompleteIgd(tabelDokterIgd, row, 0),
                nilaiTabelAutocompleteIgd(tabelDokterIgd, row, 1));
        isNumber();
        popupDokterIgd.setVisible(false);
        TNoRM.requestFocus();
    }

    private void pilihPenjabAutocompleteIgd(){
        int row = tabelPenjabIgd.getSelectedRow();
        if (row < 0) return;
        isiJenisBayarOtomatisIgd(nilaiTabelAutocompleteIgd(tabelPenjabIgd, row, 0),
                nilaiTabelAutocompleteIgd(tabelPenjabIgd, row, 1));
        popupPenjabIgd.setVisible(false);
        AsalRujukan.requestFocus();
    }

    private void pilihPerujukAutocompleteIgd(){
        int row = tabelPerujukIgd.getSelectedRow();
        if (row < 0) return;
        isiPerujukOtomatisIgd(nilaiTabelAutocompleteIgd(tabelPerujukIgd, row, 0),
                nilaiTabelAutocompleteIgd(tabelPerujukIgd, row, 1));
        popupPerujukIgd.setVisible(false);
        BtnSimpan.requestFocus();
    }

    private void isiDokterOtomatisIgd(String kode, String nama){
        boolean awal = sedangIsiAutocompleteIgd;
        sedangIsiAutocompleteIgd = true;
        try {
            if (timerDokterIgd != null) timerDokterIgd.stop();
            if (popupDokterIgd != null) popupDokterIgd.setVisible(false);
            KdDokter.setText(kode == null ? "" : kode);
            TDokter.setText(nama == null ? "" : nama);
        } finally { sedangIsiAutocompleteIgd = awal; }
    }

    private void isiJenisBayarDariKodeIgd(){
        String kode = kdpnj.getText().trim();
        String nama = "";
        if (!kode.equals("")) {
            nama = Sequel.cariIsi("select penjab.png_jawab from penjab where penjab.kd_pj=?", kode);
        }
        isiJenisBayarOtomatisIgd(kode, nama);
    }

    /** Pengisian otomatis tidak boleh memunculkan popup Jenis Bayar. */
    private void isiJenisBayarOtomatisIgd(String kode, String nama){
        boolean awal = sedangIsiJenisBayarOtomatisIgd;
        sedangIsiJenisBayarOtomatisIgd = true;
        try {
            if (timerPenjabIgd != null) timerPenjabIgd.stop();
            if (popupPenjabIgd != null) popupPenjabIgd.setVisible(false);
            kdpnj.setText(kode == null ? "" : kode);
            nmpnj.setText(nama == null ? "" : nama);
        } finally { sedangIsiJenisBayarOtomatisIgd = awal; }
    }

    private void isiPerujukOtomatisIgd(String nama, String alamat){
        boolean awal = sedangIsiAutocompleteIgd;
        sedangIsiAutocompleteIgd = true;
        try {
            if (timerPerujukIgd != null) timerPerujukIgd.stop();
            if (popupPerujukIgd != null) popupPerujukIgd.setVisible(false);
            AsalRujukan.setText(nama == null ? "" : nama);
            alamatperujuk = (alamat == null || alamat.trim().equals("")) ? "-" : alamat.trim();
        } finally { sedangIsiAutocompleteIgd = awal; }
    }

    /** Dokter yang diketik harus menghasilkan kd_dokter valid sebelum simpan. */
    private boolean validasiDokterAutocompleteIgd(){
        String kode = KdDokter.getText().trim();
        String nama = TDokter.getText().trim();
        muatCacheDokterIgdJikaPerlu();
        if (kode.equals("") && !nama.equals("")) {
            String kodeDitemukan = "", namaDitemukan = "";
            for (CacheDokterIgd item : cacheDokterIgd) {
                if (item.nama.equalsIgnoreCase(nama)) {
                    if (!kodeDitemukan.equals("") && !kodeDitemukan.equals(item.kode)) {
                        JOptionPane.showMessageDialog(null, "Nama dokter lebih dari satu. Silakan pilih dari autocomplete.");
                        TDokter.requestFocus();
                        return false;
                    }
                    kodeDitemukan = item.kode;
                    namaDitemukan = item.nama;
                }
            }
            if (!kodeDitemukan.equals("")) {
                isiDokterOtomatisIgd(kodeDitemukan, namaDitemukan);
                return true;
            }
        }
        if (kode.equals("") || namaDokterIgdBerdasarkanKode(kode).equals("")
                || !namaDokterIgdBerdasarkanKode(kode).equalsIgnoreCase(nama)) {
            JOptionPane.showMessageDialog(null, "Dokter belum valid. Pilih dari autocomplete atau tombol Pilih Dokter.");
            TDokter.requestFocus();
            return false;
        }
        return true;
    ...
    /** Mencegah kd_pj tidak valid dan error foreign key saat simpan. */
    private boolean validasiJenisBayarAutocompleteIgd(){
        String kode = kdpnj.getText().trim();
        String nama = nmpnj.getText().trim();
        muatCachePenjabIgdJikaPerlu();
        if (kode.equals("") && !nama.equals("")) {
            String kodeDitemukan = "", namaDitemukan = "";
            for (CachePenjabIgd item : cachePenjabIgd) {
                if (item.nama.equalsIgnoreCase(nama)) {
                    if (!kodeDitemukan.equals("") && !kodeDitemukan.equals(item.kode)) {
                        JOptionPane.showMessageDialog(null, "Nama jenis bayar lebih dari satu. Silakan pilih dari autocomplete.");
                        nmpnj.requestFocus();
                        return false;
                    }
                    kodeDitemukan = item.kode;
            ...
            if (!kodeDitemukan.equals("")) {
                isiJenisBayarOtomatisIgd(kodeDitemukan, namaDitemukan);
                return true;
            }
        }
        if (kode.equals("") || namaPenjabIgdBerdasarkanKode(kode).equals("")
                || !namaPenjabIgdBerdasarkanKode(kode).equalsIgnoreCase(nama)) {
            JOptionPane.showMessageDialog(null, "Jenis bayar belum valid. Pilih dari autocomplete atau tombol Pilih Jenis Bayar.");
            nmpnj.requestFocus();
            return false;
        }
        return true;
...
private void KdDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_KdDokterKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_PAGE_DOWN){
            isNumber();
            isiDokterOtomatisIgd(KdDokter.getText(), getDokter().tampil3(KdDokter.getText()));
        }else if(evt.getKeyCode()==KeyEvent.VK_UP){
...
private void BtnDokterActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnDokterActionPerformed
        akses.setform("DlgIGD");
        getDokter().isCek();        
        getDokter().TCari.requestFocus();
        getDokter().setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
        getDokter().setLocationRelativeTo(internalFrame1);
        getDokter().setVisible(true);
...
private void MnCetakSuratKeteranganSakitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCetakSuratKeteranganSakitActionPerformed
     if(tabMode.getRowCount()==0){
...
}//GEN-LAST:event_MnCetakSuratKeteranganSakitActionPerformed
...
private void btnKelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnKelActionPerformed
        akses.setform("DlgIGD");
        getPasien().kel.emptTeks();
        getPasien().kel.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
        getPasien().kel.setLocationRelativeTo(internalFrame1);
        getPasien().kel.setVisible(true);
...
private void btnKecActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnKecActionPerformed
   akses.setform("DlgIGD");
   getPasien().kec.emptTeks();
        getPasien().kec.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
        getPasien().kec.setLocationRelativeTo(internalFrame1);
        getPasien().kec.setVisible(true);      
...
private void btnKabActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnKabActionPerformed
        akses.setform("DlgIGD");
        getPasien().kab.emptTeks();
        getPasien().kab.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
        getPasien().kab.setLocationRelativeTo(internalFrame1);
        getPasien().kab.setVisible(true);   
...
private void btnPenjabActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPenjabActionPerformed
        akses.setform("DlgIGD");
        getPasien().penjab.onCari();
        getPasien().penjab.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
        getPasien().penjab.setLocationRelativeTo(internalFrame1);
        getPasien().penjab.setVisible(true);
...
private void MnLaporanRekapKunjunganBulananPoliActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnLaporanRekapKunjunganBulananPoliActionPerformed
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        // Beri kesempatan Swing menggambar form terlebih dahulu; query tabel
        // tetap memakai fungsi tampil() yang sama dan tidak mengubah hasil data.
        LCount.setText("Memuat data IGD ...");
        javax.swing.Timer tundaMuat=new javax.swing.Timer(80, new ActionListener() {
            @Override public void actionPerformed(ActionEvent e) {
                tampil();
            }
        });
        tundaMuat.setRepeats(false);
        tundaMuat.start();
    ...
    private void AsalRujukanKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_AsalRujukanKeyPressed
        if(navigasiAutocompleteIgd(evt, tabelPerujukIgd, popupPerujukIgd, 3)){
            return;
        }
        if(evt.getKeyCode()==KeyEvent.VK_PAGE_UP){
            ...
            btnPenjab1ActionPerformed(null);
        }
    ...
    private void btnPenjab1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPenjab1ActionPerformed
        akses.setform("DlgIGD");
        getRujukMasuk().tampil2();
        getRujukMasuk().TCariPerujuk.requestFocus();
        getRujukMasuk().WindowPerujuk.setSize(this.getWidth()-20,this.getHeight()-20);
        getRujukMasuk().WindowPerujuk.setLocationRelativeTo(internalFrame1);
        getRujukMasuk().WindowPerujuk.setVisible(true);
    ...
    private void MnMeninggalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnMeninggalActionPerformed
        if(TNoRw.getText().trim().equals("")){
            ...
            if(Sequel.cariInteger("select count(kamar_inap.no_rawat) from kamar_inap where kamar_inap.no_rawat=?",TNoRw.getText())>0){
                ...
                dlgPasienMati.setNoRm(TNoRM.getText(), TPasien.getText(), TNoRw.getText());
                dlgPasienMati.isCek();
    ...
    private void MnPermintaanRanapActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnPermintaanRanapActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            if(Sequel.cariInteger("select count(kamar_inap.no_rawat) from kamar_inap where kamar_inap.no_rawat=?",TNoRw.getText())>0){
                ...
                form.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
                form.setLocationRelativeTo(internalFrame1);
                form.setNoRm(TNoRw.getText(),TNoRM.getText(),TPasien.getText(),TDokter.getText(),TPngJwb.getText(),"UGD/IGD",Sequel.cariIsi("select pasien.no_tlp from pasien where pasien.no_rkm_medis=?",TNoRM.getText()),KdDokter.getText());
                form.setVisible(true);
                this.setCursor(Cursor.getDefaultCursor());
            }
        }
    ...
    private void MnCetakSuratKeteranganBerbadanSehatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCetakSuratKeteranganBerbadanSehatActionPerformed
     if(tabMode.getRowCount()==0){
        JOptionPane.showMessageDialog(null,"Maaf, data pasien sudah habis...!!!!");
        TNoRw.requestFocus();
    }else if(TPasien.getText().trim().equals("")){
    ...
    private void MnRujukSatuSehatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnRujukSatuSehatActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            try {
                SatuSehatRujukanForm dlgki=new SatuSehatRujukanForm(null,false);
                isiDataRujukanSatuSehat(dlgki);
                dlgki.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
                dlgki.setLocationRelativeTo(internalFrame1);
                dlgki.setVisible(true);
            } finally {
    ...
    }//GEN-LAST:event_MnRujukSatuSehatActionPerformed
    ...
    */
    ...
    private String nilaiTabel(javax.swing.JTable tabel, int row, int col) {
        try {
            Object nilai = tabel.getValueAt(row, col);
            return nilai == null ? "" : nilai.toString();
        } catch (Exception e) {
            return "";
        }
    ...
    private widget.Button BtnAmbilFotoKTP;
    private widget.Button BtnBatal;
    private widget.Button BtnCari;
    private widget.Button BtnPantauFingerBPJS;
    private widget.Button BtnRefreshFotoKTP;
    ...
    private widget.Button BtnUnduhFotoKTP;
    private widget.Button BtnUnggahFotoKTP;
    private widget.Button BtnZoomFotoKTP;
    ...
    private widget.Label LblFotoKTP;
    ...
    private javax.swing.JMenuItem MnCetakSuratKeteranganBerbadanSehat;
    private javax.swing.JMenuItem MnCetakSuratKeteranganSakit;
    ...
    private widget.Button MnRujukSatuSehat;
    ...
    private javax.swing.JMenuItem MnSisruteSatuSehat;
...
//        try{  
//            ps=koneksi.prepareStatement("select DISTINCT reg_periksa.no_reg,reg_periksa.no_rawat,reg_periksa.tgl_registrasi,reg_periksa.jam_reg,"+
//                   "reg_periksa.kd_dokter,dokter.nm_dokter,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.jk,concat(reg_periksa.umurdaftar,' ',reg_periksa.sttsumur)as umur,poliklinik.nm_poli,"+
...
//                   "              WHERE berkas_digital_perawatan.no_rawat = reg_periksa.no_rawat "+
//                   "                AND berkas_digital_perawatan.kode = 'SOD') "+
//                   "          THEN 'ADA SO'"+
//                   "     WHEN reg_periksa.status_lanjut = 'Ranap' THEN 'TIDAK ADA SO' "+     
//                   "     ELSE 'RAWAT JALAN' END AS SO "+
//                   //end modif 
//                   "from reg_periksa LEFT JOIN berkas_digital_perawatan ON reg_periksa.no_rawat = berkas_digital_perawatan.no_rawat inner join dokter inner join pasien inner join poliklinik inner join penjab "+
//                   "on reg_periksa.kd_dokter=dokter.kd_dokter and reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
...
//                        rs.getString("kd_pj"),rs.getString("status_bayar"),rs.getString("SO")
//                    });
//                } 
//            } catch (Exception e) {
//                System.out.println("Notif : "+e);
...
//        LCount.setText(""+tabMode.getRowCount());
    ...
    try {
        ps = koneksi.prepareStatement(
            ...
            "     ELSE 'RAWAT JALAN' END AS SO, " +
            "DATE_FORMAT((" +
            "     SELECT MAX(ki.tgl_keluar) " +
            "     FROM reg_periksa ri " +
            "     INNER JOIN kamar_inap ki ON ki.no_rawat=ri.no_rawat " +
            "     WHERE ri.no_rkm_medis=reg_periksa.no_rkm_medis " +
            "       AND ri.status_lanjut='Ranap' " +
            "       AND ki.stts_pulang NOT IN ('-','Pindah Kamar') " +
            "       AND ki.tgl_keluar IS NOT NULL " +
            "       AND ki.tgl_keluar<>'0000-00-00 00:00:00' " +
            "       AND ki.tgl_keluar<CONCAT(reg_periksa.tgl_registrasi,' 00:00:00')" +
            "), '%Y-%m-%d') AS last_ranap, " +
            "DATE_FORMAT((" +
            "     SELECT MAX(rr.tgl_registrasi) " +
            "     FROM reg_periksa rr " +
            "     WHERE rr.no_rkm_medis=reg_periksa.no_rkm_medis " +
            "       AND rr.status_lanjut='Ralan' " +
            "       AND rr.tgl_registrasi<reg_periksa.tgl_registrasi" +
            "), '%Y-%m-%d') AS last_ralan, " +
            "IF(reg_periksa.kd_pj='BPJ',IF(bridging_sep.no_sep IS NULL,'SEP Belum Terbit',bridging_sep.no_sep),'-') AS no_sep, " +
            "IF(reg_periksa.kd_pj='BPJ',IF(sdk.no_rawat IS NOT NULL,sdk.jam_val," +
            "IF(khadir.no_rawat IS NOT NULL,DATE_FORMAT(khadir.tanggal_konfirmasi,'%Y-%m-%d %H:%i:%s'),'Belum Sidik Jari')),'-') AS jam_sidik_jari " +
            "from reg_periksa " +
            ...
            "left join (select sj.no_rawat,MAX(sj.validasi) as jam_val from sidik_jari_bpjs sj " +
            "where sj.validasi>=? and sj.validasi<DATE_ADD(?,INTERVAL 1 DAY) group by sj.no_rawat) sdk " +
            "on reg_periksa.no_rawat=sdk.no_rawat " +
            "left join bpjs_konfirmasi_hadir khadir on reg_periksa.no_rawat=khadir.no_rawat " +
            ...
            (TCari.getText().trim().equals("") ? "" :
                ...
                "reg_periksa.stts_daftar like ? or pasien.nm_pasien like ? or poliklinik.nm_poli like ? or " +
                "reg_periksa.p_jawab like ? or reg_periksa.almt_pj like ? or reg_periksa.hubunganpj like ? or " +
                "penjab.png_jawab like ?) ") + terbitsep +
            " order by reg_periksa.no_rawat"
        );
        try {
            String tglAwalFinger = Valid.SetTgl(DTPCari1.getSelectedItem() + "");
            String tglAkhirFinger = Valid.SetTgl(DTPCari2.getSelectedItem() + "");
            ps.setString(1, tglAwalFinger);
            ps.setString(2, tglAkhirFinger);
            ps.setString(3, tglAwalFinger);
            ps.setString(4, tglAkhirFinger);
            if (!TCari.getText().trim().equals("")) {
                ps.setString(5,  "%" + TCari.getText().trim() + "%");
                ps.setString(6,  "%" + TCari.getText().trim() + "%");
                ps.setString(7,  "%" + TCari.getText().trim() + "%");
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                String tglRegStr = rs.getString(3);
                String riwayat = "-";

                try {
                    java.time.LocalDate tglReg = java.time.LocalDate.parse(tglRegStr);
                    String lastPulangStr = rs.getString("last_ranap");
                    String lastRalanStr = rs.getString("last_ralan");

                    java.time.LocalDate lastRanap = null, lastRalan = null;
                    if (lastPulangStr != null && !lastPulangStr.trim().equals("")) {
                        lastRanap = java.time.LocalDate.parse(lastPulangStr);
                    }
                    if (lastRalanStr != null && !lastRalanStr.trim().equals("")) {
                        lastRalan = java.time.LocalDate.parse(lastRalanStr);
                    }

                    java.time.LocalDate lastDate = null;
                    String sumber = "";
                    if (lastRanap != null && (lastRalan == null || !lastRalan.isAfter(lastRanap))) {
                        lastDate = lastRanap;
                        sumber = "Ranap";
                    }
                    if (lastRalan != null && (lastRanap == null || lastRalan.isAfter(lastRanap))) {
                        lastDate = lastRalan;
                        sumber = "Ralan";
                    }

                    if (lastDate != null) {
                        long hari = java.time.temporal.ChronoUnit.DAYS.between(lastDate, tglReg);
                        if (hari >= 0) {
                            if (hari > 60) {
                                riwayat = "1 Tahun " + sumber;
                            } else if (hari > 30) {
                                riwayat = "1 Bulan " + sumber;
                            } else {
                                riwayat = hari + " hari " + sumber;
                            }
                        }
                    }
                } catch (Exception exHari) {
                    System.out.println("Hitung Riwayat Terakhir: " + exHari);
                }
                ...
                tabMode.addRow(new Object[]{
                    ...
                    Valid.SetAngka(rs.getDouble(15)), rs.getString(16), rs.getString(17), rs.getString(18),
                    rs.getString("kd_pj"), rs.getString("status_bayar"), rs.getString("SO"),
                    riwayat, rs.getString("no_sep"), rs.getString("jam_sidik_jari")
                });
            }
        } catch (Exception e) {
            System.out.println("Notif : " + e);
    ...
    public void emptTeks() {
     ...
     /**
      * Tombol hasil penunjang IGD dipasang dinamis agar file .form tidak berubah.
      */
     private void initTombolHasilPenunjangIGD() {
         try {
             BtnHasilLab = new widget.Button();
             try { BtnHasilLab.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/hasil_lab.png"))); } catch (Exception exIcon) {}
             BtnHasilLab.setMnemonic('L');
             BtnHasilLab.setText("Lihat Hasil Lab");
             BtnHasilLab.setToolTipText("Lihat Hasil Laboratorium");
             BtnHasilLab.setName("BtnHasilLab");
             BtnHasilLab.setPreferredSize(new java.awt.Dimension(135, 30));
             ...
             try { BtnHasilRad.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/hasil_rad.png"))); } catch (Exception exIcon) {}
             BtnHasilRad.setMnemonic('R');
             BtnHasilRad.setText("Lihat Hasil Rad");
             BtnHasilRad.setToolTipText("Lihat Hasil Radiologi");
             BtnHasilRad.setName("BtnHasilRad");
             BtnHasilRad.setPreferredSize(new java.awt.Dimension(135, 30));
             ...
             BtnHasilLab.setVisible(false);
             BtnHasilRad.setVisible(false);
             panelGlass7.add(BtnHasilLab);
             panelGlass7.add(BtnHasilRad);
             panelGlass7.revalidate();
             panelGlass7.repaint();
         } catch (Exception ex) {
             System.out.println("Notifikasi init tombol hasil penunjang IGD : " + ex);
         }
     }

     private void BtnHasilLabActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHasilLabActionPerformed
         bukaDlgHasilPenunjang(JENIS_HASIL_LAB_IGD);
     ...
     private void BtnHasilRadActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHasilRadActionPerformed
         bukaDlgHasilPenunjang(JENIS_HASIL_RAD_IGD);
     }//GEN-LAST:event_BtnHasilRadActionPerformed

     private String getNoRawatBerkasTerpilih() {
         String noRawatTerpilih = "";
         try {
             if (TNoRw != null && TNoRw.getText() != null) {
                 noRawatTerpilih = TNoRw.getText().trim();
             }
             if (noRawatTerpilih.equals("") && tbPetugas != null && tbPetugas.getSelectedRow() > -1) {
                 noRawatTerpilih = tbPetugas.getValueAt(tbPetugas.getSelectedRow(), 2).toString().trim();
             }
         } catch (Exception ex) {
             System.out.println("Notifikasi ambil no rawat hasil penunjang IGD : " + ex);
         }
         return noRawatTerpilih;
     }

     /** Data satu berkas hasil Lab/Radiologi yang ditampilkan pada browser. */
     private static final String JENIS_HASIL_LAB_IGD = DlgHasilPenunjang.JENIS_LAB;
     private static final String JENIS_HASIL_RAD_IGD = DlgHasilPenunjang.JENIS_RAD;

     private void bukaDlgHasilPenunjang(String jenis) {
         String noRawatTerpilih = getNoRawatBerkasTerpilih();
         if (noRawatTerpilih.equals("")) {
             JOptionPane.showMessageDialog(null, "Maaf, silahkan pilih dulu data pasien...!!!");
             tbPetugas.requestFocus();
             return;
         }
         DlgHasilPenunjang.buka(this, noRawatTerpilih, jenis);
     }

     private void refreshTombolHasilLabRad() {
         DlgHasilPenunjang.refreshTombolAsync(getNoRawatBerkasTerpilih(), BtnHasilLab, BtnHasilRad, panelGlass7, 30);
     }
    ...
    private void getData() {
        if(tbPetugas.getSelectedRow()!= -1){
            ...
            isiJenisBayarOtomatisIgd(
                    ...
                    tbPetugas.getValueAt(tbPetugas.getSelectedRow(),17).toString());
            refreshTombolHasilLabRad();
        }
    }
        ...
        try {   
            ...
            try {
                ...
                while(rs.next()){
                    ...
                    isiJenisBayarOtomatisIgd(rs.getString("kd_pj"), rs.getString("png_jawab"));
    ...
    public void isCek(){
        ...
        MnCetakSuratKeteranganSakit.setEnabled(akses.getsurat_sakit());
...
//        MnSkorBromagePascaAnestesi.setEnabled(akses.getskor_bromage_pasca_anestesi());
    ...
    public void SetPasien(String norm,String nosisrute,String FaskesAsal){
        ...
        isiPerujukOtomatisIgd(FaskesAsal, "-");
        isPas();
    }
    ...
    public void setPasien(String NamaPasien,String Kontak,String Alamat,String TempatLahir,String TglLahir,
        ...
        this.nosisrute=nosisrute;
        isiPerujukOtomatisIgd(FaskesAsal, "-");
        getPasien().setPasien(NamaPasien, Kontak, Alamat, TempatLahir, TglLahir, JK, NoKartuJKN, NIK);
        getPasien().setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
        getPasien().setLocationRelativeTo(internalFrame1);
        getPasien().setVisible(true);
    }
    ...
    // ================= PATCH RSAJ : BIO/FINGER + FOTO KTP IGD =================
    private void initBioFingerDanFotoKTPIGD(){
        BtnPantauFingerBPJS.setVisible(false);
        otomatisRefreshFingerIGD();
        pasangPemantauNoRMFotoKTP();
        tampilkanFotoKTPKosong();
        aturTombolFotoKTP();

        tbPetugas.getSelectionModel().addListSelectionListener(new javax.swing.event.ListSelectionListener() {
            @Override
            public void valueChanged(javax.swing.event.ListSelectionEvent e) {
                if(!e.getValueIsAdjusting()){
                    aturTombolPantauFingerBPJS();
                    panggilFotoKTPDariTabel();
                }
            }
        });
    }

    private void pantauFingerBPJSIGD(){
        if(tbPetugas.getSelectedRow() == -1){
            JOptionPane.showMessageDialog(null,"Silakan pilih pasien pada tabel IGD terlebih dahulu.");
            tbPetugas.requestFocus();
            return;
        }

        int baris = tbPetugas.getSelectedRow();
        String noRawat = nilaiTabel(tbPetugas, baris, 2);
        String kdPj = nilaiTabel(tbPetugas, baris, IDX_IGD_KD_PJ);
        String noSep = nilaiTabel(tbPetugas, baris, IDX_IGD_NO_SEP);
        String statusFinger = nilaiTabel(tbPetugas, baris, IDX_IGD_FINGER);

        if(!kdPj.equals("BPJ")){
            JOptionPane.showMessageDialog(null,"Pantau Finger hanya untuk pasien BPJS Kesehatan.");
            return;
        }
        if(!noSep.equals("SEP Belum Terbit")){
            JOptionPane.showMessageDialog(null,"SEP pasien sudah terbit, pemantauan finger tidak diperlukan.");
            aturTombolPantauFingerBPJS();
            return;
        }

        int usiaTahun = ambilUsiaTahunSaatRegistrasi(noRawat);
        if(usiaTahun < 0){
            JOptionPane.showMessageDialog(null,
                    "Umur pasien tidak dapat ditentukan dari tanggal lahir dan tanggal registrasi.\n"+
                    "Silakan periksa kembali data tanggal lahir pasien.");
            return;
        }

        if(usiaTahun < 17){
            if(sudahKonfirmasiHadirAnak(noRawat)){
                JOptionPane.showMessageDialog(null,"Pasien usia <17 tahun ini sudah dikonfirmasi hadir di loket.");
                aturTombolPantauFingerBPJS();
                return;
            }
            if(simpanKonfirmasiHadirAnak(noRawat)){
                JOptionPane.showMessageDialog(null,
                        "Kedatangan pasien usia <17 tahun berhasil dikonfirmasi.\n"+
                        "Pasien tidak perlu melakukan Bio/Finger.");
                tampil();
                aturTombolPantauFingerBPJS();
            }
            return;
        ...
        if(sudahAdaMonitorFingerBPJS(noRawat)){
            JOptionPane.showMessageDialog(null,"Pasien sudah masuk antrean Pantau Finger BPJS.");
            aturTombolPantauFingerBPJS();
            aktifkanAutoRefreshPantauFingerIGD();
            return;
        }
        if(!statusFinger.equals("Belum Sidik Jari")){
            JOptionPane.showMessageDialog(null,"Finger pasien sudah tercatat, silakan lanjutkan pembuatan SEP.");
            return;
        }

        String sqlPantau = "INSERT INTO bpjs_finger_monitor " +
                "(no_rawat,no_kartu,tgl_pelayanan,status,attempt_count,last_checked_at,next_check_at,"+
                "response_code,response_message,requested_at,completed_at,created_at,updated_at) " +
                "SELECT rp.no_rawat,p.no_peserta,rp.tgl_registrasi,'ACTIVE',0,NULL,NOW(),NULL,NULL,NOW(),NULL,NOW(),NOW() " +
                "FROM reg_periksa rp INNER JOIN pasien p ON rp.no_rkm_medis=p.no_rkm_medis " +
                "WHERE rp.no_rawat=? AND rp.kd_pj='BPJ' " +
                "ON DUPLICATE KEY UPDATE no_kartu=VALUES(no_kartu),tgl_pelayanan=VALUES(tgl_pelayanan),"+
                "status='ACTIVE',attempt_count=0,last_checked_at=NULL,next_check_at=NOW(),"+
                "response_code=NULL,response_message=NULL,requested_at=NOW(),completed_at=NULL,updated_at=NOW()";

        try(PreparedStatement psPantau = koneksi.prepareStatement(sqlPantau)){
            psPantau.setString(1,noRawat);
            int hasil = psPantau.executeUpdate();
            if(hasil > 0){
                JOptionPane.showMessageDialog(null,
                        "Pasien berhasil dimasukkan ke antrean Pantau Finger BPJS.\n"+
                        "Status akan diperbarui otomatis saat Bio/Finger sudah tercatat.");
                tampil();
                aturTombolPantauFingerBPJS();
                aktifkanAutoRefreshPantauFingerIGD();
            }else{
                JOptionPane.showMessageDialog(null,"Data pasien BPJS tidak ditemukan.");
            }
        }catch(Exception e){
            JOptionPane.showMessageDialog(null,
                    "Gagal menyimpan antrean Pantau Finger BPJS.\n"+
                    "Pastikan tabel bpjs_finger_monitor sudah dibuat.\n\n"+e.getMessage());
            System.out.println("Notifikasi Pantau Finger BPJS IGD : "+e);
        }
    }

    private void otomatisRefreshFingerIGD(){
        if(timerPantauFingerIGD != null){
            timerPantauFingerIGD.stop();
        }
        timerPantauFingerIGD = new Timer(INTERVAL_PANTAU_FINGER_IGD, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                if(!isVisible()){
                    timerPantauFingerIGD.stop();
                ...
                refreshStatusPantauFingerIGD();
            }
        });
        timerPantauFingerIGD.setRepeats(true);
        timerPantauFingerIGD.setInitialDelay(INTERVAL_PANTAU_FINGER_IGD);
    }

    private void aktifkanAutoRefreshPantauFingerIGD(){
        if(timerPantauFingerIGD == null){
            otomatisRefreshFingerIGD();
        }
        if(!timerPantauFingerIGD.isRunning()){
            timerPantauFingerIGD.start();
        }
    }

    private void hentikanAutoRefreshPantauFingerIGD(){
        if(timerPantauFingerIGD != null){
            timerPantauFingerIGD.stop();
        }
    }

    private void aturTombolPantauFingerBPJS(){
        try{
            BtnPantauFingerBPJS.setVisible(false);
            BtnPantauFingerBPJS.setEnabled(false);
            if(tbPetugas == null || tbPetugas.getSelectedRow() == -1 || tabMode.getRowCount() == 0){
                return;
            }
            ...
            BtnPantauFingerBPJS.setVisible(true);
            if(!noSep.equals("SEP Belum Terbit")){
                BtnPantauFingerBPJS.setToolTipText("SEP sudah terbit, tombol Bio/Finger dinonaktifkan.");
                return;
            }

            int usiaTahun = ambilUsiaTahunSaatRegistrasi(noRawat);
            if(usiaTahun >= 0 && usiaTahun < 17){
                if(sudahKonfirmasiHadirAnak(noRawat)){
                    BtnPantauFingerBPJS.setToolTipText("Pasien usia <17 tahun sudah dikonfirmasi hadir di loket.");
                }else{
                    BtnPantauFingerBPJS.setEnabled(true);
                    BtnPantauFingerBPJS.setToolTipText("Pasien usia <17 tahun: klik untuk menandai pasien sudah hadir di loket.");
                }
                return;
            }
            ...
            String statusFinger = nilaiTabel(tbPetugas, baris, IDX_IGD_FINGER);
            if(!statusFinger.equals("Belum Sidik Jari")){
                BtnPantauFingerBPJS.setToolTipText("Bio/Finger pasien sudah tercatat.");
                return;
            }
            if(sudahAdaMonitorFingerBPJS(noRawat)){
                BtnPantauFingerBPJS.setToolTipText("Pasien sudah ada di antrean bpjs_finger_monitor.");
                aktifkanAutoRefreshPantauFingerIGD();
                return;
            }

            BtnPantauFingerBPJS.setEnabled(true);
            BtnPantauFingerBPJS.setToolTipText("Klik untuk memasukkan pasien ke antrean Pantau Finger BPJS.");
        }catch(Exception e){
            BtnPantauFingerBPJS.setVisible(false);
            BtnPantauFingerBPJS.setEnabled(false);
            System.out.println("Notifikasi Atur Tombol Pantau Finger BPJS IGD : "+e);
        }
    }

    private int ambilUsiaTahunSaatRegistrasi(String noRawat){
        if(noRawat == null || noRawat.trim().equals("")) return -1;
        String sqlUsia = "SELECT TIMESTAMPDIFF(YEAR,p.tgl_lahir,rp.tgl_registrasi) AS usia_tahun " +
                "FROM reg_periksa rp INNER JOIN pasien p ON rp.no_rkm_medis=p.no_rkm_medis " +
                "WHERE rp.no_rawat=? LIMIT 1";
        try(PreparedStatement psUsia = koneksi.prepareStatement(sqlUsia)){
            psUsia.setString(1,noRawat);
            try(ResultSet rsUsia = psUsia.executeQuery()){
                if(rsUsia.next()){
                    int usia = rsUsia.getInt("usia_tahun");
                    if(!rsUsia.wasNull()) return usia;
                }
            }
        }catch(Exception e){
            System.out.println("Notifikasi Cek Usia Pasien BPJS IGD : "+e);
        }
        return -1;
    }

    private boolean sudahKonfirmasiHadirAnak(String noRawat){
        if(noRawat == null || noRawat.trim().equals("")) return false;
        String sql = "SELECT no_rawat FROM bpjs_konfirmasi_hadir WHERE no_rawat=? LIMIT 1";
        try(PreparedStatement psCek = koneksi.prepareStatement(sql)){
            psCek.setString(1,noRawat);
            try(ResultSet rsCek = psCek.executeQuery()){
                return rsCek.next();
            }
        }catch(Exception e){
            System.out.println("Notifikasi Cek Konfirmasi Hadir Pasien Anak IGD : "+e);
            return false;
        }
    }

    private boolean simpanKonfirmasiHadirAnak(String noRawat){
        String sql = "INSERT INTO bpjs_konfirmasi_hadir " +
                "(no_rawat,no_rkm_medis,jenis_konfirmasi,usia_tahun,tanggal_konfirmasi,petugas) " +
                "SELECT rp.no_rawat,rp.no_rkm_medis,'ANAK_TANPA_FINGER'," +
                "TIMESTAMPDIFF(YEAR,p.tgl_lahir,rp.tgl_registrasi),NOW(),? " +
                "FROM reg_periksa rp INNER JOIN pasien p ON rp.no_rkm_medis=p.no_rkm_medis " +
                "WHERE rp.no_rawat=? AND rp.kd_pj='BPJ' " +
                "AND TIMESTAMPDIFF(YEAR,p.tgl_lahir,rp.tgl_registrasi)<17 " +
                "ON DUPLICATE KEY UPDATE jenis_konfirmasi=VALUES(jenis_konfirmasi)," +
                "usia_tahun=VALUES(usia_tahun),tanggal_konfirmasi=VALUES(tanggal_konfirmasi),petugas=VALUES(petugas)";
        try(PreparedStatement psSimpan = koneksi.prepareStatement(sql)){
            psSimpan.setString(1,akses.getkode());
            psSimpan.setString(2,noRawat);
            return psSimpan.executeUpdate() > 0;
        }catch(Exception e){
            JOptionPane.showMessageDialog(null,
                    "Gagal menyimpan konfirmasi hadir pasien usia <17 tahun.\n"+
                    "Pastikan tabel bpjs_konfirmasi_hadir sudah dibuat.\n\n"+e.getMessage());
            System.out.println("Notifikasi Simpan Konfirmasi Hadir Pasien Anak IGD : "+e);
            return false;
        }
    }

    private boolean sudahAdaMonitorFingerBPJS(String noRawat){
        if(noRawat == null || noRawat.trim().equals("")) return false;
        String sql = "select no_rawat from bpjs_finger_monitor where no_rawat=? limit 1";
        try(PreparedStatement psCek = koneksi.prepareStatement(sql)){
            psCek.setString(1,noRawat);
            try(ResultSet rsCek = psCek.executeQuery()){
                return rsCek.next();
            ...
            System.out.println("Notifikasi Cek Monitor Finger BPJS IGD : "+e);
            return false;
        }
    }

    private void refreshStatusPantauFingerIGD(){
        if(tabMode.getRowCount() == 0){
            hentikanAutoRefreshPantauFingerIGD();
            return;
        }

        Map<String,Integer> barisCalonPantau = new HashMap<String,Integer>();
        for(int baris=0; baris<tabMode.getRowCount(); baris++){
            String noRawat = nilaiModelIGD(baris,2);
            String kdPj = nilaiModelIGD(baris,IDX_IGD_KD_PJ);
            String noSep = nilaiModelIGD(baris,IDX_IGD_NO_SEP);
            String statusFinger = nilaiModelIGD(baris,IDX_IGD_FINGER);
            if(!noRawat.equals("") && kdPj.equals("BPJ") && noSep.equals("SEP Belum Terbit") &&
                    statusFinger.equals("Belum Sidik Jari")){
                barisCalonPantau.put(noRawat,baris);
            }
        }
        if(barisCalonPantau.isEmpty()){
            hentikanAutoRefreshPantauFingerIGD();
            return;
        }

        StringBuilder parameter = new StringBuilder();
        for(int i=0;i<barisCalonPantau.size();i++){
            if(i>0) parameter.append(",");
            parameter.append("?");
        }
        String sql = "SELECT monitor.no_rawat,MAX(sidik.validasi) AS jam_sidik_jari " +
                "FROM bpjs_finger_monitor monitor " +
                "LEFT JOIN sidik_jari_bpjs sidik ON sidik.no_rawat=monitor.no_rawat " +
                "WHERE monitor.no_rawat IN ("+parameter.toString()+") GROUP BY monitor.no_rawat";

        boolean adaAntrean = false;
        boolean masihMenunggu = false;
        boolean adaPerubahan = false;
        try(PreparedStatement psStatus = koneksi.prepareStatement(sql)){
            int p=1;
            for(String noRawat:barisCalonPantau.keySet()) psStatus.setString(p++,noRawat);
            try(ResultSet rsStatus = psStatus.executeQuery()){
                while(rsStatus.next()){
                    adaAntrean = true;
                    Integer baris = barisCalonPantau.get(rsStatus.getString("no_rawat"));
                    String jam = rsStatus.getString("jam_sidik_jari");
                    if(baris != null && jam != null && !jam.trim().equals("")){
                        tabMode.setValueAt(jam,baris,IDX_IGD_FINGER);
                        adaPerubahan = true;
                    }else{
                        masihMenunggu = true;
                    }
            ...
            System.out.println("Notifikasi Auto Refresh Pantau Finger BPJS IGD : "+e);
            return;
        }
        ...
        if(!adaAntrean || !masihMenunggu){
            hentikanAutoRefreshPantauFingerIGD();
        }
    }

    private String nilaiModelIGD(int baris, int kolom){
        try{
    ...
    private void ambilFotoKTP(){
        bukaModulFotoKTP("kamera", "kamera Foto KTP");
    }

    private void unggahFotoKTP(){
        bukaUploadFotoKTP("unggah", "unggah hasil scan Foto KTP");
    }

    private void bukaModulFotoKTP(String mode, String namaAksi){
        ...
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        try{
            Valid.panggilUrlcamera("photoktp/login.php?act=login&mode="+mode+"&usere="+koneksiDB.USERHYBRIDWEB()+
                    "&passwordte="+koneksiDB.PASHYBRIDWEB()+"&norm="+TNoRM.getText().trim());
        }catch(Exception e){
            JOptionPane.showMessageDialog(rootPane, "Gagal membuka "+namaAksi+" : "+e.getMessage());
        }finally{
            this.setCursor(Cursor.getDefaultCursor());
        }
    }

    private void bukaUploadFotoKTP(String mode, String namaAksi){
        if(TNoRM.getText().trim().equals("") || TPasien.getText().trim().equals("")){
            Valid.textKosong(TNoRM, "pasien");
            return;
        }
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        try{
            Valid.panggilUrl("photoktp/login.php?act=login&mode="+mode+"&usere="+koneksiDB.USERHYBRIDWEB()+
                    "&passwordte="+koneksiDB.PASHYBRIDWEB()+"&norm="+TNoRM.getText().trim());
        }catch(Exception e){
            JOptionPane.showMessageDialog(rootPane, "Gagal membuka "+namaAksi+" : "+e.getMessage());
        }finally{
    ...
    private void pasangPemantauNoRMFotoKTP(){
        timerMuatFotoKTP = new Timer(DELAY_MUAT_FOTO_KTP, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                panggilFotoKTP();
            }
        });
        timerMuatFotoKTP.setRepeats(false);

        TNoRM.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            private void jadwalkanMuatFotoKTP(){
                aturTombolFotoKTP();
                if(timerMuatFotoKTP != null){
                    timerMuatFotoKTP.restart();
                }
            }
            @Override public void insertUpdate(DocumentEvent e) { jadwalkanMuatFotoKTP(); }
            @Override public void removeUpdate(DocumentEvent e) { jadwalkanMuatFotoKTP(); }
            @Override public void changedUpdate(DocumentEvent e) { jadwalkanMuatFotoKTP(); }
        });
    }

    private void panggilFotoKTPDariTabel(){
        int baris = tbPetugas.getSelectedRow();
        if(baris != -1 && tbPetugas.getColumnCount() > 7){
            Object noRM = tbPetugas.getValueAt(baris, 7);
            panggilFotoKTP(noRM == null ? "" : noRM.toString());
        }
    }

    private void panggilFotoKTP(){
        panggilFotoKTP(TNoRM.getText(), false);
    }

    private void panggilFotoKTP(String nomorRM){
        panggilFotoKTP(nomorRM, false);
    }

    private void panggilFotoKTP(String nomorRM, boolean paksaMuat){
        final String noRM = nomorRM == null ? "" : nomorRM.trim();
        if(noRM.equals("") || !noRM.matches("[A-Za-z0-9._-]{1,20}")){
            batalkanPemuatanFotoKTP();
            noRMFotoKTPTerakhir = "";
            noRMFotoKTPDiminta = "";
            tampilkanFotoKTPKosong();
        ...
        if(!paksaMuat && noRM.equals(noRMFotoKTPTerakhir)){
            return;
        }
        if(!paksaMuat && noRM.equals(noRMFotoKTPDiminta)){
            return;
        }

        batalkanPemuatanFotoKTP();
        tampilkanFotoKTPKosong();
        noRMFotoKTPDiminta = noRM;
        final long nomorProses = ++nomorProsesFotoKTP;
        LblFotoKTP.setToolTipText("Memuat thumbnail Foto KTP pasien ...");

        workerMuatFotoKTP = new SwingWorker<BufferedImage, Void>() {
            @Override
            protected BufferedImage doInBackground() throws Exception {
                return unduhFotoKTP(noRM, true);
            }

            @Override
            protected void done() {
                if(nomorProses != nomorProsesFotoKTP || isCancelled()){
                    return;
                }
                noRMFotoKTPDiminta = "";
                noRMFotoKTPTerakhir = noRM;
                try{
                    BufferedImage thumbnail = get();
                    if(thumbnail != null){
                        fotoKTPAsli = null;
                        noRMFotoKTP = noRM;
                        fotoKTPTersediaTerakhir = true;
                        tampilkanThumbnailFotoKTP(thumbnail);
                    }else{
                        fotoKTPTersediaTerakhir = false;
                        tampilkanFotoKTPKosong();
                    }
                }catch(Exception e){
                    fotoKTPTersediaTerakhir = false;
        ...
        workerMuatFotoKTP.execute();
    }

    private void batalkanPemuatanFotoKTP(){
        nomorProsesFotoKTP++;
        if(workerMuatFotoKTP != null && !workerMuatFotoKTP.isDone()){
            workerMuatFotoKTP.cancel(true);
        }
        if(workerMuatFotoKTPBesar != null && !workerMuatFotoKTPBesar.isDone()){
            workerMuatFotoKTPBesar.cancel(true);
        }
        if(workerCetakFotoKTP != null && !workerCetakFotoKTP.isDone()){
            workerCetakFotoKTP.cancel(true);
        }
    }

    private void aturTombolFotoKTP(){
        boolean adaPasien = TNoRM != null && !TNoRM.getText().trim().equals("");
        if(BtnAmbilFotoKTP != null) BtnAmbilFotoKTP.setEnabled(adaPasien);
        if(BtnUnggahFotoKTP != null) BtnUnggahFotoKTP.setEnabled(adaPasien);
        if(BtnRefreshFotoKTP != null) BtnRefreshFotoKTP.setEnabled(adaPasien);
    }


    private BufferedImage unduhFotoKTP(String noRM, boolean thumbnail){
        String[] namaFile;
        if(thumbnail){
            namaFile = new String[]{noRM+"_thumb.jpg", noRM+"_thumb.jpeg", noRM+".jpg", noRM+".jpeg"};
        }else{
            namaFile = new String[]{noRM+".jpg", noRM+".jpeg"};
        }
        for(String nama : namaFile){
            if(Thread.currentThread().isInterrupted()){
                return null;
            }
            BufferedImage foto = unduhSatuFotoKTP(noRM, nama);
            if(foto != null){
                return foto;
            }
        }
    ...
    private BufferedImage unduhSatuFotoKTP(String noRM, String namaFile){
        HttpURLConnection koneksiFoto = null;
        InputStream inputFoto = null;
        try{
            String alamatFoto = "http://"+koneksiDB.HOSTHYBRIDWEB()+":"+
                    koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+
                    "/photoktp/pages/upload/"+namaFile+"?v="+System.currentTimeMillis();
            koneksiFoto = (HttpURLConnection)new URL(alamatFoto).openConnection();
            koneksiFoto.setRequestMethod("GET");
            koneksiFoto.setConnectTimeout(TIMEOUT_KONEKSI_FOTO_KTP);
            koneksiFoto.setReadTimeout(TIMEOUT_BACA_FOTO_KTP);
            koneksiFoto.setUseCaches(false);
            koneksiFoto.setRequestProperty("Cache-Control", "no-cache");

            if(koneksiFoto.getResponseCode() != HttpURLConnection.HTTP_OK){
                return null;
            }
            String tipe = koneksiFoto.getContentType();
            if(tipe != null && tipe.toLowerCase().contains("text/html")){
                return null;
            }
            inputFoto = koneksiFoto.getInputStream();
            return ImageIO.read(inputFoto);
        }catch(Exception e){
            // Foto belum tersedia atau layanan Hybrid Web sedang tidak dapat diakses.
            // Tidak menampilkan notifikasi agar alur registrasi tetap lancar.
            return null;
        }finally{
            try{
                if(inputFoto != null){
            ...
            if(koneksiFoto != null){
                koneksiFoto.disconnect();
            }
        }
    }

    private void tampilkanThumbnailFotoKTP(BufferedImage thumbnail){
        LblFotoKTP.setText("");
        LblFotoKTP.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
        LblFotoKTP.setVerticalTextPosition(javax.swing.SwingConstants.CENTER);
        LblFotoKTP.setIcon(new ImageIcon(skalaFotoKTP(thumbnail, FOTO_KTP_LEBAR, FOTO_KTP_TINGGI)));
        LblFotoKTP.setToolTipText("Foto KTP pasien. Gunakan tombol Zoom untuk melihat lebih jelas atau Unduh untuk menyimpan dokumen.");
        BtnZoomFotoKTP.setEnabled(true);
        if(BtnUnduhFotoKTP != null){
            BtnUnduhFotoKTP.setEnabled(true);
            BtnUnduhFotoKTP.setToolTipText("Cetak Foto KTP pada kertas F4/Folio");
        }
        aturTombolFotoKTP();
    }

    private BufferedImage skalaFotoKTP(BufferedImage sumber){
        return skalaFotoKTP(sumber, FOTO_KTP_LEBAR, FOTO_KTP_TINGGI);
    }

    private BufferedImage skalaFotoKTP(BufferedImage sumber, int lebarTarget, int tinggiTarget){
        BufferedImage hasil = new BufferedImage(lebarTarget, tinggiTarget, BufferedImage.TYPE_INT_RGB);
        Graphics2D g2 = hasil.createGraphics();
        try{
            g2.setColor(Color.WHITE);
            g2.fillRect(0, 0, lebarTarget, tinggiTarget);
            g2.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
            g2.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);

            double faktor = Math.min((double)lebarTarget / sumber.getWidth(),
                    (double)tinggiTarget / sumber.getHeight());
            int lebar = (int)Math.round(sumber.getWidth() * faktor);
            int tinggi = (int)Math.round(sumber.getHeight() * faktor);
            int xGambar = (lebarTarget - lebar) / 2;
            int yGambar = (tinggiTarget - tinggi) / 2;
            g2.drawImage(sumber, xGambar, yGambar, lebar, tinggi, null);
        }finally{
            g2.dispose();
        }
        return hasil;
    ...
    private ImageIcon buatIkonKameraKTP(){
        if(ikonKameraKTP != null){
            return ikonKameraKTP;
        }
        BufferedImage gambar = new BufferedImage(54, 40, BufferedImage.TYPE_INT_ARGB);
        Graphics2D g2 = gambar.createGraphics();
        try{
            g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            g2.setColor(new Color(95, 95, 95));
            g2.fillRoundRect(4, 10, 46, 27, 5, 5);
            g2.fillRoundRect(17, 5, 19, 8, 3, 3);
            g2.setColor(Color.WHITE);
            g2.fillOval(19, 15, 16, 16);
            g2.setColor(new Color(95, 95, 95));
            g2.fillOval(22, 18, 10, 10);
        }finally{
            g2.dispose();
        }
        ikonKameraKTP = new ImageIcon(gambar);
        return ikonKameraKTP;
    }

    private ImageIcon buatIkonZoomKTP(){
        if(ikonZoomKTP != null){
            return ikonZoomKTP;
        }
        URL lokasiIkon = getClass().getResource("/picture/zoom-in.png");
        if(lokasiIkon != null){
            ikonZoomKTP = new ImageIcon(lokasiIkon);
        }else{
            // Tombol tetap dapat dipakai; periksa folder source/picture bila icon kosong.
            ikonZoomKTP = new ImageIcon();
            System.out.println("Notif Foto KTP : File icon /picture/zoom-in.png tidak ditemukan.");
        }
        return ikonZoomKTP;
    }

    private void tampilkanFotoKTPKosong(){
        fotoKTPAsli = null;
        noRMFotoKTP = "";
        LblFotoKTP.setIcon(buatIkonKameraKTP());
        LblFotoKTP.setText("<html><center>Unggah / Ambil Foto KTP<br>Pasien</center></html>");
        LblFotoKTP.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
        LblFotoKTP.setVerticalTextPosition(javax.swing.SwingConstants.BOTTOM);
        LblFotoKTP.setToolTipText("Foto KTP belum tersedia. Gunakan tombol Kamera atau Unggah di sebelah kiri.");
        if(BtnZoomFotoKTP != null){
            BtnZoomFotoKTP.setEnabled(false);
        }
        if(BtnUnduhFotoKTP != null){
            BtnUnduhFotoKTP.setEnabled(false);
    ...
    private void tampilkanFotoKTPBesar(){
        final String noRM = noRMFotoKTP;
        if(noRM.equals("")){
            return;
        }
        if(fotoKTPAsli != null){
            bukaDialogFotoKTPBesar(fotoKTPAsli);
            return;
        }
        if(workerMuatFotoKTPBesar != null && !workerMuatFotoKTPBesar.isDone()){
            return;
        ...
        BtnZoomFotoKTP.setEnabled(false);
        BtnZoomFotoKTP.setToolTipText("Memuat foto KTP ukuran penuh ...");
        workerMuatFotoKTPBesar = new SwingWorker<BufferedImage, Void>() {
            @Override
            protected BufferedImage doInBackground() throws Exception {
                return unduhFotoKTP(noRM, false);
            }

            @Override
            protected void done() {
                if(!noRM.equals(noRMFotoKTP)){
                    return;
                }
                try{
                    BufferedImage foto = get();
                    if(foto != null){
                        fotoKTPAsli = foto;
                        BtnZoomFotoKTP.setEnabled(true);
                        BtnZoomFotoKTP.setToolTipText("Perbesar Foto KTP Pasien");
                        bukaDialogFotoKTPBesar(foto);
                    }else{
                        BtnZoomFotoKTP.setEnabled(fotoKTPTersediaTerakhir);
                        BtnZoomFotoKTP.setToolTipText("Foto ukuran penuh tidak ditemukan. Silakan Muat Ulang bila foto baru saja diunggah.");
                    }
                }catch(Exception e){
                    BtnZoomFotoKTP.setEnabled(fotoKTPTersediaTerakhir);
                    BtnZoomFotoKTP.setToolTipText("Gagal memuat foto ukuran penuh. Coba lagi.");
                }
            }
        };
        workerMuatFotoKTPBesar.execute();
    }

    private void bukaDialogFotoKTPBesar(final BufferedImage gambar){
        if(gambar == null){
            return;
        }

        final javax.swing.JDialog lihatFoto = new javax.swing.JDialog(this, "Zoom Foto KTP Pasien", true);
        final javax.swing.JLabel labelFoto = new javax.swing.JLabel();
        ...
        labelFoto.setBorder(javax.swing.BorderFactory.createCompoundBorder(
                javax.swing.BorderFactory.createLineBorder(new Color(120,120,120)),
                javax.swing.BorderFactory.createEmptyBorder(10,10,10,10)));

        final javax.swing.JScrollPane scroll = new javax.swing.JScrollPane(labelFoto);
        scroll.getViewport().setBackground(new Color(245,245,245));
        scroll.setBorder(javax.swing.BorderFactory.createEmptyBorder(6,6,6,6));

        final javax.swing.JSlider sliderZoom = new javax.swing.JSlider(25, 600, 100);
        sliderZoom.setMajorTickSpacing(25);
        sliderZoom.setMinorTickSpacing(5);
        sliderZoom.setPaintTicks(false);
        sliderZoom.setFocusable(false);

        final javax.swing.JLabel lblInfoZoom = new javax.swing.JLabel(" ");
        lblInfoZoom.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);

        final Runnable refreshZoom = new Runnable(){
            @Override
            public void run() {
                int persen = Math.max(25, Math.min(600, sliderZoom.getValue()));
                int lebar = Math.max(1, (int)Math.round(gambar.getWidth() * (persen / 100.0)));
                int tinggi = Math.max(1, (int)Math.round(gambar.getHeight() * (persen / 100.0)));
                BufferedImage skala = skalaFotoKTP(gambar, lebar, tinggi);
                labelFoto.setIcon(new ImageIcon(skala));
                labelFoto.setPreferredSize(new Dimension(lebar + 20, tinggi + 20));
                lblInfoZoom.setText("Zoom : " + persen + "%    Ukuran : " + lebar + " x " + tinggi + " px");
                labelFoto.revalidate();
            }
        };

        javax.swing.JButton btnKurang = new javax.swing.JButton("-");
        btnKurang.setFocusable(false);
        btnKurang.setMargin(new java.awt.Insets(2, 10, 2, 10));
        btnKurang.addActionListener(new java.awt.event.ActionListener(){
            @Override
            public void actionPerformed(java.awt.event.ActionEvent e) {
                sliderZoom.setValue(Math.max(sliderZoom.getMinimum(), sliderZoom.getValue() - 10));
            }
        });
        ...
        btnTambah.addActionListener(new java.awt.event.ActionListener(){
            @Override
            public void actionPerformed(java.awt.event.ActionEvent e) {
                sliderZoom.setValue(Math.min(sliderZoom.getMaximum(), sliderZoom.getValue() + 10));
            }
        });
        ...
        scroll.addMouseWheelListener(new java.awt.event.MouseWheelListener(){
            ...
            public void mouseWheelMoved(java.awt.event.MouseWheelEvent e) {
                if(e.isControlDown()){
                    int nilai = sliderZoom.getValue() - (e.getWheelRotation() * 10);
                    sliderZoom.setValue(Math.max(sliderZoom.getMinimum(), Math.min(sliderZoom.getMaximum(), nilai)));
                    e.consume();
                }
            }
        });

        javax.swing.JPanel panelKontrol = new javax.swing.JPanel(new java.awt.BorderLayout(8, 0));
        panelKontrol.setBorder(javax.swing.BorderFactory.createEmptyBorder(8, 8, 8, 8));
        javax.swing.JPanel panelKiri = new javax.swing.JPanel(new java.awt.FlowLayout(java.awt.FlowLayout.LEFT, 6, 0));
        panelKiri.add(new javax.swing.JLabel("Perbesar / Perkecil"));
        panelKiri.add(btnKurang);
        panelKiri.add(sliderZoom);
        panelKiri.add(btnTambah);
        panelKiri.add(btnReset);
        panelKontrol.add(panelKiri, java.awt.BorderLayout.WEST);
        panelKontrol.add(lblInfoZoom, java.awt.BorderLayout.CENTER);

        lihatFoto.getContentPane().setLayout(new java.awt.BorderLayout());
        lihatFoto.getContentPane().add(scroll, java.awt.BorderLayout.CENTER);
        lihatFoto.getContentPane().add(panelKontrol, java.awt.BorderLayout.SOUTH);

        java.awt.Dimension layar = java.awt.Toolkit.getDefaultToolkit().getScreenSize();
        int lebarDialog = Math.min((int)(layar.width * 0.92), 1220);
        int tinggiDialog = Math.min((int)(layar.height * 0.88), 860);
        lihatFoto.setSize(lebarDialog, tinggiDialog);
        lihatFoto.setMinimumSize(new java.awt.Dimension(820, 620));
        lihatFoto.setLocationRelativeTo(this);
        refreshZoom.run();
        lihatFoto.setVisible(true);
    }

    private void cetakFotoKTP(){
        final String noRM = noRMFotoKTP == null ? "" : noRMFotoKTP.trim();
        if(noRM.equals("")){
            JOptionPane.showMessageDialog(rootPane, "Foto KTP pasien belum tersedia untuk dicetak.", "Cetak Foto KTP", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int konfirmasi = JOptionPane.showConfirmDialog(rootPane,
                "Cetak Foto KTP pasien No. RM " + noRM + " pada kertas F4/Folio?\n"
                        + "Ukuran foto 85,60 mm x 53,98 mm, margin kiri 6 cm, margin atas 2,5 cm.",
                "Konfirmasi Cetak Foto KTP", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
        if(konfirmasi != JOptionPane.YES_OPTION){
            return;
        }

        if(fotoKTPAsli != null && noRM.equals(noRMFotoKTP)){
            bukaDialogCetakFotoKTP(fotoKTPAsli);
            return;
        }
        ...
        BtnUnduhFotoKTP.setEnabled(false);
        BtnUnduhFotoKTP.setToolTipText("Memuat Foto KTP ukuran penuh untuk dicetak ...");
        workerCetakFotoKTP = new SwingWorker<BufferedImage, Void>() {
            @Override
            protected BufferedImage doInBackground() throws Exception {
                return unduhFotoKTP(noRM, false);
            }
            ...
            protected void done(){
                try{
                    BufferedImage gambar = get();
                    if(gambar == null){
                        JOptionPane.showMessageDialog(rootPane,
                                "Foto KTP ukuran penuh tidak ditemukan. Silakan klik Muat Ulang Foto KTP.",
                                "Cetak Foto KTP", JOptionPane.WARNING_MESSAGE);
                    }else{
                        fotoKTPAsli = gambar;
                        bukaDialogCetakFotoKTP(gambar);
                    }
                }catch(Exception e){
                    JOptionPane.showMessageDialog(rootPane,
                            "Foto KTP gagal dimuat untuk dicetak. Periksa koneksi Hybrid Web atau coba Muat Ulang Foto KTP.\n"
                                    + (e.getMessage() == null ? "" : e.getMessage()),
                            "Cetak Foto KTP", JOptionPane.WARNING_MESSAGE);
                }finally{
                    if(BtnUnduhFotoKTP != null){
                        BtnUnduhFotoKTP.setEnabled(fotoKTPTersediaTerakhir && !noRMFotoKTP.trim().equals(""));
                        BtnUnduhFotoKTP.setToolTipText("Cetak Foto KTP pada kertas F4/Folio");
                    }
                }
            }
        };
        workerCetakFotoKTP.execute();
    }

    private void bukaDialogCetakFotoKTP(final BufferedImage gambar){
        final double pointPerMm = 72d / 25.4d;
        final double lebarF4 = 210d * pointPerMm;
        final double tinggiF4 = 330d * pointPerMm;
        final double marginKiri = 60d * pointPerMm;
        final double marginAtas = 25d * pointPerMm;
        final double lebarKTP = 85.60d * pointPerMm;
        final double tinggiKTP = 53.98d * pointPerMm;

        PrinterJob pekerjaanCetak = PrinterJob.getPrinterJob();
        PageFormat formatHalaman = pekerjaanCetak.defaultPage();
        Paper kertas = new Paper();
        kertas.setSize(lebarF4, tinggiF4);
        // Posisi gambar memakai koordinat fisik halaman. Margin yang diminta
        // berada jauh dari area tidak tercetak printer pada umumnya.
        kertas.setImageableArea(0d, 0d, lebarF4, tinggiF4);
        formatHalaman.setPaper(kertas);
        formatHalaman.setOrientation(PageFormat.PORTRAIT);

        pekerjaanCetak.setPrintable(new Printable() {
            @Override
            public int print(Graphics graphics, PageFormat pageFormat, int nomorHalaman) throws PrinterException {
                if(nomorHalaman > 0){
                ...
                try{
                    ...
                    g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                    g2.drawImage(gambar,
                            (int)Math.round(marginKiri),
                            (int)Math.round(marginAtas),
                            (int)Math.round(lebarKTP),
                            (int)Math.round(tinggiKTP), null);
                }finally{
                    g2.dispose();
                }
        ...
        if(pekerjaanCetak.printDialog()){
            try{
                pekerjaanCetak.print();
            }catch(PrinterException e){
                JOptionPane.showMessageDialog(rootPane,
                        "Foto KTP gagal dicetak: " + e.getMessage(),
                        "Cetak Foto KTP", JOptionPane.WARNING_MESSAGE);
            }
        }
    }
...
//        MnRMCatatanMonitoring.add(MnCatatanPersalinan);
    ...
    private void initPopupVisumEtRepertum() {
        javax.swing.JMenuItem mnVisumEtRepertum = new javax.swing.JMenuItem();
        mnVisumEtRepertum.setBackground(new java.awt.Color(255, 255, 254));
        mnVisumEtRepertum.setFont(new java.awt.Font("Tahoma", 1, 11));
        mnVisumEtRepertum.setForeground(new java.awt.Color(50, 50, 50));
        mnVisumEtRepertum.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png")));
        mnVisumEtRepertum.setText("Visum Et Repertum");
        mnVisumEtRepertum.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        mnVisumEtRepertum.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        mnVisumEtRepertum.setPreferredSize(new java.awt.Dimension(250, 26));
        mnVisumEtRepertum.addActionListener(new java.awt.event.ActionListener() {
            @Override
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                bukaVisumEtRepertum();
            }
        });
        jPopupMenu1.add(mnVisumEtRepertum);
    }

    private void bukaVisumEtRepertum() {
        if(tabMode.getRowCount()==0){
            JOptionPane.showMessageDialog(null,"Maaf, data pasien sudah habis...!!!!");
            TNoRw.requestFocus();
        }else if(TPasien.getText().trim().equals("")){
            ...
            if(tbPetugas.getSelectedRow()!= -1){
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                SuratVisum form=new SuratVisum(null,false);
                form.isCek();
                form.emptTeks();
                form.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
                form.setLocationRelativeTo(internalFrame1);
                form.setNoRm(TNoRw.getText(),TNoRM.getText(),TPasien.getText(),KdDokter.getText(),TDokter.getText(),DTPCari1.getDate(),DTPCari2.getDate());
                form.tampil();
                form.setVisible(true);
                this.setCursor(Cursor.getDefaultCursor());
            }
    ...
    private void isiDataRujukanSatuSehat(SatuSehatRujukanForm dlgki) {
           String noRawat=TNoRw.getText().trim();
        ...
        if(namaPasien.equals("")){
            namaPasien=TPasien.getText().trim();
        }
        String nikPasien=Sequel.cariIsi(
                "select pasien.no_ktp from reg_periksa inner join pasien on pasien.no_rkm_medis=reg_periksa.no_rkm_medis where reg_periksa.no_rawat=?",
                noRawat);
        String nikDokter=Sequel.cariIsi(
                "select pegawai.no_ktp from reg_periksa inner join pegawai on pegawai.nik=reg_periksa.kd_dokter where reg_periksa.no_rawat=?",
                noRawat);
        String idPasien="";
        String idPraktisi="";
        ...
        if(!nikDokter.equals("")){
            idPraktisi=cekViaSatuSehat.tampilIDParktisi(nikDokter);
        }
        String idEncounter=Sequel.cariIsi("select id_encounter from satu_sehat_encounter where no_rawat=?",noRawat);
        String kodeDiagnosa=Sequel.cariIsi(
                ...
                noRawat);
        String namaDiagnosa=Sequel.cariIsi(
                "select penyakit.nm_penyakit from diagnosa_pasien inner join penyakit on penyakit.kd_penyakit=diagnosa_pasien.kd_penyakit where diagnosa_pasien.status='Ralan' and diagnosa_pasien.prioritas='1' and diagnosa_pasien.no_rawat=?",
                noRawat);
        String idOrganisasi=Sequel.cariIsi(
                "select satu_sehat_mapping_lokasi_ralan.id_organisasi_satusehat from reg_periksa inner join satu_sehat_mapping_lokasi_ralan on satu_sehat_mapping_lokasi_ralan.kd_poli=reg_periksa.kd_poli where reg_periksa.no_rawat=?",
                noRawat);
        String namaOrganisasi=Sequel.cariIsi(
                "select departemen.nama from reg_periksa inner join satu_sehat_mapping_lokasi_ralan on satu_sehat_mapping_lokasi_ralan.kd_poli=reg_periksa.kd_poli inner join satu_sehat_mapping_departemen on satu_sehat_mapping_departemen.id_organisasi_satusehat=satu_sehat_mapping_lokasi_ralan.id_organisasi_satusehat inner join departemen on departemen.dep_id=satu_sehat_mapping_departemen.dep_id where reg_periksa.no_rawat=?",
                noRawat);
        if(namaOrganisasi.equals("")){
            namaOrganisasi=akses.getnamars();
        }
        String tanggalRawat=Valid.SetTgl(DTPReg.getSelectedItem()+"");
        String jamRawat=CmbJam.getSelectedItem()+":"+CmbMenit.getSelectedItem()+":"+CmbDetik.getSelectedItem();
        String rencanaCarePlan=Sequel.cariIsi(
                "select pemeriksaan_ralan.rtl from pemeriksaan_ralan where pemeriksaan_ralan.no_rawat=? and pemeriksaan_ralan.rtl<>'' order by pemeriksaan_ralan.tgl_perawatan desc,pemeriksaan_ralan.jam_rawat desc limit 1",
                noRawat);
        if(rencanaCarePlan.equals("")){
            rencanaCarePlan="Rujukan IGD untuk pemeriksaan dan penanganan lebih lanjut";
        }
        String idCarePlan=Sequel.cariIsi(
                "select ifnull(satu_sehat_careplan.id_careplan,'') from pemeriksaan_ralan "+
                ...
                "and satu_sehat_careplan.status='Ralan' "+
                "where pemeriksaan_ralan.no_rawat=? and pemeriksaan_ralan.rtl<>'' and ifnull(satu_sehat_careplan.id_careplan,'')<>'' "+
                "order by pemeriksaan_ralan.tgl_perawatan desc,pemeriksaan_ralan.jam_rawat desc limit 1",
                noRawat);
        String waktu=tanggalRawat+"T"+jamRawat+"+08:00";
        String dataPendukung=idEncounter.equals("") ? "" : "Encounter/"+idEncounter;
        String kodeFaskesSatuSehat=akses.getkodeppkbpjs();
        dlgki.setDataRujukan(
