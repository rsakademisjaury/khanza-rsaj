...
public final class SuratPernyataanMemilihDPJP extends javax.swing.JDialog {
    ...
    /* RSAJ: Tanda tangan pembuat pernyataan direkam dari desktop/drawing pad.
       Komponen ditambahkan programatik agar layout NetBeans (.form) lama tidak diubah. */
    private javax.swing.JButton BtnTTDPembuatPernyataan;
    private javax.swing.JLabel LblTTDPembuatPernyataan;
    private BufferedImage gambarTTDPembuatPernyataan;
    private String pathTTDPembuatPernyataan = "";
    ...
    public SuratPernyataanMemilihDPJP(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        pasangFiturTTDPembuatPernyataan();
    ...
    /**
     * Menambahkan tombol tanda tangan tanpa mengubah komponen yang dibuat
     * otomatis oleh NetBeans pada SuratPernyataanMemilihDPJP.form.
     */
    private void pasangFiturTTDPembuatPernyataan(){
        BtnTTDPembuatPernyataan = new javax.swing.JButton("Tanda Tangan Pembuat Pernyataan");
        BtnTTDPembuatPernyataan.setFont(new java.awt.Font("Tahoma", 0, 11));
        BtnTTDPembuatPernyataan.setToolTipText("Rekam tanda tangan pembuat pernyataan memakai mouse, layar sentuh, atau drawing pad");
        BtnTTDPembuatPernyataan.addActionListener(new java.awt.event.ActionListener() {
            @Override
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                bukaTandaTanganPembuatPernyataan();
            }
        });

        LblTTDPembuatPernyataan = new javax.swing.JLabel("Belum ada tanda tangan pembuat pernyataan");
        LblTTDPembuatPernyataan.setFont(new java.awt.Font("Tahoma", 0, 11));
        LblTTDPembuatPernyataan.setForeground(new java.awt.Color(125,75,0));

        FormInput.add(BtnTTDPembuatPernyataan);
        BtnTTDPembuatPernyataan.setBounds(105, 198, 260, 28);
        FormInput.add(LblTTDPembuatPernyataan);
        LblTTDPembuatPernyataan.setBounds(378, 198, 465, 28);
        FormInput.setPreferredSize(new Dimension(870, 238));
        PanelInput.setPreferredSize(new Dimension(292, 260));
        FormInput.revalidate();
        FormInput.repaint();
    }

    private void bukaTandaTanganPembuatPernyataan(){
        BufferedImage hasil = TandaTanganPasienWaliDialog.ambil(this, gambarTTDPembuatPernyataan);
        if(hasil != null){
            gambarTTDPembuatPernyataan = hasil;
            pathTTDPembuatPernyataan = "";
            LblTTDPembuatPernyataan.setText("Tanda tangan baru direkam. Akan disimpan saat tombol Simpan/Ganti diklik.");
            LblTTDPembuatPernyataan.setForeground(new java.awt.Color(20,120,70));
        }
    }

    private void resetTTDPembuatPernyataan(){
        gambarTTDPembuatPernyataan = null;
        pathTTDPembuatPernyataan = "";
        if(LblTTDPembuatPernyataan != null){
            LblTTDPembuatPernyataan.setText("Belum ada tanda tangan pembuat pernyataan");
            LblTTDPembuatPernyataan.setForeground(new java.awt.Color(125,75,0));
        }
    }

    private void cekStatusTTDPembuatPernyataan(String noPernyataan){
        pathTTDPembuatPernyataan = "";
        try{
            ps = koneksi.prepareStatement("select ifnull(ttd_path,'') as ttd_path from ttd_surat_pernyataan_memilih_dpjp where no_pernyataan=?");
            ps.setString(1, noPernyataan);
            rs = ps.executeQuery();
            if(rs.next()){
                pathTTDPembuatPernyataan = rs.getString("ttd_path");
            }
        }catch(Exception e){
            /* SQL patch belum dipasang atau tabel belum tersedia. */
            pathTTDPembuatPernyataan = "";
        }finally{
        ...
        if(LblTTDPembuatPernyataan != null){
            if(pathTTDPembuatPernyataan != null && !pathTTDPembuatPernyataan.trim().equals("")){
                LblTTDPembuatPernyataan.setText("Tanda tangan pembuat pernyataan sudah tersimpan. Klik tombol untuk mengganti.");
                LblTTDPembuatPernyataan.setForeground(new java.awt.Color(20,120,70));
            }else{
                LblTTDPembuatPernyataan.setText("Belum ada tanda tangan pembuat pernyataan");
                LblTTDPembuatPernyataan.setForeground(new java.awt.Color(125,75,0));
    ...
    private boolean ttdPembuatSudahTersimpan(String noPernyataan){
        try{
            ps = koneksi.prepareStatement("select ifnull(ttd_path,'') as ttd_path from ttd_surat_pernyataan_memilih_dpjp where no_pernyataan=?");
            ps.setString(1, noPernyataan);
            rs = ps.executeQuery();
            return rs.next() && rs.getString("ttd_path") != null && !rs.getString("ttd_path").trim().equals("");
        }catch(Exception e){
            return false;
        }finally{
            try{ if(rs != null){ rs.close(); } }catch(Exception e){}
    ...
    private byte[] buatPNGTTD100KB(BufferedImage sumber) throws Exception{
        BufferedImage kerja = sumber;
        for(int percobaan=0; percobaan<6; percobaan++){
            ByteArrayOutputStream output = new ByteArrayOutputStream();
            javax.imageio.ImageIO.write(kerja, "png", output);
            if(output.size() <= 100*1024){
                return output.toByteArray();
            }
            int lebar = Math.max(120, (int)Math.round(kerja.getWidth()*0.72));
            int tinggi = Math.max(60, (int)Math.round(kerja.getHeight()*0.72));
            if(lebar == kerja.getWidth() && tinggi == kerja.getHeight()){
                break;
            }
            BufferedImage kecil = new BufferedImage(lebar, tinggi, BufferedImage.TYPE_INT_ARGB);
            Graphics2D g = kecil.createGraphics();
            g.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BICUBIC);
            g.drawImage(kerja, 0, 0, lebar, tinggi, null);
            g.dispose();
            kerja = kecil;
        }
        throw new Exception("Ukuran tanda tangan melebihi 100 KB.");
    }

    /**
     * Mengirim hasil drawing pad ke endpoint web DPJP. File ditulis oleh PHP
     * ke webapps/pernyataanmemilihdpjp/pages/upload agar desktop tidak perlu
     * mengetahui lokasi fisik folder web server.
     */
    private boolean kirimTTDPembuatPernyataan(String noPernyataan){
        if(gambarTTDPembuatPernyataan == null){
            return true;
        }
        HttpURLConnection koneksiHttp = null;
        BufferedReader pembaca = null;
        try{
            byte[] png = buatPNGTTD100KB(gambarTTDPembuatPernyataan);
            String gambarBase64 = Base64.getEncoder().encodeToString(png);
            String dataKirim = "no_pernyataan="+URLEncoder.encode(noPernyataan,"UTF-8")+
            ...
            String alamat = "http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+
                    koneksiDB.HYBRIDWEB()+"/pernyataanmemilihdpjp/pages/simpanTTDPembuatDPJP.php";
            ...
            if(respons.toString().indexOf("\"success\":true") >= 0){
                gambarTTDPembuatPernyataan = null;
                pathTTDPembuatPernyataan = "tersimpan";
                LblTTDPembuatPernyataan.setText("Tanda tangan pembuat pernyataan sudah tersimpan.");
                LblTTDPembuatPernyataan.setForeground(new java.awt.Color(20,120,70));
                return true;
            }
            ...
            System.out.println("Notif TTD Pembuat DPJP : "+e);
            JOptionPane.showMessageDialog(rootPane,"Tanda tangan belum dapat dikirim ke webapps Memilih DPJP. Pastikan endpoint simpanTTDPembuatDPJP.php sudah dipasang dan server web dapat diakses.");
            return false;
        }finally{
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TNoRM.getText().trim().equals("")){
        ...
        }else if(gambarTTDPembuatPernyataan==null && (pathTTDPembuatPernyataan==null || pathTTDPembuatPernyataan.trim().equals(""))){
            JOptionPane.showMessageDialog(rootPane,"Tanda tangan pembuat pernyataan belum direkam. Klik tombol Tanda Tangan Pembuat Pernyataan terlebih dahulu.");
        }else{
            if(Sequel.menyimpantf("surat_pernyataan_memilih_dpjp","?,?,?,?,?,?,?,?,?,?,?","Data",11,new String[]{
                ...
                if(kirimTTDPembuatPernyataan(NoPenyataan.getText())){
                    emptTeks();
                }
            }
        }
    ...
    private void btnAmbilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAmbilActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            if(tbObat.getSelectedRow()>-1){
                String noPernyataanDipilih = tbObat.getValueAt(tbObat.getSelectedRow(),0).toString();
                if(!ttdPembuatSudahTersimpan(noPernyataanDipilih)){
                    JOptionPane.showMessageDialog(rootPane,"Tanda tangan pembuat pernyataan belum tersimpan. Pilih data ini, klik Tanda Tangan Pembuat Pernyataan, lalu klik Ganti terlebih dahulu.");
                    return;
                }
                Sequel.queryu("insert into antripernyataanmemilihdpjp values('"+noPernyataanDipilih+"','"+tbObat.getValueAt(tbObat.getSelectedRow(),1).toString()+"')");
                Sequel.queryu("delete from bukti_surat_pernyataan_memilih_dpjp where no_pernyataan='"+noPernyataanDipilih+"'");
    ...
    */
    ...
    private void getData() {
        if(tbObat.getSelectedRow()!= -1){
            ...
            gambarTTDPembuatPernyataan = null;
            cekStatusTTDPembuatPernyataan(NoPenyataan.getText());
        }
    }
