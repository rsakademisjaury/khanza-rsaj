...
public final class SuratPersetujuanUmum extends javax.swing.JDialog {
    ...
    /* Fitur baru RSAJ: tanda tangan pasien/wali direkam dari drawing pad desktop.
       Komponen ditambahkan secara programatik agar form NetBeans lama tidak berubah. */
    private javax.swing.JButton BtnTTDPasienWali;
    private javax.swing.JLabel LblTTDPasienWali;
    private BufferedImage gambarTTDPasienWali;
    private String pathTTDPasienWali="";
    
    public SuratPersetujuanUmum(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        pasangFiturTTDPasienWali();
        this.setLocation(8,1);
        setSize(628,724);
        
        tabMode=new DefaultTableModel(null,new Object[]{
            "No.Persetujuan","No.Rawat","No.R.M.","Nama Pasien","Umur","J.K.","Tgl.Lahir","Tanggal","Pengobatan Kepada","Nilai Kepercayaan",
            "Nama Penanggung Jawab","Umur P.J.","Nomor KTP P.J.","J.K. P.J.","Nomor Telp/HP","Bertindak Untuk","NIP","Nama Petugas","Alamat P.J./Wali"
        }){
    ...
    /**
     * Menambahkan tombol pembubuhan tanda tangan tanpa mengubah komponen
     * bawaan yang dibuat oleh NetBeans pada SuratPersetujuanUmum.form.
     */
    private void pasangFiturTTDPasienWali(){
        BtnTTDPasienWali = new javax.swing.JButton("Tanda Tangan Pasien/Wali");
        BtnTTDPasienWali.setFont(new java.awt.Font("Tahoma",0,11));
        BtnTTDPasienWali.setToolTipText("Klik untuk merekam tanda tangan pasien/wali menggunakan mouse, touch screen, atau drawing pad");
        BtnTTDPasienWali.addActionListener(new java.awt.event.ActionListener() {
            @Override
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                bukaTandaTanganPasienWali();
            }
        });

        LblTTDPasienWali = new javax.swing.JLabel("Belum ada tanda tangan pasien/wali");
        LblTTDPasienWali.setFont(new java.awt.Font("Tahoma",0,11));
        LblTTDPasienWali.setForeground(new java.awt.Color(125,75,0));

        FormInput.add(BtnTTDPasienWali);
        BtnTTDPasienWali.setBounds(114,180,215,28);
        FormInput.add(LblTTDPasienWali);
        LblTTDPasienWali.setBounds(340,180,390,28);

        FormInput.setPreferredSize(new Dimension(100,230));
        PanelInput.setPreferredSize(new Dimension(192,240));
        FormInput.revalidate();
        FormInput.repaint();
    }

    private void bukaTandaTanganPasienWali(){
        BufferedImage hasil = TandaTanganPasienWaliDialog.ambil(this, gambarTTDPasienWali);
        if(hasil != null){
            gambarTTDPasienWali = hasil;
            pathTTDPasienWali = "";
            LblTTDPasienWali.setText("Tanda tangan baru direkam. Akan disimpan saat tombol Simpan/Ganti diklik.");
            LblTTDPasienWali.setForeground(new java.awt.Color(20,120,70));
        }
    }

    private void resetTandaTanganPasienWali(){
        gambarTTDPasienWali = null;
        pathTTDPasienWali = "";
        if(LblTTDPasienWali != null){
            LblTTDPasienWali.setText("Belum ada tanda tangan pasien/wali");
            LblTTDPasienWali.setForeground(new java.awt.Color(125,75,0));
        }
    }

    private void cekStatusTandaTanganPasienWali(String noSurat){
        if(LblTTDPasienWali == null){
            return;
        }
        pathTTDPasienWali = "";
        try{
            ps=koneksi.prepareStatement("select ifnull(ttd_pasien_wali,'') as ttd_pasien_wali from surat_persetujuan_umum where no_surat=?");
            ps.setString(1,noSurat);
            rs=ps.executeQuery();
            if(rs.next()){
                pathTTDPasienWali = rs.getString("ttd_pasien_wali");
            }
        }catch(Exception e){
            /* Kolom belum ada sebelum SQL patch dijalankan. */
            pathTTDPasienWali = "";
        }finally{
        ...
        if(pathTTDPasienWali != null && !pathTTDPasienWali.trim().equals("")){
            LblTTDPasienWali.setText("Tanda tangan pasien/wali sudah tersimpan. Klik tombol untuk mengganti.");
            LblTTDPasienWali.setForeground(new java.awt.Color(20,120,70));
        }else{
            LblTTDPasienWali.setText("Belum ada tanda tangan pasien/wali");
            LblTTDPasienWali.setForeground(new java.awt.Color(125,75,0));
    ...
    /**
     * Mengirim hasil drawing pad ke endpoint PHP. Endpoint yang menyimpan file
     * berada pada webapps/persetujuanumum/pages/upload sehingga Java tidak
     * perlu mengetahui path fisik server web.
     */
    private boolean kirimTandaTanganPasienWali(String noSurat){
        if(gambarTTDPasienWali == null){
            return true;
        }
        HttpURLConnection koneksiHttp = null;
        BufferedReader pembaca = null;
        try{
            byte[] bytesTandaTangan = konversiTandaTanganMaks100KB(gambarTTDPasienWali);
            String gambarBase64 = Base64.getEncoder().encodeToString(bytesTandaTangan);
            String dataKirim = "no_surat="+URLEncoder.encode(noSurat,"UTF-8")+
                    "&ttd="+URLEncoder.encode("data:image/png;base64,"+gambarBase64,"UTF-8");
            String alamat = "http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+
                    koneksiDB.HYBRIDWEB()+"/persetujuanumum/pages/simpanTTDPasienWali.php";
            koneksiHttp = (HttpURLConnection)new URL(alamat).openConnection();
            koneksiHttp.setConnectTimeout(15000);
            koneksiHttp.setReadTimeout(30000);
            koneksiHttp.setRequestMethod("POST");
            koneksiHttp.setDoOutput(true);
            koneksiHttp.setRequestProperty("Content-Type","application/x-www-form-urlencoded; charset=UTF-8");
            koneksiHttp.getOutputStream().write(dataKirim.getBytes("UTF-8"));
            koneksiHttp.getOutputStream().flush();
            koneksiHttp.getOutputStream().close();

            int kode = koneksiHttp.getResponseCode();
            if(kode < 200 || kode >= 300){
                JOptionPane.showMessageDialog(rootPane,"Tanda tangan belum dapat dikirim ke web. HTTP "+kode+".");
                return false;
            }
            pembaca = new BufferedReader(new InputStreamReader(koneksiHttp.getInputStream(),"UTF-8"));
            StringBuilder respons = new StringBuilder();
            String baris;
            while((baris=pembaca.readLine())!=null){
                respons.append(baris);
            }
            if(respons.toString().indexOf("\"success\":true")>=0){
                gambarTTDPasienWali = null;
                pathTTDPasienWali = "tersimpan";
                LblTTDPasienWali.setText("Tanda tangan pasien/wali sudah tersimpan.");
                LblTTDPasienWali.setForeground(new java.awt.Color(20,120,70));
                return true;
            }
            JOptionPane.showMessageDialog(rootPane,"Tanda tangan belum dapat disimpan: "+respons.toString());
            return false;
        }catch(Exception e){
            System.out.println("Notif TTD Pasien/Wali : "+e);
            JOptionPane.showMessageDialog(rootPane,"Tanda tangan belum dapat dikirim ke webapps persetujuan umum. Pastikan endpoint simpanTTDPasienWali.php sudah dipasang dan server web dapat diakses.");
            return false;
        }finally{
            try{ if(pembaca!=null){pembaca.close();} }catch(Exception e){}
            if(koneksiHttp!=null){koneksiHttp.disconnect();}
        }
    }

    /**
     * Mengubah hasil tanda tangan menjadi PNG transparan maksimal 100 KB.
     * Jika masih besar, gambar diperkecil bertahap tanpa menambah latar.
     */
    private byte[] konversiTandaTanganMaks100KB(BufferedImage sumber) throws Exception{
        final int batasUkuran=100*1024;
        BufferedImage kerja=sumber;
        for(int percobaan=0;percobaan<9;percobaan++){
            ByteArrayOutputStream output=new ByteArrayOutputStream();
            javax.imageio.ImageIO.write(kerja,"png",output);
            byte[] data=output.toByteArray();
            if(data.length<=batasUkuran){
                return data;
            }
            int lebarBaru=Math.max(80,(int)Math.round(kerja.getWidth()*0.78));
            int tinggiBaru=Math.max(35,(int)Math.round(kerja.getHeight()*0.78));
            if(lebarBaru==kerja.getWidth() && tinggiBaru==kerja.getHeight()){
                break;
            }
            BufferedImage kecil=new BufferedImage(lebarBaru,tinggiBaru,BufferedImage.TYPE_INT_ARGB);
            Graphics2D g=kecil.createGraphics();
            g.setRenderingHint(RenderingHints.KEY_INTERPOLATION,RenderingHints.VALUE_INTERPOLATION_BICUBIC);
            g.setRenderingHint(RenderingHints.KEY_RENDERING,RenderingHints.VALUE_RENDER_QUALITY);
            g.drawImage(kerja,0,0,lebarBaru,tinggiBaru,null);
            g.dispose();
            kerja=kecil;
        }
        throw new Exception("Ukuran file tanda tangan masih melebihi 100 KB.");
    }
    ...
    private void initComponents() {
        ...
        jLabel45.setText("Alamat P.J./Wali :");
        ...
        AlamatPJ.setBounds(114, 150, 619, 23);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TNoRw.getText().trim().equals("")||TPasien.getText().trim().equals("")){
            ...
            Valid.textKosong(AlamatPJ,"Alamat Penanggung Jawab/Wali");
            ...
            if(Sequel.menyimpantf("surat_persetujuan_umum (no_surat,no_rawat,tanggal,pengobatan_kepada,nilai_kepercayaan,nama_pj,alamat_pj,umur_pj,no_ktppj,jkpj,bertindak_atas,no_telp,nip)","?,?,?,?,?,?,?,?,?,?,?,?,?","Data",13,new String[]{
                    NoSurat.getText(),TNoRw.getText(),Valid.SetTgl(Tanggal.getSelectedItem()+""),"-","",NamaPJ.getText(),AlamatPJ.getText(),UmurPJ.getText(),NoKTP.getText(),
                ...
                tabMode.addRow(new String[]{
                    ...
                    NoTelp.getText(),BertindakAtas.getSelectedItem().toString(),NIP.getText(),NamaPetugas.getText(),AlamatPJ.getText()
                });
                LCount.setText(""+tabMode.getRowCount());
                if(kirimTandaTanganPasienWali(NoSurat.getText())){
                    emptTeks();
                }
            }
        }
        ...
        if(tabMode.getRowCount()==0){
            ...
            if(TCari.getText().trim().equals("")){
                Valid.MyReportqry("rptDataPersetujuanUmum.jasper","report","::[ Data Persetujuan Umum ]::",
                    ...
                    "petugas.nama,surat_persetujuan_umum.alamat_pj from surat_persetujuan_umum inner join reg_periksa on surat_persetujuan_umum.no_rawat=reg_periksa.no_rawat "+
    ...
    private void AlamatPJKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_AlamatPJKeyPressed
        Valid.pindah(evt,NamaPJ,UmurPJ);
    ...
    */
            ...
            try {
                ...
                while(rs.next()){
                    tabMode.addRow(new String[]{
                        ...
                        rs.getString("nip"),rs.getString("nama"),rs.getString("alamat_pj") 
                    });
                }
            } catch (Exception e) {
    ...
    private void getData() {
         if(tbObat.getSelectedRow()!= -1){
            ...
            AlamatPJ.setText(tbObat.getValueAt(tbObat.getSelectedRow(),18).toString());
            ...
            gambarTTDPasienWali = null;
            cekStatusTandaTanganPasienWali(NoSurat.getText());
        }
    }
    ...
    private void ganti() {
        if(Sequel.mengedittf("surat_persetujuan_umum","no_surat=?","no_surat=?,no_rawat=?,tanggal=?,nama_pj=?,alamat_pj=?,umur_pj=?,no_ktppj=?,jkpj=?,bertindak_atas=?,no_telp=?,nip=?",12,new String[]{
            NoSurat.getText(),TNoRw.getText(),Valid.SetTgl(Tanggal.getSelectedItem()+""),NamaPJ.getText(),AlamatPJ.getText(),UmurPJ.getText(),NoKTP.getText(),
            JKPJ.getSelectedItem().toString().substring(0,1),BertindakAtas.getSelectedItem().toString(),NoTelp.getText(),NIP.getText(),
            ...
            tbObat.setValueAt(AlamatPJ.getText(),tbObat.getSelectedRow(),18);
            if(kirimTandaTanganPasienWali(NoSurat.getText())){
                emptTeks();
            }
        }
