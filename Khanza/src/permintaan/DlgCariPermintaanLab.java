...
import laporan.DlgWaktuTungguLab;
...
public class DlgCariPermintaanLab extends javax.swing.JDialog {
    ...
    private String pilihan="",alarm="",formalarm="",nol_detik,detik,tglsampel="",tglhasil="",norm="",kamar="",namakamar="",la="",ld="",pa="",pd="",InformasiTambahan,DiagnosaKlinis,
                    NoPermintaan="",NoRawat="",Pasien="",Permintaan="",JamPermintaan="",Sampel="",JamSampel="",Hasil="",JamHasil="",KodeDokter="",DokterPerujuk="",Ruang="",json="",finger="",FileName = "", kodeberkas = "";
    ...
    public DlgCariPermintaanLab(java.awt.Frame parent, boolean modal) {
        ...
        WarnaTableLab renderer = new WarnaTableLab(koneksi);
        tbLabRalan.setDefaultRenderer(Object.class, renderer); // Sesuaikan dengan tipe objek yang kamu gunakan
...
//        WarnaTableLab renderer = new WarnaTableLab(koneksi);
        tbLabRanap.setDefaultRenderer(Object.class, renderer); // Sesuaikan dengan tipe objek yang kamu gunakan
//        tbLabRanap.setDefaultRenderer(Object.class, new WarnaTable());
        ...
        // Event klik baris tbLabRalan
        tbLabRalan.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                int row = tbLabRalan.getSelectedRow();
                if (row != -1) {
                    System.out.println("Baris diklik di tbLabRalan: " + row);
                    System.out.println("Data: " + tbLabRalan.getValueAt(row, 0).toString());
                }
            }
        });

        // Event klik baris tbLabRanap
        tbLabRanap.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                int row = tbLabRanap.getSelectedRow();
                if (row != -1) {
                    System.out.println("Baris diklik di tbLabRanap: " + row);
                    System.out.println("Data: " + tbLabRanap.getValueAt(row, 0).toString());
                }
            }
        });
    ...
    private void initComponents() {
        ...
        BtnWaktuTunggu = new widget.Button();
        ...
        TanggalPulang.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "02-05-2025 17:10:36" }));
        ...
        BtnWaktuTunggu.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/b_print.png"))); // NOI18N
        BtnWaktuTunggu.setMnemonic('T');
        BtnWaktuTunggu.setText("Waktu Tunggu Lab");
        BtnWaktuTunggu.setToolTipText("Alt+T");
        BtnWaktuTunggu.setName("BtnWaktuTunggu"); // NOI18N
        BtnWaktuTunggu.setPreferredSize(new java.awt.Dimension(150, 30));
        BtnWaktuTunggu.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnWaktuTungguActionPerformed(evt);
            }
        });
        BtnWaktuTunggu.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                BtnWaktuTungguKeyPressed(evt);
            }
        });
        panelisi1.add(BtnWaktuTunggu);
                    ...
                    if(Sampel.equals("")||akses.getkode().equals("Admin Utama")){
                        if((Sequel.cariInteger("select count(noorder) from permintaan_pemeriksaan_lab where stts_bayar='Sudah' and noorder=?",NoPermintaan)+
                            ...
                            int confirm = JOptionPane.showConfirmDialog(null,
                                    "Maaf, Apakah Anda yakin ingin menghapus permintaan ini???\nKlik Yes untuk menghapus atau klik No untuk membatalkan.",
                                    "Konfirmasi Hapus",
                            ...
                            if (confirm == JOptionPane.YES_NO_OPTION){
                                try{
                                    ...
                                    String insertQuery = "INSERT INTO rsaj_hapus_permintaan_lab (no_rawat, noorder, waktu, petugas, ip_komputer) VALUES (?,?,?,?,?)";
                                    String[] params = {NoRawat, NoPermintaan, waktu, petugas, ipKomputer};

                                    // Panggil queryu2
                                    Sequel.queryu2(insertQuery, params.length, params);
                                    
                                    // Hapus permintaan dari tabel permintaan_lab                                    
                                    Sequel.meghapus("permintaan_lab","noorder",NoPermintaan);
                                    TeksKosong();
                                    tampil();
                                } catch (Exception e){
                                    JOptionPane.showMessageDialog(null, "Terjadi kesalahan saat mencatat data: " + e.getMessage());                                           
                                }
                            }    
                        } 
...
private void tbLabRalanKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbLabRalanKeyPressed
   if(tabMode.getRowCount()!=0){
    ...
    // Berlaku untuk tab Rawat Jalan & Rawat Inap (logikanya sama)
    if (TabPilihRawat.getSelectedIndex() == 0 || TabPilihRawat.getSelectedIndex() == 1) {
        if (!NoRawat.equals("")) {
            if (NoPermintaan.trim().equals("") || DiagnosaKlinis.trim().equals("")) {
                Valid.textKosong(TCari, "No.Permintaan");
            } else {
                ...
                try {
                    ...
                    norm = Sequel.cariIsi(
                        "SELECT rp.no_rkm_medis FROM reg_periksa rp WHERE rp.no_rawat = ?",
                        NoRawat
                    );
                    param.put("nama", Sequel.cariIsi(
                        "SELECT p.nm_pasien FROM pasien p WHERE p.no_rkm_medis = ?",
                        norm
                    ));
                    ...
                    // 🔹 Unit: Ralan = poliklinik, Ranap = bangsal
                    String unit = "-";
                    if (TabPilihRawat.getSelectedIndex() == 0) {
                        // RAWAT JALAN → nama poliklinik
                        unit = Sequel.cariIsi(
                            "SELECT IFNULL(p.nm_poli,'-') " +
                            "FROM reg_periksa rp " +
                        ...
                        // RAWAT INAP → nama bangsal terakhir
                        unit = Sequel.cariIsi(
                            "SELECT IFNULL(b.nm_bangsal,'-') " +
                            "FROM kamar_inap ki " +
                    ...
                    // Kalau di Jasper masih ada $P{parameter}, tetap dikirim
                    ...
                    // 🔹 SQL datasource: 1 baris = 1 parameter Pemeriksaan
                    String sql =
                        "SELECT pl.noorder, tl.Pemeriksaan " +
                        "FROM permintaan_lab pl " +
                        "INNER JOIN permintaan_detail_permintaan_lab pdpl " +
                        "        ON pdpl.noorder = pl.noorder " +
                        "INNER JOIN template_laboratorium tl " +
                        "        ON tl.id_template = pdpl.id_template " +
                        "WHERE pl.noorder = '" + NoPermintaan + "' " +
                        "  AND pl.no_rawat = '" + NoRawat + "' " +
                        "ORDER BY tl.urut, tl.Pemeriksaan";

                    // Debug di console kalau mau cek query-nya
                    System.out.println("DEBUG SQL BarcodePermintaanLab : " + sql);

                    Valid.MyReportqry(
                        "rptBarcodePermintaanLab.jasper",
                        "report",
    ...
    private void BtnWaktuTungguActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnWaktuTungguActionPerformed
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        DlgWaktuTungguLab WTL=new DlgWaktuTungguLab(null,false);
        WTL.setSize(internalFrame1.getWidth(),internalFrame1.getHeight());
        WTL.setLocationRelativeTo(internalFrame1);               
        WTL.setVisible(true);
        this.setCursor(Cursor.getDefaultCursor());    
    }//GEN-LAST:event_BtnWaktuTungguActionPerformed

    private void BtnWaktuTungguKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnWaktuTungguKeyPressed
        // TODO add your handling code here:
    }//GEN-LAST:event_BtnWaktuTungguKeyPressed
    ...
    */
    ...
    private widget.Button BtnWaktuTunggu;
    ...
    private void tampil() {
        ...
        try {
            ...
            ps=koneksi.prepareStatement(
                    "select permintaan_lab.noorder,permintaan_lab.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir,permintaan_lab.tgl_permintaan,"+
    ...
    private void tampil2() {
        ...
        try {
            ...
            ps=koneksi.prepareStatement(
                ...
                "pasien.nm_pasien,pasien.tgl_lahir,jns_perawatan_lab.nm_perawatan,template_laboratorium.Pemeriksaan,"+
...
//    );
//
//    BtnCetakHasilLab.setVisible(akses.getpermintaan_lab());
//    BtnHasil.setVisible(allowedUsers.contains(akses.getkode()));
//    BtnSampel.setVisible(allowedUsers.contains(akses.getkode()));       
//    BtnPrint.setVisible(allowedUsers.contains(akses.getkode()));
//    BtnKirimLica.setVisible(allowedUsers.contains(akses.getkode()));
//    BtnAmbilLica.setVisible(allowedUsers.contains(akses.getkode()));
//    BtnKirimSysmex.setVisible(allowedUsers.contains(akses.getkode()));
//    BtnAmbilSysmex.setVisible(allowedUsers.contains(akses.getkode()));
//    BtnKirimLISTeras.setVisible(allowedUsers.contains(akses.getkode()));
//    BtnAmbilLISTeras.setVisible(allowedUsers.contains(akses.getkode()));
//    BtnKirimLISMADQLAB.setVisible(allowedUsers.contains(akses.getkode()));
//    BtnAmbilLISMADQLAB.setVisible(allowedUsers.contains(akses.getkode()));
//    BtnHapus.setVisible(allowedUsers.contains(akses.getkode()));

    List<String> allowedUsers = new ArrayList<>();
    try {
        ps = koneksi.prepareStatement(
            "SELECT kd_dokter FROM dokter_mpp WHERE status = '3'"
        );
        rs = ps.executeQuery();
        while (rs.next()) {
            allowedUsers.add(rs.getString("kd_dokter"));
        }
        // Tambahkan user Admin Utama jika memang perlu dimasukkan manual
        allowedUsers.add("Admin Utama");
    } catch (Exception e) {
        System.out.println("Error saat ambil allowedUsers: " + e);
    } finally {
        if(rs != null) {
    ...
    // Cek dan atur tombol berdasarkan akses
    String userKode = akses.getkode();
    boolean isAllowed = allowedUsers.contains(userKode);

    BtnCetakHasilLab.setVisible(akses.getpermintaan_lab());
    BtnHasil.setVisible(isAllowed);
    BtnSampel.setVisible(isAllowed);
    BtnPrint.setVisible(isAllowed);
    BtnKirimLica.setVisible(isAllowed);
    BtnAmbilLica.setVisible(isAllowed);
    BtnKirimSysmex.setVisible(isAllowed);
    BtnAmbilSysmex.setVisible(isAllowed);
    BtnKirimLISTeras.setVisible(isAllowed);
    BtnAmbilLISTeras.setVisible(isAllowed);
    BtnKirimLISMADQLAB.setVisible(isAllowed);
    BtnAmbilLISMADQLAB.setVisible(isAllowed);
    BtnHapus.setVisible(isAllowed);
...
//                // Ubah di sini agar alarm berbunyi setiap 5 detik
//                if(detik.equals("05")){
//                    permintaanbaru=0;
//                    if(formalarm.contains("ralan")){
//                        tampil();
//                        for(i=0;i<tbLabRalan.getRowCount();i++){
//                            if((!tbLabRalan.getValueAt(i,0).toString().equals(""))&&tbLabRalan.getValueAt(i,5).toString().equals("")&&tbLabRalan.getValueAt(i,12).toString().toLowerCase().contains("cito")){
//                                permintaanbaru++;
//                            }
//                        }
//                    }
...
//                            if((!tbLabRanap.getValueAt(i,0).toString().equals(""))&&tbLabRanap.getValueAt(i,5).toString().equals("")&&tbLabRanap.getValueAt(i,12).toString().toLowerCase().contains("cito")){
//                                permintaanbaru++;
//                            }
//                        }
//                    }
...
//        new Timer(1000, taskPerformer).start();
    ...
    ActionListener taskPerformer = (ActionEvent e) -> {
        if (aktif) {
            ...
            if (formalarm.contains("ralan")) {
                tampil();
                for (int i = 0; i < tbLabRalan.getRowCount(); i++) {
                    if ((!tbLabRalan.getValueAt(i, 0).toString().equals("")) && 
                        tbLabRalan.getValueAt(i, 5).toString().equals("") && 
                        tbLabRalan.getValueAt(i, 12).toString().toLowerCase().contains("cito")) {
                        permintaanbaru++;
                    }
                }
            }
            ...
            if (formalarm.contains("ranap")) {
                tampil3();
                for (int i = 0; i < tbLabRanap.getRowCount(); i++) {
                    if ((!tbLabRanap.getValueAt(i, 0).toString().equals("")) && 
                        tbLabRanap.getValueAt(i, 5).toString().equals("") && 
                        tbLabRanap.getValueAt(i, 12).toString().toLowerCase().contains("cito")) {
                        permintaanbaru++;
                    }
                }
            }

            // Alarm bunyi setiap 2 detik jika ada permintaan CITO
            if (permintaanbaru > 0) {
                try {
                    music = new BackgroundMusic("./suara/alarm.mp3");
                    music.start();
