 ...
 * @author dosen
 */
public class DlgPermintaanKonsultasiMedik_web extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgPermintaanKonsultasiMedik_web(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();

        // === Web-style skinning (non-invasive, works even without FlatLaf) ===
        try {
            // Rounded corners & subtle focus (FlatLaf-friendly; ignored by other LAFs)
        ...
        // Header bar ala web (brand color) + wrap form as a "card"
        try {
            java.awt.Container cp = getContentPane();
            if (!(cp.getLayout() instanceof java.awt.BorderLayout)) {
                cp.setLayout(new java.awt.BorderLayout());
            }
            cp.add(makeHeaderWeb(), java.awt.BorderLayout.NORTH);
            cp.setBackground(new java.awt.Color(0xF5,0xF7,0xFB));

            wrapFormInputAsCard();
            applyWebPlaceholders();
            styleActionButtons();
            styleTableWeb();
        } catch (Throwable t) {
            // don't fail UI if styling can't be applied
        }
...
private void adjustWindowInputToAvailableScreen() {
    ...
    int marginBottom = 110; // Jarak dari tepi bawah (lebih besar jika dibutuhkan)    
    int scrollPaneX = 20;  // Jarak dari kiri WindowInput
    int scrollPaneY = 100; // Jarak dari atas WindowInput

    // Hitung ukuran ScrollPane2 dengan mempertimbangkan margin
    int scrollPaneWidth = screenBounds.width - scrollPaneX - marginRight;    // Lebar dengan jarak margin dari kanan
    int scrollPaneHeight = screenBounds.height - scrollPaneY - marginBottom; // Tinggi dengan jarak margin dari bawah

    // Atur posisi dan ukuran ScrollPane2
...
private void HapusJPG() {
    ...
    // --- Helpers to "web-ify" the dialog without breaking existing logic ---
    private javax.swing.JComponent makeHeaderWeb() {
        javax.swing.JPanel header = new javax.swing.JPanel(new java.awt.BorderLayout());
        header.setBorder(new javax.swing.border.EmptyBorder(14,18,14,14));
        header.setBackground(new java.awt.Color(14,165,233)); // #0EA5E9
        javax.swing.JLabel title = new javax.swing.JLabel("Permintaan Konsultasi Medik");
    ...
    private void wrapFormInputAsCard() {
        try {
            java.awt.Container parent = FormInput.getParent();
            if (parent != null) {
                // Pindahkan FormInput ke dalam panel "card" putih rounded
                parent.remove(FormInput);
                javax.swing.JPanel card = new javax.swing.JPanel(new java.awt.BorderLayout());
                card.setOpaque(true);
                card.setBackground(java.awt.Color.WHITE);
                card.setBorder(javax.swing.BorderFactory.createCompoundBorder(
                        javax.swing.BorderFactory.createLineBorder(new java.awt.Color(229,231,235)), // #E5E7EB
                        new javax.swing.border.EmptyBorder(16,16,16,16)
                ));
                card.add(FormInput, java.awt.BorderLayout.CENTER);
                parent.add(card, java.awt.BorderLayout.CENTER);
                parent.revalidate();
                parent.repaint();
            }
        } catch (Throwable ignore) {}
    ...
    private void applyWebPlaceholders() {
        setPlaceholder(NoRw, "No. Rawat");
        setPlaceholder(NoRM, "No. RM");
        setPlaceholder(NmPasien, "Nama Pasien");
        setPlaceholder(KdDokter, "Kode Dokter Pengirim");
        setPlaceholder(NmDokter, "Nama Dokter Pengirim");
        setPlaceholder(KdDokterDikonsuli, "Kode Dokter Dikonsuli");
        setPlaceholder(NmDokterDikonsuli, "Nama Dokter Dikonsuli");
        setPlaceholder(DiagnosaKerja, "Diagnosa Kerja (ICD)");
        // TextArea placeholders (if LAF supports)
        setPlaceholder(UraianKonsultasi, "Uraian/Alasan Permintaan Konsultasi...");
        setPlaceholder(JawabanKonsultasi, "Jawaban Konsultasi...");
        // Pencarian
        setPlaceholder(TCari, "Cari no.rawat/nama/dokter...");
    }
    ...
    private void styleActionButtons() {
        ...
        stylePrimaryButton(BtnReUploadPerminKonsul);
        stylePrimaryButton(BtnReUploadJawabKonsul);

        // Secondary / ghost
        styleGhostButton(BtnBatal);
        styleGhostButton(BtnEdit);
        styleGhostButton(BtnHapus);
        styleGhostButton(BtnKeluar);
        styleGhostButton(BtnPrint);
        styleGhostButton(BtnUpload1);
        styleGhostButton(BtnCari);
        styleGhostButton(BtnAll);
    }

    private void styleTableWeb() {
        try {
            tbObat.setRowHeight(26);
            tbObat.setGridColor(new java.awt.Color(235, 238, 240));
            tbObat.setShowGrid(true);
            if(tbObat.getTableHeader()!=null){
                tbObat.getTableHeader().setReorderingAllowed(true);
                tbObat.getTableHeader().setResizingAllowed(true);
            }
            // FlatLaf-friendly tweaks via client properties (safe for other LAFs)
            tbObat.putClientProperty("Table.isStriped", Boolean.TRUE);
            tbObat.putClientProperty("Table.showVerticalLines", Boolean.FALSE);
            tbObat.putClientProperty("Table.showHorizontalLines", Boolean.TRUE);
        } catch (Throwable ignore) {}
    }

    private void setPlaceholder(java.awt.Component comp, String text) {
    try {
        if (comp instanceof javax.swing.JTextField) {
            ...
            ((javax.swing.JPasswordField) comp)
                    .putClientProperty("JTextField.placeholderText", text);
        } else if (comp instanceof javax.swing.JTextArea) {
            ((javax.swing.JTextArea) comp)
                    .putClientProperty("JTextArea.placeholderText", text);
        } else if (comp instanceof javax.swing.JComboBox) {
            // hanya berfungsi kalau combobox editable
            ((javax.swing.JComboBox<?>) comp)
                    .putClientProperty("JComboBox.placeholderText", text);
        } else if (comp instanceof javax.swing.JComponent) {
            // fallback umum (boleh kamu hapus kalau tidak perlu)
    ...
    private void stylePrimaryButton(javax.swing.AbstractButton b) {
        if (b == null) return;
        try {
            b.putClientProperty("JButton.buttonType", "roundRect");
            b.putClientProperty("JComponent.minimumWidth", 110);
            b.setBackground(new java.awt.Color(14,165,233));  // #0EA5E9
            b.setForeground(java.awt.Color.WHITE);
        } catch (Throwable ignore) {}
    }

    private void styleGhostButton(javax.swing.AbstractButton b) {
        if (b == null) return;
        try {
            b.putClientProperty("JButton.buttonType", "roundRect");
            b.putClientProperty("JComponent.minimumWidth", 100);
            b.setBackground(new java.awt.Color(243,244,246)); // #F3F4F6
            b.setForeground(new java.awt.Color(17,24,39));     // #111827
        } catch (Throwable ignore) {}
    }
}
