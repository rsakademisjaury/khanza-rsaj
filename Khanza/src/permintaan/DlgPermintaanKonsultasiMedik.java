...
public class DlgPermintaanKonsultasiMedik extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        TanggalJawab.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "04-06-2025 12:42:07" }));
        ...
        TanggalPermintaan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "04-06-2025 12:42:06" }));
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoRw.getText().trim().equals("") || NoRM.getText().trim().equals("") || NmPasien.getText().trim().equals("")){
            ...
            boolean suksesSimpan=false;
            try {
                koneksi.setAutoCommit(false);

                nomorSurat();
                NoPermintaan.setText(nomorSuratGenerated);
                ...
                while (Sequel.cariInteger("SELECT COUNT(*) FROM konsultasi_medik WHERE no_permintaan=?", nomorSuratGenerated) > 0) {
                    ...
                    if (attempts >= maxAttempts) {
                        throw new SQLException("Gagal menghasilkan nomor permintaan unik setelah 10 kali percobaan.");
                    }
                    NoPermintaan.setText(nomorSuratGenerated);
                }

                suksesSimpan = Sequel.menyimpantf("konsultasi_medik", "?,?,?,?,?,?,?,?", "No.Permintaan", 8, new String[]{
                ...
                if(suksesSimpan) {
                    simpanDpjpRanapJikaPerlu(NoRw.getText(), KdDokterDikonsuli.getText());
                    koneksi.commit();
                    R1.setSelected(true);
                    tampil();
                    emptTeks();
                    ...
                    koneksi.rollback();
                    JOptionPane.showMessageDialog(null, "Maaf, gagal menyimpan permintaan konsultasi medik...!!!");
                }
            } catch (Exception e) {
                try {
                    koneksi.rollback();
                ...
                JOptionPane.showMessageDialog(null, "Terjadi kesalahan saat menyimpan permintaan konsultasi medik: " + e.getMessage());
            } finally {
                try {
...
//        ppBerkasDigitalBtnPrintActionPerformed(evt);       
    ...
    private void BtnJawabanDikonsuliActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnJawabanDikonsuliActionPerformed
        if(tabMode.getRowCount() == 0){
            ...
            if(tbObat.getSelectedRow() != -1){
                getData();
                if(akses.getkode().equals("Admin Utama") || KdDokterDikonsuli.getText().equals(akses.getkode())){
                    NoPermintaanJawaban.setText(tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
    ...
    // Menyesuaikan ukuran WindowInput tanpa menutupi taskbar.
    private void adjustWindowInputToAvailableScreen() {
        ...
        int scrollPaneWidth = screenBounds.width - scrollPaneX - marginRight;
        int scrollPaneHeight = screenBounds.height - scrollPaneY - marginBottom;

        scrollPane2.setBounds(scrollPaneX, scrollPaneY, scrollPaneWidth, scrollPaneHeight);
    }
...
//        ppBerkasDigitalBtnPrintActionPerformed(evt);        
    ...
    private void BtnSimpanJawabanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanJawabanActionPerformed
        String noPermintaanJawaban = NoPermintaanJawaban.getText().trim();
        String tanggalJawab = Valid.SetTgl(TanggalJawab.getSelectedItem()+"") + " " + TanggalJawab.getSelectedItem().toString().substring(11,19);
        String diagnosaJawaban = JawabanDiagnosaKerja.getText().trim();
        String uraianJawaban = Valid.KarakterHaram(JawabanKonsultasi.getText());
        ...
        if(NoRw.getText().trim().equals("") || NoRM.getText().trim().equals("") || NmPasien.getText().trim().equals("")){
            ...
            Valid.textKosong(UraianKonsultasi, "Uraian Konsultasi");
        } else if(diagnosaJawaban.equals("")){
            Valid.textKosong(JawabanDiagnosaKerja, "Jawaban Diagnosa Kerja");
        } else if(noPermintaanJawaban.equals("")){
            ...
            if(Sequel.menyimpantf("jawaban_konsultasi_medik", "?,?,?,?", 4, new String[]{
                    noPermintaanJawaban, tanggalJawab, diagnosaJawaban, uraianJawaban
                }, "no_permintaan=?", "tanggal=?,diagnosa_kerja=?,uraian_jawaban=?", 4, new String[]{
                    tanggalJawab, diagnosaJawaban, uraianJawaban, noPermintaanJawaban
                }) == true) {
                JOptionPane.showMessageDialog(WindowInput, "Jawaban konsul berhasil tersimpan");
                NoPermintaan.setText(noPermintaanJawaban);
                R2.setSelected(true);
                tampil();
                emptTeks();
                WindowInput.dispose();
...
//        ppBerkasDigitalBtnPrintActionPerformed(evt);    
    ...
    private void BtnUploadActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnUploadActionPerformed
        if (tbObat.getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(null, "Maaf, silahkan pilih data konsultasi yang akan di-upload...!!!!");
            return;
        }

        String noRawatTujuan = cariNoRawatKunjunganTujuanUpload();
        if (noRawatTujuan.equals("")) {
            tampilkanPesanKunjunganTujuanTidakDitemukan();
            return;
        }
        ...
        FileName = timestamp + "_" + noRawatTujuan.replace("/", "") + "_PermintaanKonsul";

        CreatePDF(FileName);
        ConvertPDFtoJPG(FileName);
        boolean uploadBerhasil = UploadJPG(FileName, "berkasrawat/pages/upload/", noRawatTujuan);
        HapusJPG();

        if (uploadBerhasil) {
            bukaBerkasDigital(noRawatTujuan);
        }
    ...
    private void BtnUpload1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnUpload1ActionPerformed
        ...
        FileName2 = timestamp + "_" + noRawatTujuan.replace("/", "") + "_JawabanKonsul";

        CreatePDF2(FileName2);
        ConvertPDFtoJPG2(FileName2);
        boolean uploadBerhasil = UploadJPG2(FileName2, "berkasrawat/pages/upload/", noRawatTujuan);
        HapusJPG();
...
//        sql = "";
        if (akses.getjml2() >= 1) {
            // Hapus cekDokterMPP jika tidak diperlukan, atau isi dengan nilai yang sesuai
//            sql = "(konsultasi_medik.kd_dokter='" + akses.getkode() + "' or konsultasi_medik.kd_dokter_dikonsuli='" + akses.getkode() + "') and ";
        ...
        if (R1.isSelected() == true) {
            ...
            ps = koneksi.prepareStatement(
                ...
                "inner join dokter as dokterdikonsuli on konsultasi_medik.kd_dokter_dikonsuli=dokterdikonsuli.kd_dokter " +
                "inner join jawaban_konsultasi_medik on jawaban_konsultasi_medik.no_permintaan=konsultasi_medik.no_permintaan " +
                "where " + sql + " konsultasi_medik.no_rawat like ? " +
                (TCari.getText().equals("") ? "" : "and (konsultasi_medik.no_rawat like ? or reg_periksa.no_rkm_medis like ? or pasien.nm_pasien like ? " +
...
//            sql="";
//            if(akses.getjml2()>=1){
//                String cekDokterMPP = null;
//                sql="(konsultasi_medik.kd_dokter='" + cekDokterMPP + "' or konsultasi_medik.kd_dokter='" + akses.getkode() + "' or konsultasi_medik.kd_dokter_dikonsuli='" + akses.getkode() + "') and ";
//            }
//            
//            if(R1.isSelected()==true){
//               BtnLihatJawabanKonsul.setVisible(false);
...
////                    "or penjab.png_jawab like ? or konsultasi_medik.no_permintaan like ? or konsultasi_medik.jenis_permintaan like ?)")+" order by konsultasi_medik.tanggal");
//                try {
//                    // Set parameter untuk pencarian no_rawat dan teks pencarian lainnya
//                    ps.setString(1, "%" + TCari.getText().trim() + "%");
...
public void emptTeks() {
    ...
    private String nilaiTabel(int kolom) {
        if(tbObat.getSelectedRow() == -1 || tbObat.getValueAt(tbObat.getSelectedRow(), kolom) == null) {
            return "";
        }
        return tbObat.getValueAt(tbObat.getSelectedRow(), kolom).toString();
    }

    private void getData() {
        if(tbObat.getSelectedRow()!= -1){
            NoPermintaan.setText(nilaiTabel(0));
            NoRw.setText(nilaiTabel(1));
            NoRM.setText(nilaiTabel(2));
            NmPasien.setText(nilaiTabel(3));
            Permintaan.setSelectedItem(nilaiTabel(9));
            KdDokter.setText(nilaiTabel(10));
            NmDokter.setText(nilaiTabel(11));
            KdDokterDikonsuli.setText(nilaiTabel(12));
            NmDokterDikonsuli.setText(nilaiTabel(13));
            DiagnosaKerja.setText(nilaiTabel(14));
            UraianKonsultasi.setText(nilaiTabel(15));
            JawabanDiagnosaKerja.setText(nilaiTabel(17));
            JawabanKonsultasi.setText(nilaiTabel(18));

            if(!nilaiTabel(8).trim().equals("")){
                Valid.SetTgl2(TanggalPermintaan,nilaiTabel(8));
            }
            if(!nilaiTabel(16).trim().equals("")){
                Valid.SetTgl2(TanggalJawab,nilaiTabel(16));
            } else {
                TanggalJawab.setDate(new Date());
            }
        }
    ...
    private void simpanDpjpRanapJikaPerlu(String noRawat, String kdDokterDikonsuli) {
        try {
            if(Sequel.cariIsi("SELECT status_lanjut FROM reg_periksa WHERE no_rawat=?", noRawat).equals("Ranap") &&
               !dpjpRanapSudahAda(noRawat, kdDokterDikonsuli)){
                Sequel.menyimpan("dpjp_ranap", "?,?", "Data DPJP Ranap", 2, new String[]{
                    noRawat, kdDokterDikonsuli
                });
            }
        } catch (Exception e) {
            System.out.println("Notif simpan DPJP Ranap dari konsultasi medik : " + e);
        }
    }

    private boolean dpjpRanapSudahAda(String noRawat, String kdDokter) {
        boolean ada=false;
        PreparedStatement psDpjp=null;
        ResultSet rsDpjp=null;
        try {
            psDpjp = koneksi.prepareStatement("select count(*) from dpjp_ranap where no_rawat=? and kd_dokter=?");
            psDpjp.setString(1, noRawat);
            psDpjp.setString(2, kdDokter);
            rsDpjp = psDpjp.executeQuery();
            if(rsDpjp.next()){
                ada = rsDpjp.getInt(1) > 0;
            }
        } catch (Exception e) {
            System.out.println("Notif cek DPJP Ranap dari konsultasi medik : " + e);
        } finally {
            try {
                if(rsDpjp != null) rsDpjp.close();
                if(psDpjp != null) psDpjp.close();
            } catch (Exception e) {
                System.out.println("Notif tutup cek DPJP Ranap : " + e);
            }
        }
        return ada;
    }
...
//        "SELECT MAX(CONVERT(LEFT(no_permintaan, 3), SIGNED)) FROM konsultasi_medik WHERE " +
//        "no_permintaan LIKE '%/KM/" + bln_romawi + "/" + TanggalPermintaan.getSelectedItem().toString().substring(6, 10) + "'"
//    );
//
...
//    nomorSuratGenerated = String.format("%03d", nextNumber) + "/KM/" + bln_romawi + "/" + 
//                          TanggalPermintaan.getSelectedItem().toString().substring(6, 10);
//    
//    // END MODIF MUSTAFA
    }    
...
/**
 * Menentukan no. rawat tujuan upload konsultasi.
 * - Konsultasi pasien rawat inap: berkas tetap diunggah ke no. rawat konsultasi saat ini.
 * - Konsultasi pasien rawat jalan: berkas diunggah ke kunjungan pertama pasien ke dokter yang
 *   dikonsuli setelah konsultasi dibuat. Kunjungan yang dibatalkan tidak dipilih.
 */
private String cariNoRawatKunjunganTujuanUpload() {
    if (tbObat.getSelectedRow() == -1) {
        return "";
    }

    String noPermintaan = tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString().trim();
    String noRawatKonsultasi = tbObat.getValueAt(tbObat.getSelectedRow(), 1).toString().trim();

    // Khusus rawat inap, konsultasi dan jawabannya harus tersimpan pada no. rawat konsultasi saat ini.
    // Jangan hanya mengandalkan status_lanjut, karena pada beberapa alur rawat inap
    // status tersebut belum/ tidak terbaca sebagai Ranap. Kamar inap aktif ditandai stts_pulang='-'.
    String statusLanjut = Sequel.cariIsi("SELECT status_lanjut FROM reg_periksa WHERE no_rawat=?", noRawatKonsultasi);
    String noRawatKamarInapAktif = Sequel.cariIsi(
            "SELECT no_rawat FROM kamar_inap WHERE no_rawat=? AND stts_pulang='-' LIMIT 1", noRawatKonsultasi);

    if ("Ranap".equalsIgnoreCase(statusLanjut) || !noRawatKamarInapAktif.trim().equals("")) {
        return noRawatKonsultasi;
    }

    String noRawatTujuan = "";
    String query = "SELECT rp_tujuan.no_rawat "
            + "FROM konsultasi_medik km "
            + "INNER JOIN reg_periksa rp_asal ON rp_asal.no_rawat=km.no_rawat "
            + "INNER JOIN reg_periksa rp_tujuan ON rp_tujuan.no_rkm_medis=rp_asal.no_rkm_medis "
            + "WHERE km.no_permintaan=? "
            + "AND rp_tujuan.kd_dokter=km.kd_dokter_dikonsuli "
            + "AND rp_tujuan.stts<>'Batal' "
            + "AND TIMESTAMP(rp_tujuan.tgl_registrasi,rp_tujuan.jam_reg)>km.tanggal "
            + "ORDER BY rp_tujuan.tgl_registrasi ASC, rp_tujuan.jam_reg ASC, rp_tujuan.no_rawat ASC "
            + "LIMIT 1";

    try (PreparedStatement psCari = koneksi.prepareStatement(query)) {
        psCari.setString(1, noPermintaan);
        try (ResultSet rsCari = psCari.executeQuery()) {
            if (rsCari.next()) {
                noRawatTujuan = rsCari.getString("no_rawat");
            }
        }
    } catch (Exception e) {
        System.out.println("Gagal mencari kunjungan tujuan upload konsultasi : " + e);
        JOptionPane.showMessageDialog(null, "Terjadi kesalahan saat mencari kunjungan dokter yang dikonsuli: " + e.getMessage(), "Kesalahan", JOptionPane.ERROR_MESSAGE);
    }

    return noRawatTujuan == null ? "" : noRawatTujuan.trim();
}

private void tampilkanPesanKunjunganTujuanTidakDitemukan() {
    JOptionPane.showMessageDialog(null,
            "Upload dibatalkan. Tidak ditemukan kunjungan rawat jalan pasien ke dokter yang dikonsuli setelah konsultasi dibuat.\n"
            + "Untuk pasien rawat inap aktif, berkas akan langsung menggunakan no. rawat konsultasi.\n"
            + "Berkas tidak disimpan agar tidak salah tertaut ke no. rawat lain.",
            "Kunjungan Tujuan Tidak Ditemukan", JOptionPane.WARNING_MESSAGE);
}
...
private void ppBerkasDigitalBtnPrintActionPerformed(java.awt.event.ActionEvent evt) {
    if (tbObat.getSelectedRow() == -1) {
        JOptionPane.showMessageDialog(null, "Maaf, silahkan pilih data konsultasi...!!!!");
        return;
    }

    bukaBerkasDigital(tbObat.getValueAt(tbObat.getSelectedRow(), 1).toString());
}

private void bukaBerkasDigital(String noRawat) {
    ...
    try {
        if (akses.gethapus_berkas_digital_perawatan() == true) {
            berkas.loadURL("http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/" + "berkasrawat/login2.php?act=login&usere=" + koneksiDB.USERHYBRIDWEB() + "&passwordte=" + koneksiDB.PASHYBRIDWEB() + "&no_rawat=" + noRawat);
        } else {
            berkas.loadURL("http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/" + "berkasrawat/login2nonhapus.php?act=login&usere=" + koneksiDB.USERHYBRIDWEB() + "&passwordte=" + koneksiDB.PASHYBRIDWEB() + "&no_rawat=" + noRawat);
        }
    } catch (Exception ex) {
        System.out.println("Notifikasi : " + ex);
    }
...
//
//        // Simpan ke JPG
//        File jpgFile = new File(jpgDir, FileName2 + ".jpg");
//        ImageIO.write(image, "jpg", jpgFile);
//
//        // Tutup dokumen PDF
//        document.close();
...
private boolean UploadJPG(String FileName, String docpath, String noRawatTujuan) {
    boolean semuaUploadBerhasil = true;
    try {
        File jpgDir = new File("tmpJPG");
        if (!jpgDir.exists()) {
        ...
        for (File jpgFile : jpgFiles) {
            ...
            try {
                ...
                if (Sequel.cariInteger("SELECT COUNT(no_rawat) AS jumlah FROM berkas_digital_perawatan WHERE lokasi_file='" + filePath + "'") > 0) {
                    uploadSuccess = Sequel.mengedittf("berkas_digital_perawatan", "lokasi_file=?", "no_rawat=?,kode=?, lokasi_file=?", 4, new String[]{
                        noRawatTujuan, kodeberkas, filePath, filePath
                    });
                } else {
                    uploadSuccess = Sequel.menyimpantf("berkas_digital_perawatan", "?,?,?", "No.Rawat", 3, new String[]{
                        noRawatTujuan, kodeberkas, filePath
                    });
                }
        ...
        if (semuaUploadBerhasil) {
            JOptionPane.showMessageDialog(null, "Semua file berhasil di-upload ke no. rawat " + noRawatTujuan + "!", "Informasi", JOptionPane.INFORMATION_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(null, "Sebagian file gagal disimpan ke berkas digital.", "Peringatan", JOptionPane.WARNING_MESSAGE);
        }
    } catch (Exception e) {
        System.out.println("Upload error: " + e);
        JOptionPane.showMessageDialog(null, "Terjadi kesalahan saat upload: " + e.getMessage(), "Kesalahan", JOptionPane.ERROR_MESSAGE);
...
private boolean UploadJPG2(String FileName2, String docpath, String noRawatTujuan) {
    boolean semuaUploadBerhasil = true;
    try {
        File jpgDir = new File("tmpJPG");
        if (!jpgDir.exists()) {
        ...
        for (File jpgFile : jpgFiles) {
            ...
            try {
                ...
                if (Sequel.cariInteger("SELECT COUNT(no_rawat) AS jumlah FROM berkas_digital_perawatan WHERE lokasi_file='" + filePath + "'") > 0) {
                    uploadSuccess = Sequel.mengedittf("berkas_digital_perawatan", "lokasi_file=?", "no_rawat=?,kode=?, lokasi_file=?", 4, new String[]{
                        noRawatTujuan, kodeberkas2, filePath, filePath
                    });
                } else {
                    uploadSuccess = Sequel.menyimpantf("berkas_digital_perawatan", "?,?,?", "No.Rawat", 3, new String[]{
                        noRawatTujuan, kodeberkas2, filePath
                    });
                }
...
//    try {
//        File file = new File("tmpJPG/" + FileName2 + ".jpg");
//        byte[] data = FileUtils.readFileToByteArray(file);
//        HttpClient httpClient = new DefaultHttpClient();
//        HttpPost postRequest = new HttpPost("http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/upload.php?doc=" + docpath);
//        ByteArrayBody fileData = new ByteArrayBody(data, FileName2 + ".jpg");
//        MultipartEntity reqEntity = new MultipartEntity(HttpMultipartMode.BROWSER_COMPATIBLE);
//        reqEntity.addPart("file", fileData);
//        postRequest.setEntity(reqEntity);
//        httpClient.execute(postRequest);
...
//        boolean uploadSuccess = false;
//        kodeberkas2 = Sequel.cariIsi("SELECT kode FROM master_berkas_digital WHERE nama LIKE '%Berkas Konsultasi%'");
//        if (Sequel.cariInteger("SELECT COUNT(no_rawat) AS jumlah FROM berkas_digital_perawatan WHERE lokasi_file='pages/upload/" + FileName2 + ".jpg'") > 0) {
//            uploadSuccess = Sequel.mengedittf("berkas_digital_perawatan", "lokasi_file=?", "no_rawat=?,kode=?, lokasi_file=?", 4, new String[]{
//                tbObat.getValueAt(tbObat.getSelectedRow(), 1).toString().trim(), kodeberkas2, "pages/upload/" + FileName2 + ".jpg", "pages/upload/" + FileName2 + ".jpg"
//            });
//        } else {
//            uploadSuccess = Sequel.menyimpantf("berkas_digital_perawatan", "?,?,?", "No.Rawat", 3, new String[]{
//                tbObat.getValueAt(tbObat.getSelectedRow(), 1).toString().trim(), kodeberkas2, "pages/upload/" + FileName2 + ".jpg"
//            });
//        }
//        
//        if (uploadSuccess) {
