 ...
 * @author dosen
 */
public class DlgLihatKonsultasiMedik extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgLihatKonsultasiMedik(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        TanggalJawab.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "14-08-2025 17:07:36" }));
        ...
        jLabel42.setText("Jawaban Konsul :");
        ...
        scrollPane2.setBounds(20, 100, 105, 490);
        ...
        TanggalPermintaan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "14-08-2025 17:07:35" }));
...
private void adjustWindowInputToAvailableScreen() {
    // Dapatkan ukuran layar yang tersedia
    java.awt.GraphicsEnvironment ge = java.awt.GraphicsEnvironment.getLocalGraphicsEnvironment();
    java.awt.Rectangle screenBounds = ge.getMaximumWindowBounds();

    // Atur ukuran WindowInput
    WindowInput.setSize(screenBounds.width, screenBounds.height);
    WindowInput.setLocation(screenBounds.x, screenBounds.y);

    // Margin & posisi awal
    int marginRight = 40;   // jarak dari tepi kanan
    int marginBottom = 110; // jarak dari tepi bawah
    int areaX = 20;
    int areaY = 100;

    // Hitung ukuran area konten
    int areaW = screenBounds.width - areaX - marginRight;
    int areaH = screenBounds.height - areaY - marginBottom;

    // Atur bounds komponen utama
    if (splitUraianJawaban != null) {
        splitUraianJawaban.setBounds(areaX, areaY, areaW, areaH);
        splitUraianJawaban.setDividerLocation(0.5);
    } else {
        // fallback jika split belum dibuat (misal dipanggil sebelum setup)
        scrollPane2.setBounds(areaX, areaY, areaW, areaH);
    }

    // Posisi label kiri & kanan
    try {
        jLabel42.setBounds(areaX, 70, 120, 23); // "Uraian Konsul :"
    } catch (Throwable t) {}
    try {
        if (labelJawabanKanan != null) {
            int rightX = areaX + (areaW / 2) + 10;
            labelJawabanKanan.setBounds(rightX, 70, 130, 23);
        }
    } catch (Throwable t) {}
...
//        ppBerkasDigitalBtnPrintActionPerformed(evt);    
    ...
    if(tabMode.getRowCount() == 0){
        ...
        TCari.requestFocus();
            ...
            if(tbObat.getSelectedRow() != -1){
                // Cek jika akses adalah Admin Utama atau langsung melanjutkan logika tanpa validasi dokter
                NoPermintaanJawaban.setText(NoPermintaan.getText());
                adjustWindowInputToAvailableScreen(); // Panggil metode untuk menyesuaikan ukuran form
                WindowInput.setAlwaysOnTop(false);
                WindowInput.setVisible(true);
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgLihatKonsultasiMedik dialog = new DlgLihatKonsultasiMedik(new javax.swing.JFrame(), true);
...
// === Custom fields for split view of Uraian (left) and Jawaban (right) in WindowInput ===
...
// === End custom fields ===
    ...
    /** Set kode dokter (penerima konsul) untuk memfilter daftar */
    public void setKdDokter(String kdDokter) {
        this.kdDokterFilter = kdDokter;
...
public void tampil() {
    ...
    // bangun filter dokter yang dituju (kd_dokter_dikonsuli)
    if (kdDokterFilter != null && !kdDokterFilter.trim().isEmpty()) {
        sql = "konsultasi_medik.kd_dokter_dikonsuli='" + kdDokterFilter + "' and ";
    } else {
        // fallback default: kosongkan jika tidak ada filter
        sql = "";
    }
    ...
    // ===== Prefix WHERE untuk dokter (tanpa ubah parameter ps) =====
    // - modeBalasan=true  → filter pengirim
    // - else              → filter penerima (kdDokterFilter)
    String whereDoctor = "";
    if (modeBalasan && kdDokterPengirim != null && !kdDokterPengirim.trim().isEmpty()) {
        whereDoctor = "konsultasi_medik.kd_dokter='" + kdDokterPengirim + "' and ";
    } else if (kdDokterFilter != null && !kdDokterFilter.trim().isEmpty()) {
        whereDoctor = "konsultasi_medik.kd_dokter_dikonsuli='" + kdDokterFilter + "' and ";
    } // kalau dua-duanya kosong, biarkan empty (lihat semua)

    try {
        if (R1.isSelected()) {
            // =================== BELUM DIJAWAB (penerima) ===================
            BtnLihatJawabanKonsul.setVisible(false);
            BtnDokumentasiKonsul2.setVisible(false);
            ...
            ps = koneksi.prepareStatement(
                ...
                "inner join dokter as dokterdikonsuli on konsultasi_medik.kd_dokter_dikonsuli=dokterdikonsuli.kd_dokter " +
                "where " + wherePrefix() +
                "konsultasi_medik.no_permintaan not in " +
                "  (select DISTINCT jawaban_konsultasi_medik.no_permintaan from jawaban_konsultasi_medik) " +
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{
                        rs.getString("no_permintaan"), rs.getString("no_rawat"), rs.getString("no_rkm_medis"), rs.getString("nm_pasien"), rs.getString("jk"),
                        ...
                        "", "", "" // kolom kosong yg sudah ada di modelmu
                    });
                }
            } finally {
                if (rs != null) rs.close();
            ...
            // =================== SUDAH DIJAWAB (penerima ATAU pengirim) ===================
            // - kalau modeBalasan=true → filter pengirim (kdDokterPengirim)
            // - kalau modeBalasan=false → filter penerima (kdDokterFilter)
            ...
            ps = koneksi.prepareStatement(
                ...
                "konsultasi_medik.kd_dokter, dokterkonsul.nm_dokter as dokterkonsul, konsultasi_medik.kd_dokter_dikonsuli, dokterdikonsuli.nm_dokter as dokterdikonsuli, " +
                "konsultasi_medik.diagnosa_kerja as diagnosakerjakonsul, konsultasi_medik.uraian_konsultasi, " +
                "jawaban_konsultasi_medik.tanggal as tanggaljawaban, jawaban_konsultasi_medik.diagnosa_kerja as diagnosakerjajawaban, jawaban_konsultasi_medik.uraian_jawaban " +
                "from konsultasi_medik " +
                "inner join reg_periksa on konsultasi_medik.no_rawat=reg_periksa.no_rawat " +
                "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis " +
                "inner join penjab on reg_periksa.kd_pj=penjab.kd_pj " +
                ...
                "inner join jawaban_konsultasi_medik on jawaban_konsultasi_medik.no_permintaan=konsultasi_medik.no_permintaan " +
                "where " + whereDoctor +
                "konsultasi_medik.no_rawat like ? " +
                (TCari.getText().equals("") ? "" :
                 "and (konsultasi_medik.no_rawat like ? or reg_periksa.no_rkm_medis like ? or pasien.nm_pasien like ? " +
                 "or penjab.png_jawab like ? or konsultasi_medik.no_permintaan like ? or konsultasi_medik.jenis_permintaan like ?) ") +
                "order by konsultasi_medik.tanggal"
    ...
    try {
        ...
        for (File jpgFile : jpgFiles) {
            ...
            try {
                // Simpan ke database setiap file yang di-upload
                boolean uploadSuccess = false;
                kodeberkas2 = Sequel.cariIsi("SELECT kode FROM master_berkas_digital WHERE nama LIKE '%Laporan Tindakan%'");
                String filePath = "pages/upload/" + jpgFile.getName();
...
private void setupSplitPaneForWindowInput(){
    try{
        ...
        UraianKonsulPreview.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOSONAR
        ...
        // Remove old single-pane if it was already added by GUI builder
        try { internalFrame2.remove(scrollPane2); } catch (Throwable t) { /* ignore */ }
        
        // Build split pane: left = Uraian, right = existing scrollPane2 (Jawaban)
        splitUraianJawaban = new javax.swing.JSplitPane(javax.swing.JSplitPane.HORIZONTAL_SPLIT, scrollPaneUraianPreview, scrollPane2);
        splitUraianJawaban.setOneTouchExpandable(true);
        splitUraianJawaban.setContinuousLayout(true);
        splitUraianJawaban.setResizeWeight(0.5); // divide space equally on resize
        splitUraianJawaban.setName("splitUraianJawaban");
        
        internalFrame2.add(splitUraianJawaban);
        splitUraianJawaban.setBounds(20, 100, 700, 490); // final bounds are set in adjustWindowInputToAvailableScreen
        
        // Repurpose existing label jLabel42 to "Uraian Konsul :" (left)
        try { jLabel42.setText("Uraian Konsul :"); } catch (Throwable t) { /* ignore */ }
        
        // Add right-side label for "Jawaban Konsul :"
        ...
        labelJawabanKanan.setBounds(400, 70, 120, 23); // updated precisely during adjust method
        
        // Ensure left preview starts empty
