...
public final class DlgPembayaranPerAKunBayar3 extends javax.swing.JDialog {
    ...
    private PreparedStatement ps,psakunbayar,psDeposit;
    private ResultSet rs,rsakunbayar,rsDeposit;
    ...
    private StringBuilder htmlContent,htmlContent3;
    ...
    private void initComponents() {
        ...
        TabRawat.addTab("Model 3", Scroll2);
...
private void tampil3(){
    ...
    model.addColumn("No.Rawat/No.Nota");
    model.addColumn("Nama Pasien");
    model.addColumn("Jenis/Cara Bayar");
    model.addColumn("Pembayaran");
    model.addColumn("Petugas");
    model.addColumn("Akun Bayar");
    ...
    try {
        ...
        try {
            ...
            if (rsakunbayar != null) rsakunbayar.close();
            if (psakunbayar != null) psakunbayar.close();
        }
        ...
        try {
            ...
            while (rs.next()) {
                ...
                row.add(no);
                row.add(rs.getString("tgl_bayar"));
                row.add(nonota);
                row.add(rs.getString("nama_pasien"));
                row.add(carabayar);
                row.add(Valid.SetAngka(rs.getDouble("jumlah_bayar")));
                row.add(petugas);
                row.add(""); // Akun Bayar bisa diisi sesuai kebutuhan
                model.addRow(row);
        ...
        // Tambahkan total ke tabel
        for (i = 0; i < kolom; i++) {
            if (totalbayar[i] > 0) {
                Vector<Object> row = new Vector<>();
                row.add("");
                row.add("Total " + namabayar[i]);
                row.add("");
                row.add("");
                row.add("");
                row.add("");
                row.add("");
                row.add(Valid.SetAngka(totalbayar[i]));
                model.addRow(row);
            }
        }
        ...
        if (all > 0) {
            Vector<Object> row = new Vector<>();
            row.add("");
            row.add("Jumlah Total");
            row.add("");
            row.add("");
            row.add("");
            row.add("");
            row.add("");
            row.add(Valid.SetAngka(all));
            model.addRow(row);
        }

        // Tampilkan tabel di JScrollPane
//        Scroll2 scrollPane = new Scroll2(table);
        // Tambahkan scrollPane ke frame atau panel Anda
        // Contoh: this.add(scrollPane);
