 ...
 * @author perpustakaan
 */
public final class DlgPembayaranLaboratoriumRalan extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgPembayaranLaboratoriumRalan(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        for (int i = 0; i < cm.getColumnCount(); i++) {
            ...
            } else if ("Unit Poli".equalsIgnoreCase(name)) {
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgPembayaranLaboratoriumRalan dialog = new DlgPembayaranLaboratoriumRalan(new javax.swing.JFrame(), true);
...
//          + "WHERE rp.status_lanjut='Ralan' and rp.status_bayar='Sudah Bayar' "
...
//        }
//
//        // SELECT: tambahkan kolom laboratorium & tetap filter > 0
//        String sql =
//            "SELECT " +
//            "  rp.tgl_registrasi        AS tgl_ralan, " +
//            "  nj.no_nota               AS no_nota, " +
//            "  rp.no_rkm_medis          AS no_rkm_medis, " +
//            "  p.nm_pasien              AS nm_pasien, " +
//            "  ts.tgl_bayar             AS tgl_bayar, " +
//            "  pol.nm_poli              AS nm_poli, " +
...
//            "LEFT JOIN tagihan_sadewa ts ON ts.no_nota  = rp.no_rawat " +  // Tgl. Bayar
//            "LEFT JOIN poliklinik pol    ON pol.kd_poli = rp.kd_poli " +
//            "LEFT JOIN dokter d          ON d.kd_dokter = rp.kd_dokter " +
//            "LEFT JOIN penjab pj         ON pj.kd_pj = rp.kd_pj " +
//            "LEFT JOIN ( " +
...
//            "WHERE COALESCE(lab.lab_total,0) > 0 " +   // hanya baris dengan nilai lab
...
//                int c = 0;
//                row[c++] = rs.getString("tgl_ralan");                // Tgl. Ralan
//                row[c++] = nz(rs.getString("no_nota"));              // No. Nota
//                row[c++] = rs.getString("no_rkm_medis");             // No. RM
//                row[c++] = rs.getString("nm_pasien");                // Nama Pasien
//                row[c++] = nz(rs.getString("tgl_bayar"));            // Tgl. Bayar
//                row[c++] = rs.getString("nm_poli");                  // Unit Poli
//                row[c++] = df.format(rs.getDouble("laboratorium"));  // Laboratorium (format ribuan)
...
//                int c = 0;
//
//                double labVal = rs.getDouble("laboratorium"); // ambil nilai mentah
//                totalLaboratorium += labVal;                  // akumulasikan total
//
//                row[c++] = rs.getString("tgl_ralan");                // Tgl. Ralan
//                row[c++] = nz(rs.getString("no_nota"));              // No. Nota
//                row[c++] = rs.getString("no_rkm_medis");             // No. RM
...
//                row[c++] = nz(rs.getString("tgl_bayar"));            // Tgl. Bayar
//                row[c++] = rs.getString("nm_poli");                  // Unit Poli
//                row[c++] = df.format(labVal);                        // Laboratorium (format ribuan)
//                row[c++] = rs.getString("cara_bayar");               // Cara Bayar
//                row[c++] = rs.getString("dokter");                   // Dokter
//                row[c++] = rs.getString("status_bayar");             // Status Bayar
//                tabMode.addRow(row);
...
//        if (tabMode.getRowCount() > 0) {
//            // opsional: baris kosong pemisah
////            tabMode.addRow(new Object[]{ "", "", "", "", "", "", "", "", "", "" });
//
//            // baris TOTAL: label di kolom "Unit Poli" (index 5), jumlah di kolom "Laboratorium" (index 6)
...
//            totalRow[5] = "TOTAL"; // label
//            totalRow[6] = new DecimalFormat("#,##0").format(totalLaboratorium); // jumlah lab
...
//    }
    ...
    try {
        ...
        // tmp_rp: kumpulan no_rawat Ralan yang Sudah Bayar sesuai filter
        try (Statement st = koneksi.createStatement()) {
            st.execute("DROP TEMPORARY TABLE IF EXISTS tmp_rp");
        }
        String createTmp =
        ...
        // ===== SQL utama: Laboratorium = PK/MB + PA =====
        String sql =
            "SELECT " +
            "  rp.tgl_registrasi          AS tgl_ralan, " +
            "  nj.no_nota                 AS no_nota, " +
            ...
            "JOIN pasien p               ON p.no_rkm_medis = rp.no_rkm_medis " +
            "LEFT JOIN nota_jalan nj     ON nj.no_rawat = rp.no_rawat " +
            "LEFT JOIN tagihan_sadewa ts ON ts.no_nota  = rp.no_rawat " +  // tanggal bayar
            "LEFT JOIN poliklinik pol    ON pol.kd_poli = rp.kd_poli " +
            "LEFT JOIN dokter d          ON d.kd_dokter = rp.kd_dokter " +
            "LEFT JOIN penjab pj         ON pj.kd_pj = rp.kd_pj " +
            "LEFT JOIN ( " +
            "   /* Total laboratorium per no_rawat = PK/MB (detail) + PA (header total_byr) */ " +
            ...
            ") lab ON lab.no_rawat = rp.no_rawat " +
            "WHERE COALESCE(lab.lab_total,0) > 0 " +   // tampilkan hanya yang ada nilai Lab
            "GROUP BY rp.no_rawat " +
            "ORDER BY rp.tgl_registrasi ASC";

        final java.text.DecimalFormat df = new java.text.DecimalFormat("#,##0");
        double totalLaboratorium = 0.0;
        ...
        try (PreparedStatement ps = koneksi.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                totalLaboratorium += labVal;

                row[c++] = rs.getString("tgl_ralan");
        ...
        if (tabMode.getRowCount() > 0) {
            ...
            totalRow[6] = new java.text.DecimalFormat("#,##0").format(totalLaboratorium);
...
public void tampil2() {
    ...
    try {
        ...
        String createTmp =
          ...
          + "  AND (SELECT MIN(ki2.tgl_masuk) FROM kamar_inap ki2 WHERE ki2.no_rawat = rp.no_rawat) BETWEEN ? AND ? "
        ...
        try (PreparedStatement psTmp = koneksi.prepareStatement(createTmp)) {
            psTmp.setString(1, tgl1);
            psTmp.setString(2, tgl2);
            psTmp.setString(3, "%" + KdPoli.getText() + "%");      // reuse field to filter bangsal
            psTmp.setString(4, "%" + KdDokter.getText() + "%");
            psTmp.setString(5, "%" + KdCaraBayar.getText() + "%");
            psTmp.setString(6, "%" + TCari.getText() + "%");
            psTmp.setString(7, "%" + TCari.getText() + "%");
        ...
        String sql =
            ...
            "LEFT JOIN tagihan_sadewa ts  ON ts.no_nota  = rp.no_rawat " +
            "LEFT JOIN dokter d           ON d.kd_dokter = rp.kd_dokter " +
            "LEFT JOIN penjab pj          ON pj.kd_pj = rp.kd_pj " +
            "LEFT JOIN ( " +
            "   SELECT dpl.no_rawat, SUM(dpl.biaya_item) AS lab_total " +
        ...
        try (PreparedStatement ps = koneksi.prepareStatement(sql);
            ...
            while (rs.next()) {
                Object[] row = new Object[10];
                int c = 0;
                row[c++] = rs.getString("tgl_ranap");                 // Tgl. Ranap (ditampilkan di kolom "Tgl. Ralan")
                row[c++] = nz(rs.getString("no_nota"));               // No. Nota (nota_inap)
                row[c++] = rs.getString("no_rkm_medis");              // No. RM
                row[c++] = rs.getString("nm_pasien");                 // Nama Pasien
                row[c++] = nz(rs.getString("tgl_bayar"));             // Tgl. Bayar
                row[c++] = rs.getString("nm_bangsal");                // Unit (Bangsal)
        ...
        System.out.println("Notif tampil2: " + e);
        try (Statement st2 = koneksi.createStatement()) {
            st2.execute("DROP TEMPORARY TABLE IF EXISTS tmp_rp");
        } catch (Exception ignore) {}
    }
