...
public class DlgBilingRanap extends javax.swing.JDialog {
    private final DefaultTableModel tabModeRwJlDr,tabModeTambahan,tabModePotongan,tabModeKamIn,tabModeAkunBayar,tabModeAkunPiutang,tabModeLab,tabModeRad,tabModeApotek,tabModePermintaan;
    //Dialog pendukung dibuat saat pertama kali dipakai agar form utama cepat tampil.
    public DlgPemberianObat beriobat;
    public DlgRawatInap rawatinap;
    public DlgDeposit deposit;
    public DlgCariCaraBayar carabayar;
    ...
    /*
     * Clinical Service Board - Status Permintaan
     * Komponen dibuat secara programatik agar file .form lama tetap aman.
     * Tiga tabel/model lama tetap menjadi sumber data sehingga query dan
    * fungsi Laboratorium, Radiologi, serta Resep tidak berubah.
     */
    private JTable tbStatusPermintaan;
    private JPanel panelDaftarPermintaan;
    private CardLayout layoutDaftarPermintaan;
    private TableRowSorter<DefaultTableModel> sorterPermintaan;
    ...
    private String biaya="",tambahan="",totals="",norawatbayi="",centangdokterranap="",kd_pj="",
            ...
            sqlpsobatoperasi_monitor="select obatbhp_ok.nm_obat,beri_obat_operasi.hargasatuan,sum(beri_obat_operasi.jumlah) as jumlah, "+
                    "sum(beri_obat_operasi.hargasatuan*beri_obat_operasi.jumlah) as total "+
                    "from obatbhp_ok inner join beri_obat_operasi "+
                    "on beri_obat_operasi.kd_obat=obatbhp_ok.kd_obat where "+
                    "beri_obat_operasi.no_rawat=? and beri_obat_operasi.kd_obat in ('OK00003133','OK00003134','OK00003135','OK00003136','OK00003137','OK00003138','OK00003139','OK00003140','OK00003142','OK00003143','OK00003144','altdkr01','AltOK01','AltOK02','AltOK03','OK00003146') group by beri_obat_operasi.kd_obat",
            sqlpsobatoperasi_nonmonitor="select obatbhp_ok.nm_obat,beri_obat_operasi.hargasatuan,sum(beri_obat_operasi.jumlah) as jumlah, "+
                    "sum(beri_obat_operasi.hargasatuan*beri_obat_operasi.jumlah) as total "+
                    "from obatbhp_ok inner join beri_obat_operasi "+
                    "on beri_obat_operasi.kd_obat=obatbhp_ok.kd_obat where "+
                    "beri_obat_operasi.no_rawat=? and beri_obat_operasi.kd_obat not in ('OK00003133','OK00003134','OK00003135','OK00003136','OK00003137','OK00003138','OK00003139','OK00003140','OK00003142','OK00003143','OK00003144','altdkr01','AltOK01','AltOK02','AltOK03','OK00003146') group by beri_obat_operasi.kd_obat",
            ...
            sqlpsranapdokter="SELECT concat(jns_perawatan_inap.nm_perawatan, \" - \",  dokter.nm_dokter) as nm_perawatan,rawat_inap_dr.biaya_rawat as total_byrdr,count(rawat_inap_dr.kd_jenis_prw) as jml, "+
    ...
    private double ttl=0,y=0,subttl=0,lab,ttlobat,ttlretur,ppnobat,piutang=0,kekurangan=0,itembayar=0,itempiutang=0, 
            ...
            ttlLaborat=0,ttlRadiologi=0,ttlOperasi=0,ttlObat=0,ttlObatOperasi=0,ttlRanap_Dokter=0,ttlRanap_Paramedis=0,ttlRalan_Dokter=0,
            ...
            persenbayi=0;
    private int x=0,z=0,i=0,countbayar=0,jml=0,r=0,row2=0,cek;
    ...
    private boolean sukses=false,persenBayiDimuat=false;
    private File file;
    ...
    public DlgBilingRanap(java.awt.Frame parent, boolean modal) {
        ...
        /*
         * Model gabungan hanya untuk tampilan Clinical Service Board.
         * Isi model tetap disalin dari tabModeLab, tabModeRad dan
         * tabModeApotek setelah proses query lama selesai dijalankan.
         */
        tabModePermintaan=new DefaultTableModel(null,new Object[]{
            "Jenis Layanan","No. Permintaan/Resep","Tanggal & Jam","Dokter","Status"
            }){
              @Override public boolean isCellEditable(int rowIndex, int colIndex){return false;}
        };
        inisialisasiClinicalServiceBoard();
        inisialisasiBadgeTabRawat();
        //Dialog obat/rawat inap/deposit/cara bayar tidak lagi dibuat di constructor.
        //Listener masing-masing dipasang oleh getter lazy saat dialog pertama dipakai.
        ...
        try {
            ...
            try {
                ...
                System.out.println("Notifikasi : "+e+e.getStackTrace()[0].getLineNumber());
            } finally{
                if(rsrekening!=null){
                    rsrekening.close();
                }
    ...
    // atau:
    // TabRawat.setBorder(javax.swing.BorderFactory.createEmptyBorder());

    TabRawat.setFont(new java.awt.Font("Tahoma", java.awt.Font.BOLD, 12));

    TabRawat.setUI(new javax.swing.plaf.basic.BasicTabbedPaneUI() {
        @Override
        ...
        protected void paintContentBorder(java.awt.Graphics g, int tabPlacement, int selectedIndex) {
            // kosongkan supaya tidak ada garis kotak area content
        }

        @Override
        protected void paintTabBorder(java.awt.Graphics g, int tabPlacement, int tabIndex,
                int x, int y, int w, int h, boolean isSelected) {
            // kosongkan supaya antar tab tidak ada garis border
        }
    ...
    //Ikon bawaan tab disamakan dengan ikon pada badge tab kustom.
    TabRawat.setIconAt(0, IKON_TAB_TAGIHAN);
    TabRawat.setIconAt(1, IKON_TAB_PEMBAYARAN);
    TabRawat.setIconAt(2, IKON_TAB_STATUS);
    }

    //================ LAZY INITIALIZATION DIALOG PENDUKUNG ================
    //Dialog besar hanya dibuat ketika benar-benar dibuka oleh pengguna.
    //Semua listener lama tetap dipasang satu kali saat objek dibuat.
    private DlgPemberianObat getBeriObat(){
        if(beriobat==null){
            final DlgPemberianObat dialog=new DlgPemberianObat(null,false);
            java.awt.event.WindowAdapter perbaruiBilling=new java.awt.event.WindowAdapter(){
                @Override
            ...
            dialog.dlgobt.addWindowListener(perbaruiBilling);
            dialog.addWindowListener(perbaruiBilling);
            beriobat=dialog;
        }
        return beriobat;
    }

    private DlgRawatInap getRawatInap(){
        if(rawatinap==null){
            final DlgRawatInap dialog=new DlgRawatInap(null,false);
            dialog.perawatan.addWindowListener(new java.awt.event.WindowAdapter(){
                @Override
                public void windowClosed(WindowEvent e){
                    if(akses.getform().equals("DLgBilingRanap")){
                        isRawat();
    ...
    private DlgDeposit getDeposit(){
        if(deposit==null){
            final DlgDeposit dialog=new DlgDeposit(null,false);
    ...
    private DlgCariCaraBayar getCaraBayar(){
        if(carabayar==null){
            final DlgCariCaraBayar dialog=new DlgCariCaraBayar(null,false);
            dialog.addWindowListener(new java.awt.event.WindowAdapter(){
                @Override
                public void windowClosed(WindowEvent e){
                    if(akses.getform().equals("DLgBilingRanap")){
                        if(dialog.getTable().getSelectedRow()!=-1){
                            int baris=dialog.getTable().getSelectedRow();
                            kdpenjab.setText(dialog.getTable().getValueAt(baris,1).toString());
                            nmpenjab.setText(dialog.getTable().getValueAt(baris,2).toString());
                        }
                        kdpenjab.requestFocus();
                    }
                }
            });
            dialog.getTable().addKeyListener(new KeyListener(){
                @Override public void keyTyped(KeyEvent e){}
                @Override
                public void keyPressed(KeyEvent e){
                    if(akses.getform().equals("DLgBilingRanap")&&e.getKeyCode()==KeyEvent.VK_SPACE){
                        dialog.dispose();
                    }
                }
    ...
    //============== AKHIR LAZY INITIALIZATION DIALOG PENDUKUNG ==============
    ...
    private void initComponents() {
        ...
        MnTambahAdm = new javax.swing.JMenuItem();
        ...
        MnTambahAdm.setBackground(new java.awt.Color(255, 255, 254));
        MnTambahAdm.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        MnTambahAdm.setForeground(new java.awt.Color(50, 50, 50));
        MnTambahAdm.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        MnTambahAdm.setText("Tambah Adm");
        MnTambahAdm.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        MnTambahAdm.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        MnTambahAdm.setName("MnTambahAdm"); // NOI18N
        MnTambahAdm.setPreferredSize(new java.awt.Dimension(250, 28));
        MnTambahAdm.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MnTambahAdmActionPerformed(evt);
            }
        });
        jPopupMenu1.add(MnTambahAdm);
        ...
        WindowInput6.setTitle("Ganti Jenis Bayar");
        WindowInput6.setMinimumSize(new java.awt.Dimension(650, 130));
        WindowInput6.setName("WindowInput6"); // NOI18N
        WindowInput6.setResizable(false);
        WindowInput6.setSize(new java.awt.Dimension(650, 130));
        WindowInput6.setType(java.awt.Window.Type.POPUP);
        ...
        internalFrame7.setMinimumSize(new java.awt.Dimension(650, 130));
        ...
        DTPTgl.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "22-04-2026 08:14:24" }));
        ...
        BtnNota.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/invoice (2).png"))); // NOI18N
        ...
        BtnNota.setPreferredSize(new java.awt.Dimension(90, 30));
        ...
        TtlSemua.setHorizontalAlignment(javax.swing.JTextField.RIGHT);
        TtlSemua.setText("0");
        TtlSemua.setFont(new java.awt.Font("Segoe UI Semibold", 1, 18)); // NOI18N
        ...
        jLabel15.setBounds(340, 60, 150, 23);

        TagihanPPn.setEditable(false);
        TagihanPPn.setHorizontalAlignment(javax.swing.JTextField.RIGHT);
        TagihanPPn.setText("0");
        TagihanPPn.setFont(new java.awt.Font("Segoe UI Semibold", 1, 18)); // NOI18N
        TagihanPPn.setHighlighter(null);
        TagihanPPn.setName("TagihanPPn"); // NOI18N
        panelBayar.add(TagihanPPn);
        TagihanPPn.setBounds(1120, 60, 160, 26);
        ...
        scrollPane4.setBounds(20, 170, 627, 210);
        ...
        jLabel6.setBounds(330, 390, 130, 23);
        ...
        scrollPane5.setBounds(680, 170, 627, 210);
        ...
        TCari.setBounds(130, 130, 430, 26);
        ...
        BtnCariBayar.setBounds(570, 130, 25, 23);
        ...
        TCari1.setBounds(800, 130, 430, 26);
        ...
        btnCariPiutang.setBounds(1240, 130, 25, 23);
        ...
        jLabel13.setBounds(20, 390, 110, 23);

        Deposit.setEditable(false);
        Deposit.setHorizontalAlignment(javax.swing.JTextField.RIGHT);
        Deposit.setFont(new java.awt.Font("Segoe UI Semibold", 1, 18)); // NOI18N
        Deposit.setName("Deposit"); // NOI18N
        panelBayar.add(Deposit);
        Deposit.setBounds(130, 390, 160, 26);
        ...
        BtnSeek2.setBounds(300, 390, 25, 23);

        ChkPiutang.setText("Piutang");
        ChkPiutang.setFont(new java.awt.Font("Segoe UI Semibold", 1, 24)); // NOI18N
        ChkPiutang.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        ChkPiutang.setIconTextGap(10);
        ...
        chkRanap.setBounds(150, 20, 95, 23);
        ...
        jLabel18.setBounds(970, 60, 150, 23);
        ...
        BtnAll.setBounds(600, 130, 25, 23);
        ...
        BtnAll1.setBounds(1270, 130, 25, 23);
        ...
        panelPermintaan.setLayout(new java.awt.GridLayout(2, 2));

        scrollPane6.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(255, 255, 255)), "1. Permintaan Laborat : ", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Segoe UI Semibold", 1, 14), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        scrollPane8.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(255, 255, 255)), "3. Permintaan Resep : ", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Segoe UI Semibold", 1, 14), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        scrollPane7.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(255, 255, 255)), "2. Permintaan Radiologi : ", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Segoe UI Semibold", 1, 14), new java.awt.Color(50, 50, 50))); // NOI18N
                        ...
                        if(tbBilling.getValueAt(tbBilling.getSelectedRow(), kolom).toString().contains("Rincian Biaya")){
                            if(akses.gettindakan_ranap()==true){
                                if(Sequel.cariRegistrasi(TNoRw.getText())>0){
                                    ...
                                    getRawatInap().perawatan.setNoRm(TNoRw.getText(),"rawat_inap_dr",DTPTgl.getDate(),"00","00","00",true,TPasien.getText());
                                    getRawatInap().perawatan.setPetugas("","","","");
                                    getRawatInap().perawatan.isCek();
                                    getRawatInap().perawatan.tampil();
                                    getRawatInap().perawatan.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
                                    getRawatInap().perawatan.setLocationRelativeTo(internalFrame1);
                                    getRawatInap().perawatan.setVisible(true);
                                }
                            }                        
                            ...
                            if(akses.getberi_obat()==true){
                                if(Sequel.cariRegistrasi(TNoRw.getText())>0){
                                    ...
                                    getBeriObat().dlgobt.setNoRm(TNoRw.getText(),TNoRM.getText(),TPasien.getText(),DTPTgl.getDate(),"00","00","00",true);
                                    getBeriObat().dlgobt.isCek();
                                    getBeriObat().dlgobt.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
                                    getBeriObat().dlgobt.tampil();
                                    getBeriObat().dlgobt.setLocationRelativeTo(internalFrame1);
                                    getBeriObat().dlgobt.setVisible(true);
                                }
                            }                        
...
private void MnRawatInapActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnRawatInapActionPerformed
       if(TPasien.getText().trim().equals("")){
           ...
           akses.setform("DLgBilingRanap");
            getRawatInap().setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
            getRawatInap().setLocationRelativeTo(internalFrame1);
            getRawatInap().isCek();
            getRawatInap().setNoRm(TNoRw.getText(),DTPTgl.getDate(),new Date());    
            getRawatInap().tampilDr();
            getRawatInap().setVisible(true);
        }
}//GEN-LAST:event_MnRawatInapActionPerformed

private void MnTambahAdmActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnTambahAdmActionPerformed
    String kdJenisPrw="adm013";
    PreparedStatement psAdm=null, psAkunAdm=null;
    ResultSet rsAdm=null, rsAkunAdm=null;
    ...
    String kdPjPasien=Sequel.cariIsi(
            "select kd_pj from reg_periksa where no_rawat=?",TNoRw.getText());

    if(kdPjPasien.equals("150")){
        kdJenisPrw="adm011";
    }else if(kdPjPasien.equals("001")){
        String kelasKamar=Sequel.cariIsi(
                "select kamar.kelas from kamar_inap inner join kamar "+
                "on kamar_inap.kd_kamar=kamar.kd_kamar "+
                "where kamar_inap.no_rawat=? "+
                "order by kamar_inap.tgl_masuk desc,kamar_inap.jam_masuk desc limit 1",
                TNoRw.getText()).trim();

        if(kelasKamar.equals("Kelas VVIP")){
            kdJenisPrw="adm006";
        }else if(kelasKamar.equals("Kelas VIP")){
            kdJenisPrw="adm007";
        }else if(kelasKamar.equals("Kelas 1")){
            kdJenisPrw="adm008";
        }else if(kelasKamar.equals("Kelas 2")){
            kdJenisPrw="Adm009";
        }else if(kelasKamar.equals("Kelas 3")){
            kdJenisPrw="Adm010";
        }else{
            JOptionPane.showMessageDialog(rootPane,
                    "Maaf, kelas kamar pasien Umum tidak ditemukan atau belum memiliki mapping administrasi.\n"+
                    "Kelas kamar terbaca : "+(kelasKamar.equals("")?"-":kelasKamar));
            return;
        }
    }

    String nipPetugas=akses.getkode();
    String namaPetugas=Sequel.cariIsi("select nama from petugas where nip=?",nipPetugas);
    if(namaPetugas.trim().equals("")){
        JOptionPane.showMessageDialog(rootPane,"Maaf, akun login "+nipPetugas+" tidak terdaftar sebagai petugas...!!!");
        return;
    }

    String nmPerawatan="",material="0",bhp="0",tarifPetugas="0",kso="0",menejemen="0",totalBiaya="0";
    String suspenPiutangTindakan="",pendapatanTindakan="",bebanJasaParamedis="",utangJasaParamedis="",
           bebanKso="",utangKso="",bebanJasaSarana="",utangJasaSarana="",hppBhp="",persediaanBhp="",
           bebanJasaMenejemen="",utangJasaMenejemen="";

    try{
        psAdm=koneksi.prepareStatement(
            "select nm_perawatan,material,bhp,tarif_tindakanpr,kso,menejemen,total_byrpr "+
            "from jns_perawatan_inap where kd_jenis_prw=?");
        psAdm.setString(1,kdJenisPrw);
        rsAdm=psAdm.executeQuery();
        if(rsAdm.next()){
            nmPerawatan=rsAdm.getString("nm_perawatan");
            material=rsAdm.getString("material");
            bhp=rsAdm.getString("bhp");
            tarifPetugas=rsAdm.getString("tarif_tindakanpr");
            kso=rsAdm.getString("kso");
            menejemen=rsAdm.getString("menejemen");
            totalBiaya=rsAdm.getString("total_byrpr");
        }else{
            JOptionPane.showMessageDialog(rootPane,"Tindakan dengan kode "+kdJenisPrw+" tidak ditemukan di master Tindakan Rawat Inap...!!!");
            return;
        }
    }catch(Exception e){
        JOptionPane.showMessageDialog(rootPane,"Gagal membaca tindakan "+kdJenisPrw+" : "+e);
        return;
    }finally{
        try{if(rsAdm!=null) rsAdm.close();}catch(Exception e){}
        try{if(psAdm!=null) psAdm.close();}catch(Exception e){}
    }

    try{
        psAkunAdm=koneksi.prepareStatement(
            "select Suspen_Piutang_Tindakan_Ranap,Tindakan_Ranap,"+
            ...
            "Beban_Jasa_Menejemen_Tindakan_Ranap,Utang_Jasa_Menejemen_Tindakan_Ranap from set_akun_ranap");
        rsAkunAdm=psAkunAdm.executeQuery();
        if(rsAkunAdm.next()){
            suspenPiutangTindakan=rsAkunAdm.getString("Suspen_Piutang_Tindakan_Ranap");
            pendapatanTindakan=rsAkunAdm.getString("Tindakan_Ranap");
            bebanJasaParamedis=rsAkunAdm.getString("Beban_Jasa_Medik_Paramedis_Tindakan_Ranap");
            utangJasaParamedis=rsAkunAdm.getString("Utang_Jasa_Medik_Paramedis_Tindakan_Ranap");
            bebanKso=rsAkunAdm.getString("Beban_KSO_Tindakan_Ranap");
            utangKso=rsAkunAdm.getString("Utang_KSO_Tindakan_Ranap");
            bebanJasaSarana=rsAkunAdm.getString("Beban_Jasa_Sarana_Tindakan_Ranap");
            utangJasaSarana=rsAkunAdm.getString("Utang_Jasa_Sarana_Tindakan_Ranap");
            hppBhp=rsAkunAdm.getString("HPP_BHP_Tindakan_Ranap");
            persediaanBhp=rsAkunAdm.getString("Persediaan_BHP_Tindakan_Ranap");
            bebanJasaMenejemen=rsAkunAdm.getString("Beban_Jasa_Menejemen_Tindakan_Ranap");
            utangJasaMenejemen=rsAkunAdm.getString("Utang_Jasa_Menejemen_Tindakan_Ranap");
        }else{
            JOptionPane.showMessageDialog(rootPane,"Setting akun rawat inap belum tersedia...!!!");
            return;
        }
    }catch(Exception e){
        JOptionPane.showMessageDialog(rootPane,"Gagal membaca setting akun rawat inap : "+e);
        return;
    }finally{
        try{if(rsAkunAdm!=null) rsAkunAdm.close();}catch(Exception e){}
        try{if(psAkunAdm!=null) psAkunAdm.close();}catch(Exception e){}
    }

    LocalDateTime sekarang=LocalDateTime.now();
    String tglPerawatan=sekarang.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
    String jamRawat=sekarang.format(DateTimeFormatter.ofPattern("HH:mm:ss"));

    Sequel.AutoComitFalse();
    boolean suksesAdm=true;
    boolean berhasilCommit=false;
    try{
        if(Sequel.menyimpantf("rawat_inap_pr","?,?,?,?,?,?,?,?,?,?,?","Data",11,new String[]{
            TNoRw.getText(),kdJenisPrw,nipPetugas,tglPerawatan,jamRawat,
            material,bhp,tarifPetugas,kso,menejemen,totalBiaya
        })==true){
            Sequel.queryu("delete from tampjurnal");

            if(Valid.SetAngka(totalBiaya)>0){
                Sequel.menyimpan("tampjurnal","'"+suspenPiutangTindakan+"','Suspen Piutang Tindakan Ranap','"+totalBiaya+"','0'","debet=debet+'"+totalBiaya+"'","kd_rek='"+suspenPiutangTindakan+"'");
                Sequel.menyimpan("tampjurnal","'"+pendapatanTindakan+"','Pendapatan Tindakan Rawat Inap','0','"+totalBiaya+"'","kredit=kredit+'"+totalBiaya+"'","kd_rek='"+pendapatanTindakan+"'");
            }
            if(Valid.SetAngka(tarifPetugas)>0){
                Sequel.menyimpan("tampjurnal","'"+bebanJasaParamedis+"','Beban Jasa Medik Paramedis Tindakan Ranap','"+tarifPetugas+"','0'","debet=debet+'"+tarifPetugas+"'","kd_rek='"+bebanJasaParamedis+"'");
                Sequel.menyimpan("tampjurnal","'"+utangJasaParamedis+"','Utang Jasa Medik Paramedis Tindakan Ranap','0','"+tarifPetugas+"'","kredit=kredit+'"+tarifPetugas+"'","kd_rek='"+utangJasaParamedis+"'");
            }
            if(Valid.SetAngka(kso)>0){
                Sequel.menyimpan("tampjurnal","'"+bebanKso+"','Beban KSO Tindakan Ranap','"+kso+"','0'","debet=debet+'"+kso+"'","kd_rek='"+bebanKso+"'");
                Sequel.menyimpan("tampjurnal","'"+utangKso+"','Utang KSO Tindakan Ranap','0','"+kso+"'","kredit=kredit+'"+kso+"'","kd_rek='"+utangKso+"'");
            }
            if(Valid.SetAngka(material)>0){
                Sequel.menyimpan("tampjurnal","'"+bebanJasaSarana+"','Beban Jasa Sarana Tindakan Ranap','"+material+"','0'","debet=debet+'"+material+"'","kd_rek='"+bebanJasaSarana+"'");
                Sequel.menyimpan("tampjurnal","'"+utangJasaSarana+"','Utang Jasa Sarana Tindakan Ranap','0','"+material+"'","kredit=kredit+'"+material+"'","kd_rek='"+utangJasaSarana+"'");
            }
            if(Valid.SetAngka(bhp)>0){
                Sequel.menyimpan("tampjurnal","'"+hppBhp+"','HPP BHP Tindakan Ranap','"+bhp+"','0'","debet=debet+'"+bhp+"'","kd_rek='"+hppBhp+"'");
                Sequel.menyimpan("tampjurnal","'"+persediaanBhp+"','Persediaan BHP Tindakan Ranap','0','"+bhp+"'","kredit=kredit+'"+bhp+"'","kd_rek='"+persediaanBhp+"'");
            }
            if(Valid.SetAngka(menejemen)>0){
                Sequel.menyimpan("tampjurnal","'"+bebanJasaMenejemen+"','Beban Jasa Menejemen Tindakan Ranap','"+menejemen+"','0'","debet=debet+'"+menejemen+"'","kd_rek='"+bebanJasaMenejemen+"'");
                Sequel.menyimpan("tampjurnal","'"+utangJasaMenejemen+"','Utang Jasa Menejemen Tindakan Ranap','0','"+menejemen+"'","kredit=kredit+'"+menejemen+"'","kd_rek='"+utangJasaMenejemen+"'");
            }
        ...
        if(suksesAdm==true){
            ...
            Sequel.RollBack();
            JOptionPane.showMessageDialog(rootPane,"Terjadi kesalahan saat menambahkan Adm.\nTransaksi dibatalkan dan tidak ada data yang disimpan...!!!");
        }
    }catch(Exception e){
        suksesAdm=false;
        Sequel.RollBack();
        JOptionPane.showMessageDialog(rootPane,"Terjadi kesalahan saat menambahkan Adm : "+e+"\nTransaksi dibatalkan...!!!");
    }finally{
    ...
    if(berhasilCommit){
        isRawat();
        JOptionPane.showMessageDialog(rootPane,"Tambah Adm berhasil.\n"+nmPerawatan+" ("+kdJenisPrw+") telah ditambahkan atas nama "+namaPetugas+".");
    }
}//GEN-LAST:event_MnTambahAdmActionPerformed
...
private void MnHapusTagihanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnHapusTagihanActionPerformed
    ...
    String kd_pj_pasien = Sequel.cariIsi("SELECT kd_pj FROM reg_periksa WHERE no_rawat=?", TNoRw.getText());

//    // Jika pasien kd_pj = '001', maka hanya user 19890776 yang boleh
//    if (kd_pj_pasien.equals("001") && !akses.getkode().equals("19890776") && !akses.getkode().equals("Admin Utama")) {
//        JOptionPane.showMessageDialog(null, 
//            "Akses ditolak!\nPasien Umum hanya bisa dibuka oleh Kabid. Keuangan.");
//        return;
//    }

    // Jika pasien BUKAN penjamin BPJ,
    // maka hanya user dengan status=5 di dokter_mpp + Admin Utama yang boleh
    if (!"BPJ".equals(kd_pj_pasien)) {

        // Cek apakah user adalah Kabid Keuangan
        boolean isKabidKeuangan = Sequel.cariInteger(
                "SELECT COUNT(*) FROM dokter_mpp WHERE status='5' AND kd_dokter=?",
                akses.getkode()
        ) > 0;

        // Selain Kabid Keuangan dan Admin Utama, akses ditolak
        if (!isKabidKeuangan && !"Admin Utama".equals(akses.getkode())) {
            JOptionPane.showMessageDialog(
                    null,
                    "Akses ditolak!\nPasien selain BPJS hanya bisa dibuka oleh Kabid. Keuangan."
            );
            return;
        }
    }
	...
	try{
        ...
        if(i>0){
            ...
            // ===================== DI SINI KITA CATAT LOG HAPUS NOTA =====================
                if (sukses == true) {
                    PreparedStatement psLog = null;
                    try {
                        // ambil no_nota dari nota_jalan SEBELUM dihapus
                        ...
                        // Mendapatkan nama petugas / user
                        String petugas = akses.getkode();

                        // Insert data ke tabel log
                        psLog = koneksi.prepareStatement(
                                "INSERT INTO rsaj_hapus_nota_salah " +
                                "(no_rawat, no_nota, waktu, petugas, ip_komputer) " +
                                "VALUES (?,?,?,?,?)"
                        );
                        psLog.setString(1, TNoRw.getText());
                        psLog.setString(2, noNota);
                        psLog.setString(3, waktu);
                        psLog.setString(4, petugas);
                        psLog.setString(5, ipKomputer);
                        psLog.executeUpdate();

                    } catch (Exception e) {
                        // kalau mau lebih ketat, biar gagal log langsung batalkan semuanya:
                        sukses = false;
                        System.out.println("Notifikasi simpan log hapus nota salah : " + e);
                    } finally {
                        if (psLog != null) {
                            try { psLog.close(); } catch (Exception ex) {}
                        }
                    }
                }
                // ==============================================================================
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        //notif belum terlayani
//        }else if(Sequel.cariInteger("SELECT COUNT(rp.no_rawat) AS no_rw FROM reg_periksa rp WHERE rp.no_rawat = ? AND rp.biaya_reg = '0'",TNoRw.getText())>0){
//            JOptionPane.showMessageDialog(null,"Maaf, biaya registrasi masih 0...!!!");            
        }else if(Sequel.cariInteger("SELECT COUNT(pl.no_rawat) AS no_rw FROM permintaan_lab pl WHERE pl.no_rawat = ? AND pl.tgl_hasil = '0000-00-00' AND pl.jam_hasil = '00:00:00'",TNoRw.getText())>0){
            JOptionPane.showMessageDialog(null,"Maaf, masih ada permintaan lab PK yang belum terlayani...!!!");    
        }else if(Sequel.cariInteger("SELECT COUNT(pla.no_rawat) AS no_rw FROM permintaan_labpa pla WHERE pla.no_rawat = ? AND pla.tgl_hasil = '0000-00-00' AND pla.jam_hasil = '00:00:00'",TNoRw.getText())>0){
            JOptionPane.showMessageDialog(null,"Maaf, masih ada permintaan lab PA yang belum terlayani...!!!"); 
        }else if(Sequel.cariInteger("SELECT COUNT(plm.no_rawat) AS no_rw FROM permintaan_labmb plm WHERE plm.no_rawat = ? AND plm.tgl_hasil = '0000-00-00' AND plm.jam_hasil = '00:00:00'",TNoRw.getText())>0){
            JOptionPane.showMessageDialog(null,"Maaf, masih ada permintaan lab MB yang belum terlayani...!!!"); 
        }else if(Sequel.cariInteger("SELECT COUNT(plr.no_rawat) AS no_rw FROM permintaan_radiologi plr WHERE plr.no_rawat = ? AND plr.tgl_hasil = '0000-00-00' AND plr.jam_hasil = '00:00:00'",TNoRw.getText())>0){
            JOptionPane.showMessageDialog(null,"Maaf, masih ada permintaan Radiologi yang belum terlayani...!!!");
        }else if(Sequel.cariInteger("SELECT COUNT(ro.no_rawat) AS no_rw FROM resep_obat ro WHERE ro.no_rawat = ? AND ro.tgl_perawatan = '0000-00-00' AND ro.jam = '00:00:00'",TNoRw.getText())>0){
            JOptionPane.showMessageDialog(null,"Maaf, masih ada permintaan Resep yang belum terlayani...!!!");
        }else if(Sequel.cariInteger("SELECT COUNT(ki.no_rawat) AS no_rw FROM kamar_inap ki WHERE ki.no_rawat = ? AND ki.tgl_keluar = '0000-00-00' AND ki.jam_keluar = '00:00:00' AND ki.stts_pulang = '-'",TNoRw.getText())>0){
            JOptionPane.showMessageDialog(null,"Maaf, Status Pasien belum dipulangkan, silahkan menghubungi unit terkait...!!!");    
//        }else if(Sequel.cariInteger("SELECT COUNT(no_rawat) AS no_rw FROM bridging_surat_kontrol_bpjs WHERE no_rawat = ?", TNoRw.getText()) == 0) {
//            JOptionPane.showMessageDialog(null, "Maaf, SKDP belum diterbitkan...!!!");    
        //notif
        }else if(i==0){ 
            if(piutang<=0){
                if(kekurangan<0){
    ...
    private void btnPenjabActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPenjabActionPerformed
        akses.setform("DLgBilingRanap");
        getCaraBayar().isCek();
        getCaraBayar().setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
        getCaraBayar().setLocationRelativeTo(internalFrame1);
        getCaraBayar().setAlwaysOnTop(false);
        getCaraBayar().setVisible(true);
    }//GEN-LAST:event_btnPenjabActionPerformed

    private void BtnSeek2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSeek2ActionPerformed
            akses.setform("DLgBilingRanap");
            getDeposit().setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
            getDeposit().setLocationRelativeTo(internalFrame1);
            getDeposit().isCek();
            getDeposit().setNoRm(TNoRw.getText(),Valid.SetTgl2(tgl_registrasi),DTPTgl.getDate());
            getDeposit().setAlwaysOnTop(false);
            getDeposit().setVisible(true);
    }//GEN-LAST:event_BtnSeek2ActionPerformed

    private void MnDataObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDataObatActionPerformed
        akses.setform("DLgBilingRanap");
        getBeriObat().setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
        getBeriObat().setLocationRelativeTo(internalFrame1); 
        getBeriObat().isCek();
        getBeriObat().setNoRm(TNoRw.getText(),DTPTgl.getDate(),new Date(),"ranap"); 
        getBeriObat().setVisible(true);
    ...
    private void BtnNotaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnNotaActionPerformed
                // cek billing sudah tersimpan atau belum
        try {
            pscekbilling=koneksi.prepareStatement(sqlpscekbilling);
            try {
                pscekbilling.setString(1,TNoRw.getText());
        ...
        // Periksa status_bayar dan kd_pj
        String statusBayar = Sequel.cariIsi("select reg_periksa.status_bayar from reg_periksa where reg_periksa.no_rawat=?", TNoRw.getText());
        String kd_pj = Sequel.cariIsi("select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?", TNoRw.getText());

        if (statusBayar.equals("Belum Bayar") && kd_pj.equals("001")) {
            openNota();
            return;
        }        

//        // Jika kode pembayarannya '001', lewati validasi dan langsung buka nota
//        String kd_pj = Sequel.cariIsi("select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?", TNoRw.getText());
//        if (kd_pj.equals("001")) {
//            openNota();
//            return;
//        }
        ...
        if (cek == 0){
            JOptionPane.showMessageDialog(null,"Nota belum tersimpan, tidak bisa mencetak. Simpan nota dulu sebelum mencetak!");
            return;
        }
...
i=Sequel.cariInteger("select count(billing.no_rawat) from billing where billing.no_rawat=?",TNoRw.getText());
            ...
            try{
                ...
                if(i>0){                       
                    ...
                    if(i==1){
                        ttl=(ttlLaborat+ttlRadiologi+ttlOperasi+ttlObat+ttlRanap_Dokter+ttlRanap_Paramedis+ttlRalan_Dokter+
                                ttlRalan_Paramedis+ttlTambahan+ttlKamar+ttlRegistrasi+ttlHarian+ttlRetur_Obat+ttlResep_Pulang+ttlService);
                        Valid.panggilUrl("billing/LaporanBilling2A.php?petugas="+akses.getkode().replaceAll(" ","_")+"&ttl="+ttl+"&tanggal="+DTPTgl.getSelectedItem().toString().replaceAll(" ","_")+"&usere="+koneksiDB.USERHYBRIDWEB()+"&passwordte="+koneksiDB.PASHYBRIDWEB());
                    }else if(i==2){
                        Valid.panggilUrl("billing/LaporanBilling3.php?petugas="+akses.getkode().replaceAll(" ","_")+"&tanggal="+DTPTgl.getSelectedItem().toString().replaceAll(" ","_")+"&usere="+koneksiDB.USERHYBRIDWEB()+"&passwordte="+koneksiDB.PASHYBRIDWEB());
                    }else if(i==3){
                        if(piutang>0){
    ...
    private void MnInputObat1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnInputObat1ActionPerformed
        if(TPasien.getText().trim().equals("")){
            ...
            if(Sequel.cariRegistrasi(TNoRw.getText())>0){
                ...
                akses.setform("DLgBilingRanap");
                getBeriObat().dlgobt.isCek();
                getBeriObat().dlgobt.setNoRm(norawatbayi,"-","By.Ny."+TPasien.getText(),DTPTgl.getDate(),"00","00","00",true);
                getBeriObat().dlgobt.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
                getBeriObat().dlgobt.tampil();
                getBeriObat().dlgobt.setLocationRelativeTo(internalFrame1);
                getBeriObat().dlgobt.setVisible(true);
    ...
    private void MnRawatInap1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnRawatInap1ActionPerformed
        if(TPasien.getText().trim().equals("")){
            ...
            if(Sequel.cariRegistrasi(TNoRw.getText())>0){
                ...
                getRawatInap().setLocationRelativeTo(internalFrame1);
                getRawatInap().isCek();
                getRawatInap().setNoRm(norawatbayi,DTPTgl.getDate(),new Date());
                getRawatInap().tampilDr();
                getRawatInap().setVisible(true);
            }                
        }
    ...
    private void MnDataObat1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDataObat1ActionPerformed
        if(Sequel.cariRegistrasi(TNoRw.getText())>0){
            ...
            getBeriObat().setLocationRelativeTo(internalFrame1);
            getBeriObat().isCek();
            getBeriObat().setNoRm(norawatbayi,DTPTgl.getDate(),new Date(),"ranap");
            getBeriObat().setVisible(true);
        }            
    ...
    private void TabRawatMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TabRawatMouseClicked
        if(TabRawat.getSelectedIndex()==2){
            try {
                ...
                //Gabungkan hasil tiga query lama ke Clinical Service Board.
                sinkronkanStatusPermintaan();
            }
        }
    ...
    */
    ...
    private javax.swing.JMenuItem MnTambahAdm;
    ...
    //================ CLINICAL SERVICE BOARD - STATUS PERMINTAAN ================
    // Seluruh ukuran, warna, font dan jarak tampilan baru diatur pada blok ini.
    // Query lama tidak dipindahkan atau diubah agar fungsi sumber data tetap aman.
    private static final Icon IKON_LAB=muatIkonLayanan(
            "/picture/medical-lab.png","LAB",new Color(0,121,107));
    private static final Icon IKON_RAD=muatIkonLayanan(
            "/picture/hasil_rad.png","RAD",new Color(37,99,235));
    private static final Icon IKON_RESEP=muatIkonLayanan(
            "/picture/pill.png","RESEP",new Color(124,58,237));
    private static final Icon IKON_TAB_TAGIHAN=muatIkonLayanan(
            "/picture/receipt.png","TAGIHAN",new Color(63,81,90));
    private static final Icon IKON_TAB_PEMBAYARAN=muatIkonLayanan(
            "/picture/money.png","BAYAR",new Color(63,81,90));
    private static final Icon IKON_TAB_STATUS=muatIkonLayanan(
            "/picture/check.png","STATUS",new Color(63,81,90));

    private static Icon muatIkonLayanan(String lokasi,String jenisCadangan,Color warnaCadangan){
        java.net.URL sumber=DlgBilingRanap.class.getResource(lokasi);
        if(sumber==null){
            return new ClinicalMiniIcon(jenisCadangan,warnaCadangan);
        }
        javax.swing.ImageIcon ikon=new javax.swing.ImageIcon(sumber);
        if(ikon.getIconWidth()==16&&ikon.getIconHeight()==16){
            return ikon;
        }
        return new javax.swing.ImageIcon(ikon.getImage().getScaledInstance(
                16,16,java.awt.Image.SCALE_SMOOTH));
    }

    /*
     * Header tab berbentuk badge/pill. Hanya komponen visual tab yang diganti;
     * panel isi, index tab dan handler lama tetap dipertahankan.
     */
    private void inisialisasiBadgeTabRawat(){
        if(TabRawat==null||TabRawat.getTabCount()<3){
            return;
        }

        final BadgeTabRawat[] badgeTab=new BadgeTabRawat[]{
            new BadgeTabRawat(0,"Data Tagihan",IKON_TAB_TAGIHAN),
            new BadgeTabRawat(1,"Pembayaran",IKON_TAB_PEMBAYARAN),
            new BadgeTabRawat(2,"Status Permintaan",IKON_TAB_STATUS)
        };
        for(int indeks=0;indeks<badgeTab.length;indeks++){
            TabRawat.setTabComponentAt(indeks,badgeTab[indeks]);
        }
        TabRawat.setBackground(Color.WHITE);
        TabRawat.setBorder(BorderFactory.createEmptyBorder(4,4,0,4));
        ...
        TabRawat.addChangeListener(new javax.swing.event.ChangeListener(){
            ...
            public void stateChanged(javax.swing.event.ChangeEvent e){
                perbaruiBadgeTabRawat(badgeTab);
            }
        });
        perbaruiBadgeTabRawat(badgeTab);
    }

    private void perbaruiBadgeTabRawat(BadgeTabRawat[] badgeTab){
        int indeksAktif=TabRawat.getSelectedIndex();
        for(BadgeTabRawat badge:badgeTab){
            badge.setAktif(badge.getIndeksTab()==indeksAktif);
        }
    }

    private class BadgeTabRawat extends JPanel{
        private final int indeksTab;
        private final Icon ikonNormal;
        private final JLabel labelTab;
        private boolean aktif,hover;

        BadgeTabRawat(int indeksTab,String judul,Icon ikonNormal){
            this.indeksTab=indeksTab;
            this.ikonNormal=ikonNormal;
            setOpaque(false);
            setLayout(new BorderLayout());
            //Ruang sebelum dan sesudah separator dibuat seimbang.
            setBorder(BorderFactory.createEmptyBorder(3,3,3,indeksTab<2?14:3));
            setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));

            labelTab=new JLabel(judul,SwingConstants.CENTER);
            labelTab.setHorizontalTextPosition(SwingConstants.RIGHT);
            labelTab.setIconTextGap(7);
            //Tinggi bertambah 2 px; font tetap 12 pt.
            labelTab.setBorder(BorderFactory.createEmptyBorder(6,8,6,8));
            labelTab.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
            add(labelTab,BorderLayout.CENTER);

            java.awt.event.MouseAdapter pilihTab=new java.awt.event.MouseAdapter(){
                @Override
                public void mouseClicked(java.awt.event.MouseEvent evt){
                    TabRawat.setSelectedIndex(BadgeTabRawat.this.indeksTab);
                    //Handler query lama tetap menjadi satu-satunya pemuat data.
                    if(BadgeTabRawat.this.indeksTab==2){
            ...
            addMouseListener(pilihTab);
            labelTab.addMouseListener(pilihTab);
        }
        ...
        private void perbaruiTampilanBadge(){
            //Ikon selalu memakai warna asli, termasuk ketika tab aktif.
            labelTab.setIcon(ikonNormal);
            labelTab.setForeground(aktif?Color.WHITE:
                    (hover?new Color(30,88,170):new Color(45,58,65)));
            labelTab.setFont(new Font((aktif||hover)?"Segoe UI Semibold":"Segoe UI",Font.PLAIN,12));
            repaint();
        }
        ...
        protected void paintComponent(Graphics g){
            ...
            int ruangKanan=indeksTab<2?14:3;
            int lebar=Math.max(0,getWidth()-3-ruangKanan);
            int tinggi=Math.max(0,getHeight()-7);
            g2.setColor(aktif?new Color(37,99,235):
                    (hover?new Color(239,246,255):Color.WHITE));
            g2.fillRoundRect(3,3,lebar,tinggi,tinggi,tinggi);
            g2.setColor(aktif?new Color(29,78,216):
                    (hover?new Color(96,165,250):new Color(203,213,225)));
            g2.drawRoundRect(3,3,lebar,tinggi,tinggi,tinggi);
            if(indeksTab<2){
                int xSeparator=getWidth()-5;
                g2.setColor(new Color(203,213,225));
                g2.drawLine(xSeparator,7,xSeparator,Math.max(7,getHeight()-8));
            }
            g2.dispose();
        }
    }

    private void inisialisasiClinicalServiceBoard() {
        final Color putih=new Color(255,255,255);
        final Color garis=new Color(222,230,236);
        final Color teksUtama=new Color(38,50,56);
        final Color teksSekunder=new Color(102,116,126);
        final Font fontNormal=new Font("Segoe UI",Font.PLAIN,12);
        final Font fontSemibold=new Font("Segoe UI Semibold",Font.PLAIN,12);

        panelPermintaan.removeAll();
        panelPermintaan.setLayout(new BorderLayout());
        panelPermintaan.setBackground(putih);
        panelPermintaan.setBorder(BorderFactory.createEmptyBorder(10,12,10,12));
        panelPermintaan.setPreferredSize(new Dimension(100,520));

        //Tabel langsung memenuhi area tab tanpa ringkasan, filter dan pencarian.
        tbStatusPermintaan=new JTable();
        tbStatusPermintaan.setModel(tabModePermintaan);
        tbStatusPermintaan.setFont(fontNormal);
        tbStatusPermintaan.setRowHeight(34);
        tbStatusPermintaan.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
        tbStatusPermintaan.setFillsViewportHeight(true);
        tbStatusPermintaan.setShowVerticalLines(false);
        tbStatusPermintaan.setShowHorizontalLines(true);
        tbStatusPermintaan.setGridColor(new Color(232,237,241));
        tbStatusPermintaan.setSelectionBackground(new Color(224,242,241));
        tbStatusPermintaan.setSelectionForeground(teksUtama);
        tbStatusPermintaan.setDefaultRenderer(Object.class,new PermintaanCellRenderer());
        tbStatusPermintaan.getColumnModel().getColumn(4).setCellRenderer(new StatusPermintaanRenderer());

        TableColumn kolomJenis=tbStatusPermintaan.getColumnModel().getColumn(0);
        TableColumn kolomNomor=tbStatusPermintaan.getColumnModel().getColumn(1);
        TableColumn kolomTanggal=tbStatusPermintaan.getColumnModel().getColumn(2);
        TableColumn kolomDokter=tbStatusPermintaan.getColumnModel().getColumn(3);
        TableColumn kolomStatus=tbStatusPermintaan.getColumnModel().getColumn(4);
        kolomJenis.setPreferredWidth(145);
        kolomNomor.setPreferredWidth(170);
        kolomTanggal.setPreferredWidth(155);
        kolomDokter.setPreferredWidth(380);
        kolomStatus.setPreferredWidth(145);

        JTableHeader header=tbStatusPermintaan.getTableHeader();
        header.setFont(fontSemibold);
        header.setBackground(new Color(239,245,248));
        header.setForeground(teksUtama);
        header.setPreferredSize(new Dimension(0,34));
        header.setReorderingAllowed(false);

        sorterPermintaan=new TableRowSorter<DefaultTableModel>(tabModePermintaan){
            @Override
            public void toggleSortOrder(int column){
                //Prioritas Belum Terlayani dan urutan layanan dibuat tetap
                //agar tidak berubah saat header tidak sengaja diklik.
            }
        };
        sorterPermintaan.setComparator(0,new java.util.Comparator<String>(){
            @Override
            public int compare(String jenisPertama,String jenisKedua){
                return urutanJenisLayanan(jenisPertama)-urutanJenisLayanan(jenisKedua);
            }
        });
        sorterPermintaan.setComparator(4,new java.util.Comparator<String>(){
            @Override
            public int compare(String statusPertama,String statusKedua){
                return urutanStatusPermintaan(statusPertama)-urutanStatusPermintaan(statusKedua);
            }
        });
        List<RowSorter.SortKey> urutan=new ArrayList<RowSorter.SortKey>();
        urutan.add(new RowSorter.SortKey(4,SortOrder.ASCENDING));
        urutan.add(new RowSorter.SortKey(0,SortOrder.ASCENDING));
        urutan.add(new RowSorter.SortKey(2,SortOrder.DESCENDING));
        sorterPermintaan.setSortKeys(urutan);
        tbStatusPermintaan.setRowSorter(sorterPermintaan);

        JScrollPane scrollStatus=new JScrollPane(tbStatusPermintaan);
        scrollStatus.setBorder(BorderFactory.createLineBorder(garis,1,true));
        scrollStatus.getViewport().setBackground(putih);

        //Empty state muncul jika pasien belum memiliki permintaan penunjang.
        JPanel panelKosong=new JPanel();
        panelKosong.setBackground(putih);
        panelKosong.setLayout(new BoxLayout(panelKosong,BoxLayout.Y_AXIS));
        JLabel lblKosongJudul=new JLabel("Belum ada permintaan layanan penunjang");
        lblKosongJudul.setFont(new Font("Segoe UI Semibold",Font.PLAIN,14));
        lblKosongJudul.setForeground(teksUtama);
        lblKosongJudul.setAlignmentX(Component.CENTER_ALIGNMENT);
        JLabel lblKosongInfo=new JLabel("Data Laboratorium, Radiologi, dan Resep akan tampil di sini.");
        lblKosongInfo.setFont(fontNormal);
        lblKosongInfo.setForeground(teksSekunder);
        lblKosongInfo.setAlignmentX(Component.CENTER_ALIGNMENT);
        panelKosong.add(Box.createVerticalGlue());
        panelKosong.add(lblKosongJudul);
        panelKosong.add(Box.createVerticalStrut(5));
        panelKosong.add(lblKosongInfo);
        panelKosong.add(Box.createVerticalGlue());

        layoutDaftarPermintaan=new CardLayout();
        panelDaftarPermintaan=new JPanel(layoutDaftarPermintaan);
        panelDaftarPermintaan.setOpaque(false);
        panelDaftarPermintaan.add(scrollStatus,"TABEL");
        panelDaftarPermintaan.add(panelKosong,"KOSONG");
        panelPermintaan.add(panelDaftarPermintaan,BorderLayout.CENTER);

        perbaruiTampilanDaftarPermintaan();
        panelPermintaan.revalidate();
        panelPermintaan.repaint();
    }

    //Menyalin data tiga model lama ke satu model tampilan tanpa mengubah query.
    private void sinkronkanStatusPermintaan(){
        if(tabModePermintaan==null){
            return;
        }
        tabModePermintaan.setRowCount(0);
        tambahkanKeClinicalBoard("Laboratorium",tabModeLab);
        tambahkanKeClinicalBoard("Radiologi",tabModeRad);
        tambahkanKeClinicalBoard("Resep",tabModeApotek);
        if(sorterPermintaan!=null){
            sorterPermintaan.sort();
        }
        perbaruiTampilanDaftarPermintaan();
    }

    private void tambahkanKeClinicalBoard(String jenisLayanan,DefaultTableModel modelSumber){
        for(int baris=0;baris<modelSumber.getRowCount();baris++){
            String nomor=nilaiModel(modelSumber,baris,0);
            String tanggal=nilaiModel(modelSumber,baris,1);
            String jam=nilaiModel(modelSumber,baris,2);
            String dokter=nilaiModel(modelSumber,baris,3);
            String statusPermintaan=nilaiModel(modelSumber,baris,4);
            String tanggalJam=tanggal+(jam.equals("")?"":" "+jam);
            tabModePermintaan.addRow(new Object[]{
                jenisLayanan,nomor,tanggalJam,dokter,statusPermintaan
            });
        }
    }

    private String nilaiModel(DefaultTableModel model,int baris,int kolom){
        Object nilai=model.getValueAt(baris,kolom);
        return nilai==null?"":nilai.toString().trim();
    }

    //Menampilkan tabel atau empty state tanpa mengubah data model sumber.
    private void perbaruiTampilanDaftarPermintaan(){
        int jumlahTampil=sorterPermintaan==null?tabModePermintaan.getRowCount():sorterPermintaan.getViewRowCount();
        sesuaikanLebarKolomStatusPermintaan();
        if(layoutDaftarPermintaan!=null&&panelDaftarPermintaan!=null){
            layoutDaftarPermintaan.show(panelDaftarPermintaan,jumlahTampil==0?"KOSONG":"TABEL");
        }
    }

    //Lebar kolom dihitung dari header dan isi tabel agar teks tetap rapi terbaca.
    private void sesuaikanLebarKolomStatusPermintaan(){
        if(tbStatusPermintaan==null||tbStatusPermintaan.getColumnCount()==0){
            return;
        }
        int[] lebarMinimum={145,165,145,180,140};
        int[] lebarMaksimum={190,285,215,480,180};
        int jumlahBaris=Math.min(tbStatusPermintaan.getRowCount(),250);
        JTableHeader header=tbStatusPermintaan.getTableHeader();

        for(int kolomView=0;kolomView<tbStatusPermintaan.getColumnCount();kolomView++){
            TableColumn kolom=tbStatusPermintaan.getColumnModel().getColumn(kolomView);
            javax.swing.table.TableCellRenderer rendererHeader=kolom.getHeaderRenderer();
            if(rendererHeader==null){
                rendererHeader=header.getDefaultRenderer();
            }
            Component komponenHeader=rendererHeader.getTableCellRendererComponent(
                    tbStatusPermintaan,kolom.getHeaderValue(),false,false,-1,kolomView);
            int lebar=komponenHeader.getPreferredSize().width+20;

            for(int baris=0;baris<jumlahBaris;baris++){
                javax.swing.table.TableCellRenderer rendererIsi=
                        tbStatusPermintaan.getCellRenderer(baris,kolomView);
                Component komponenIsi=tbStatusPermintaan.prepareRenderer(rendererIsi,baris,kolomView);
                lebar=Math.max(lebar,komponenIsi.getPreferredSize().width+18);
            }

            lebar=Math.max(lebarMinimum[kolomView],Math.min(lebar,lebarMaksimum[kolomView]));
            kolom.setMinWidth(lebarMinimum[kolomView]);
            kolom.setPreferredWidth(lebar);
            kolom.setMaxWidth(lebarMaksimum[kolomView]);
        }
    }

    private static String formatTanggalPermintaan(Object nilai){
        String tanggal=nilai==null?"":nilai.toString();
        if(tanggal.length()>=10&&tanggal.charAt(4)=='-'&&tanggal.charAt(7)=='-'){
            return tanggal.substring(8,10)+"-"+tanggal.substring(5,7)+"-"+tanggal.substring(0,4)+tanggal.substring(10);
        }
        return tanggal;
    }

    //Semua permintaan yang belum dilayani selalu ditempatkan paling atas.
    private static int urutanStatusPermintaan(String statusPermintaan){
        if(statusPermintaan==null){
            return 99;
        }
        String status=statusPermintaan.toLowerCase();
        if(status.contains("belum")){
            return 1;
        }
        if(status.contains("sudah")){
            return 2;
        }
        return 3;
    }

    //Di dalam setiap status, urutan layanan: Laboratorium, Radiologi, Resep.
    private static int urutanJenisLayanan(String jenisLayanan){
        if(jenisLayanan==null){
            return 99;
        }
        if(jenisLayanan.equalsIgnoreCase("Laboratorium")){
            return 1;
        }
        if(jenisLayanan.equalsIgnoreCase("Radiologi")){
            return 2;
        }
        if(jenisLayanan.equalsIgnoreCase("Resep")){
            return 3;
        }
        return 99;
    }

    private static boolean barisAwalGrup(JTable table,int row){
        if(row<=0){
            return true;
        }
        Object jenisSekarang=table.getValueAt(row,0);
        Object jenisSebelumnya=table.getValueAt(row-1,0);
        Object statusSekarang=table.getValueAt(row,4);
        Object statusSebelumnya=table.getValueAt(row-1,4);
        return jenisSekarang==null||jenisSebelumnya==null||statusSekarang==null||statusSebelumnya==null||
                !jenisSekarang.toString().equals(jenisSebelumnya.toString())||
                !statusSekarang.toString().equals(statusSebelumnya.toString());
    }

    //Renderer sel: grup dipisahkan garis; nama dan ikon tampil pada setiap baris.
    private static class PermintaanCellRenderer extends DefaultTableCellRenderer{
        @Override
        public Component getTableCellRendererComponent(JTable table,Object value,boolean isSelected,
                boolean hasFocus,int row,int column){
            super.getTableCellRendererComponent(table,value,isSelected,hasFocus,row,column);
            boolean awalGrup=barisAwalGrup(table,row);
            setIcon(null);
            setIconTextGap(7);
            setBorder(BorderFactory.createCompoundBorder(
                    BorderFactory.createMatteBorder((awalGrup&&row>0)?2:0,0,0,0,new Color(205,216,224)),
            ...
            if(isSelected){
                ...
                setBackground(row%2==0?new Color(255,255,255):new Color(248,250,252));
                setForeground(new Color(50,60,65));
            }
            if(column==0){
                setFont(new Font("Segoe UI Semibold",Font.PLAIN,12));
                String jenis=value==null?"":value.toString();
                setText(jenis);
                if(jenis.equals("Laboratorium")){
                    setIcon(IKON_LAB);
                }else if(jenis.equals("Radiologi")){
                    setIcon(IKON_RAD);
                ...
                if(!isSelected){
                    if(jenis.equals("Laboratorium")){
                        setForeground(new Color(0,105,92));
                    }else if(jenis.equals("Radiologi")){
                        setForeground(new Color(30,88,170));
                    }else if(jenis.equals("Resep")){
                        setForeground(new Color(106,55,160));
                    }
                }
            }else{
            ...
            if(column==2){
                setText(formatTanggalPermintaan(value));
            }
            return this;
        }
    }
    ...
    /*
     * Ikon 16x16 digambar langsung dengan Java2D agar revisi ini tidak
     * menambah file resource dan tetap aman saat dipindahkan ke project lama.
     */
    private static class ClinicalMiniIcon implements Icon{
        private final String jenis;
        private final Color warna;

        ClinicalMiniIcon(String jenis,Color warna){
            this.jenis=jenis;
            this.warna=warna;
        }
        ...
        public void paintIcon(Component c,Graphics g,int x,int y){
            ...
            if(jenis.equals("LAB")){
                g2.drawLine(6,2,10,2);
                g2.drawLine(7,2,7,6);
                g2.drawLine(9,2,9,6);
                java.awt.Polygon labu=new java.awt.Polygon(
                        new int[]{7,4,3,13,12,9},new int[]{6,11,13,13,11,6},6);
                g2.drawPolygon(labu);
                g2.drawLine(5,10,11,10);
            }else if(jenis.equals("RAD")){
                g2.drawOval(5,5,6,6);
                g2.fillOval(7,7,2,2);
                g2.drawLine(8,1,8,4);
                ...
                g2.drawLine(13,3,11,5);
                g2.drawLine(5,11,3,13);
            }else if(jenis.equals("RESEP")){
                g2.rotate(-Math.PI/4,8,8);
                g2.drawRoundRect(3,5,10,6,6,6);
                g2.drawLine(8,5,8,11);
            }else if(jenis.equals("TAGIHAN")){
                g2.drawRoundRect(4,1,8,14,2,2);
                g2.drawLine(6,5,10,5);
                g2.drawLine(6,8,10,8);
                g2.drawLine(6,11,9,11);
            }else if(jenis.equals("BAYAR")){
                g2.drawRoundRect(1,4,14,9,3,3);
                g2.drawLine(2,7,14,7);
                g2.fillOval(11,9,2,2);
            }else{
    ...
    //Renderer badge membedakan status terlayani dan belum terlayani.
    private static class StatusPermintaanRenderer extends DefaultTableCellRenderer{
        private Color warnaBaris=new Color(255,255,255);
        private Color warnaBadge=new Color(238,242,244);

        StatusPermintaanRenderer(){
        ...
        public Component getTableCellRendererComponent(JTable table,Object value,boolean isSelected,
            ...
            String nilai=value==null?"":value.toString();
            setText(nilai);
            warnaBaris=isSelected?table.getSelectionBackground():(row%2==0?new Color(255,255,255):new Color(248,250,252));
            if(nilai.toLowerCase().contains("belum")){
                warnaBadge=new Color(255,243,224);
                setForeground(new Color(154,82,0));
            }else if(nilai.toLowerCase().contains("sudah")){
                warnaBadge=new Color(232,245,233);
                setForeground(new Color(27,94,62));
            }else{
                warnaBadge=new Color(238,242,244);
                setForeground(new Color(78,91,99));
            }
            setOpaque(false);
            return this;
        }
        ...
        protected void paintComponent(Graphics g){
            Graphics2D g2=(Graphics2D)g.create();
            g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);
            g2.setColor(warnaBaris);
            g2.fillRect(0,0,getWidth(),getHeight());
            int tinggi=Math.max(20,getHeight()-10);
            int lebar=Math.min(getWidth()-12,Math.max(110,getFontMetrics(getFont()).stringWidth(getText())+24));
            int x=(getWidth()-lebar)/2;
            int y=(getHeight()-tinggi)/2;
            g2.setColor(warnaBadge);
            g2.fillRoundRect(x,y,lebar,tinggi,tinggi,tinggi);
            g2.dispose();
            super.paintComponent(g);
        }
    }
    //============== AKHIR CLINICAL SERVICE BOARD - STATUS PERMINTAAN ==============
    ...
    public void isRawat() {
         try {      
            ...
            if(i<=0){
                ...
                tabModeRwJlDr.addRow(new Object[]{true,"","Total Obat Bersih, Operasi, dan Resep Pulang : "+Valid.SetAngka3(ttlSemuaObat),"",null,null,null,null,"TtlRetur Obat"});
...
//                    "order by kamar_inap.tgl_keluar desc limit 1");
                    ...
                    //MODIF MUSTAFA
                        ...
                        "CONCAT(DATE_FORMAT((SELECT MIN(kamar_inap.tgl_masuk) FROM kamar_inap WHERE kamar_inap.no_rawat = reg_periksa.no_rawat), '%e %M %Y'), " +
                        "' ', (SELECT MIN(kamar_inap.jam_masuk) FROM kamar_inap WHERE kamar_inap.no_rawat = reg_periksa.no_rawat)) AS registrasi, " +
                        "kamar_inap.kd_kamar, " +
                        "CONCAT(IF(kamar_inap.tgl_keluar = '0000-00-00', DATE_FORMAT(CURDATE(), '%e %M %Y'), DATE_FORMAT(kamar_inap.tgl_keluar, '%e %M %Y')), " +
                        "' ', kamar_inap.jam_keluar) AS keluar, " +
                        "(SELECT SUM(kamar_inap.lama) FROM kamar_inap WHERE kamar_inap.no_rawat = reg_periksa.no_rawat) AS lama, " +
                        "reg_periksa.biaya_reg, reg_periksa.umurdaftar, reg_periksa.sttsumur, reg_periksa.tgl_registrasi " +
                        "FROM reg_periksa " +
...
//                    tabModeRwJlDr.addRow(new Object[]{true,"Tgl.Perawatan",": "+rsreg.getString("registrasi")+" s.d. "+rsreg.getString("keluar")+" ( "+rsreg.getString("lama")+" Hari )","",null,null,null,null,"-"});
    ...
    private void prosesCariObat(String norawat) {    
        ...
        try{ 
            psobatoperasi=koneksi.prepareStatement(sqlpsobatoperasi_nonmonitor);
            try {
                    ...
                    if(rsobatoperasi.next()){
                        ...
                        while(rsobatoperasi.next()){
                             tabModeRwJlDr.addRow(new Object[]{true,"                           ",rsobatoperasi.getString("nm_obat"),":",
                                ...
                                ttlObatOperasi = ttlObatOperasi+rsobatoperasi.getDouble("total");
                        }
                    }
                }else{
                    if(rsobatoperasi.next()){
                ...
                if (ttlObatOperasi > 0){
                    tabModeRwJlDr.addRow(new Object[]{true,"","Total Obat & BHP Operasi : "+Valid.SetAngka(ttlObatOperasi),"",null,null,null,null,"TtlObat"});
                    ttlSemuaObat = ttlObatOperasi;
                }
            } catch (Exception e) {
                System.out.println("Notifikasi : "+e);
            } finally{
        ...
        if((ttlobat-ttlretur)>0){
            ...
            ttlSemuaObat= ttlSemuaObat + ttlobat+ttlretur+ppnobat;
        } 
    ...
    private void prosesCariKamar() {
        //Persentase bayi hanya dibutuhkan untuk kasus ranap gabung.
        //Hindari query ini pada setiap pembukaan billing pasien biasa.
        if(!norawatbayi.equals("")&&!persenBayiDimuat){
            persenbayi=Sequel.cariInteger("select set_jam_minimal.bayi from set_jam_minimal");
            persenBayiDimuat=true;
        }
    ...
    private void prosesResepPulang(String norawat) {
        /*
         * Resep Pulang reguler tetap memakai tabel resep_pulang.
         * Racikan resep pulang yang dibuat melalui DlgPermintaanResepPulang
         * disimpan terpisah di permintaan_resep_pulang_racikan[_detail].
         * Keduanya ditampilkan dalam satu kelompok tagihan "Resep Pulang"
         * supaya total, jurnal, dan kuitansi mengikuti mekanisme lama.
         */
        boolean adaResepPulang = false;
        try{
            adaResepPulang =
                Sequel.cariInteger("select count(resep_pulang.kode_brng) from resep_pulang where resep_pulang.no_rawat=?",norawat)>0 ||
                Sequel.cariInteger(
                    "select count(d.kode_brng) from permintaan_resep_pulang p " +
                    "inner join permintaan_resep_pulang_racikan_detail d on d.no_permintaan=p.no_permintaan " +
                    "where p.no_rawat=? and d.jml>0",norawat)>0;
        }catch(Exception e){
            System.out.println("Notifikasi cek resep pulang racikan : "+e);
        }
        ...
        // 1. Resep pulang reguler (alur lama, tidak diubah)
        try{
        ...
        /*
         * 2. Racikan dari Permintaan Resep Pulang.
         * Setiap bahan racikan ditampilkan sebagai satu baris obat biasa,
         * sama seperti rincian obat pada Billing Ralan. Harga baru disimpan
         * di detail racikan; data lama memakai harga kelas/kamar sebagai fallback.
         */
        PreparedStatement psRacikan=null;
        ResultSet rsRacikan=null;
        try{
            String kelasRacikan=Sequel.cariIsi(
                "select kamar.kelas from kamar inner join kamar_inap on kamar.kd_kamar=kamar_inap.kd_kamar " +
                "where kamar_inap.no_rawat=? order by STR_TO_DATE(concat(kamar_inap.tgl_masuk,' ',kamar_inap.jam_masuk),'%Y-%m-%d %H:%i:%s') desc limit 1",
                norawat
            );
            String kdPjRacikan=Sequel.cariIsi("select kd_pj from reg_periksa where no_rawat=?",norawat);

            String kolomHarga="utama";
            if(kelasRacikan.equals("Kelas 1")){
                kolomHarga="kelas1";
            }else if(kelasRacikan.equals("Kelas 2")){
                kolomHarga="kelas2";
            }else if(kelasRacikan.equals("Kelas 3")){
                kolomHarga="kelas3";
            }else if(kelasRacikan.equals("Kelas VIP")){
                kolomHarga="vip";
            }else if(kelasRacikan.equals("Kelas VVIP")){
                kolomHarga="vvip";
            }

            double kenaikanRacikan=0;
            try{
                kenaikanRacikan=Sequel.cariIsiAngka2(
            ...
            psRacikan=koneksi.prepareStatement(
                "select p.no_permintaan,r.no_racik,r.nama_racik,r.aturan_pakai,d.kode_brng,db.nama_brng,d.jml," +
                "ifnull(d.harga,0) as harga_simpan,ifnull(db.h_beli,0) as h_beli,ifnull(db."+kolomHarga+",0) as harga_kelas " +
                "from permintaan_resep_pulang p " +
                "inner join permintaan_resep_pulang_racikan r on r.no_permintaan=p.no_permintaan " +
                "inner join permintaan_resep_pulang_racikan_detail d on d.no_permintaan=r.no_permintaan and d.no_racik=r.no_racik " +
                "inner join databarang db on db.kode_brng=d.kode_brng " +
                "where p.no_rawat=? and d.jml>0 " +
                "order by p.tgl_permintaan,p.jam,cast(r.no_racik as unsigned),db.nama_brng"
            );
            psRacikan.setString(1,norawat);
            rsRacikan=psRacikan.executeQuery();

            while(rsRacikan.next()){
                /*
                 * Disamakan dengan Billing Ralan:
                 * satu bahan racikan = satu baris obat biasa.
                 * Tidak ada judul "Racikan ..." maupun nama racikan pada kuitansi.
                 */
                double harga=rsRacikan.getDouble("harga_simpan");
                if(harga<=0){
                    if(kenaikanRacikan>0){
                        harga=Valid.roundUp(rsRacikan.getDouble("h_beli")+(rsRacikan.getDouble("h_beli")*kenaikanRacikan),100);
                    }else{
                        harga=Valid.roundUp(rsRacikan.getDouble("harga_kelas"),100);
                    }
                }

                String namaDetail=rsRacikan.getString("nama_brng")+" ";

                // Racikan resep pulang belum memiliki embalase/tuslah.
                // Nilai tambahan dibuat nol agar kolom dan pola baris sama seperti Billing Ralan.
                tamkur=0;

                double jumlah=rsRacikan.getDouble("jml");
                double totalRacikan=(harga*jumlah)+tamkur;
                tabModeRwJlDr.addRow(new Object[]{true,"",namaDetail,":",
                    harga,jumlah,tamkur,totalRacikan,"Resep Pulang"});
                subttl=subttl+totalRacikan;
            }
        }catch(Exception e){
            System.out.println("Notifikasi racikan resep pulang pada billing ranap : "+e);
        }finally{
            try{
                if(rsRacikan!=null){
                    rsRacikan.close();
        ...
        if(subttl>1){
             ...
             ttlSemuaObat = ttlSemuaObat + subttl;
        }
    }
...
//            // ====== Periksa Lab (RANAP) — header tanpa nominal, detail > 0, Jml×Tambahan=Total ======
//            subttl = 0;
//            psperiksalab = koneksi.prepareStatement(
...
//                   "WHERE periksa_lab.no_rawat = ? AND periksa_lab.status LIKE ? " +
...
//                rsperiksalab.beforeFirst();
//
//                // SQL rincian per item (join sesuai relasi kamu, hanya nilai > 0)
//                final String SQL_RINCI_LAB =
//                    "SELECT tl.Pemeriksaan AS pemeriksaan, " +
//                    "       COUNT(*) AS jml, " +
//                    "       SUM(dpl.biaya_item) AS total " +
//                    "FROM detail_periksa_lab dpl " +
//                    "JOIN template_laboratorium tl " +
//                    "  ON tl.id_template = dpl.id_template " +
//                    " AND tl.kd_jenis_prw = dpl.kd_jenis_prw " +
//                    "WHERE dpl.no_rawat = ? " +
//                    "  AND (dpl.kd_jenis_prw = ? OR TRIM(dpl.kd_jenis_prw) = TRIM(?)) " +
//                    "GROUP BY tl.urut, tl.Pemeriksaan " +
//                    "HAVING SUM(dpl.biaya_item) > 0 " +
//                    "ORDER BY tl.urut, tl.Pemeriksaan";
//
//                while (rsperiksalab.next()) {
//                    final String kdJenis = rsperiksalab.getString("kd_jenis_prw");
//
//                    // HEADER jenis tanpa nominal
//                    tabModeRwJlDr.addRow(new Object[]{
//                        true, "                           ",
...
//                    });
//
//                    // DETAIL: tampilkan hanya item > 0; isi Jml & Tambahan sehingga Jml×Tambahan=Total
//                    try (PreparedStatement psRinci = koneksi.prepareStatement(SQL_RINCI_LAB)) {
//                        psRinci.setString(1, norawat);
//                        psRinci.setString(2, kdJenis);
//                        psRinci.setString(3, kdJenis);
//                        try (ResultSet rsRinci = psRinci.executeQuery()) {
//                            while (rsRinci.next()) {
//                                double jml   = rsRinci.getDouble("jml");
//                                double total = rsRinci.getDouble("total");
//                                Double tambahan = (jml > 0) ? (total / jml) : null;
//
//                                tabModeRwJlDr.addRow(new Object[]{
//                                    true, "",
//                                    "    " + rsRinci.getString("pemeriksaan"),
//                                    "",
//                                    null,        // Biaya (kosong untuk detail)
//                                    jml,         // Jml
//                                    tambahan,    // Tambahan (unit price)
//                                    total,       // Total Biaya
//                                    "Laborat"
//                                });
//
//                                subttl += total; // subtotal dari DETAIL saja
//                            }
//                        }
//                    } catch (Exception e) {
//                        System.out.println("Notifikasi (rincian lab ranap): " + e);
//                    }
//                }
//
//                // TOTAL LABORAT (dari detail saja)
//                if (subttl > 1) {
//                    tabModeRwJlDr.addRow(new Object[]{ true, "", "Total Periksa Lab : " + Valid.SetAngka(subttl), "", null, null, null, null, "TtlLaborat" });
//                }
//            } catch (Exception e) {
//                System.out.println("Notifikasi Periksa Lab : " + e);
//            } finally {
//                if (rsperiksalab != null) { rsperiksalab.close(); }
//                if (psperiksalab != null) { psperiksalab.close(); }
//            }
//            // ====== END Periksa Lab (RANAP) ======

            // ====== Periksa Lab (RANAP) — PA ambil harga di judul (total_byr), PK/MB di detail ======
            subttl = 0;
            psperiksalab = koneksi.prepareStatement(
                   "SELECT jpl.nm_perawatan, " +
                   "       COUNT(pl.kd_jenis_prw) AS jml, " +
                   "       jpl.kd_jenis_prw, " +
                   "       jpl.kategori, " +                // <<— ambil kategori: PA / PK / MB
                   "       jpl.total_byr " +               // <<— ambil total_byr untuk PA
                   "FROM periksa_lab pl " +
                   "INNER JOIN jns_perawatan_lab jpl ON jpl.kd_jenis_prw = pl.kd_jenis_prw " +
                   "WHERE pl.no_rawat = ? AND pl.status LIKE ? " +
                   "GROUP BY pl.kd_jenis_prw, jpl.nm_perawatan, jpl.kategori, jpl.total_byr"
            );
            try {
                psperiksalab.setString(1, norawat);
                if ((chkRalan.isSelected()==true) && (chkRanap.isSelected()==true)) {
                ...
                // SQL rincian per item (untuk PK/MB; hanya nilai > 0)
                final String SQL_RINCI_LAB =
                    "SELECT tl.Pemeriksaan AS pemeriksaan, " +
                    "       COUNT(*) AS jml, " +
                    "       SUM(dpl.biaya_item) AS total " +
                ...
                while (rsperiksalab.next()) {
                    final String kdJenis      = rsperiksalab.getString("kd_jenis_prw");
                    final String nmPerawatan  = rsperiksalab.getString("nm_perawatan");
                    final String kategori     = rsperiksalab.getString("kategori");   // PA / PK / MB
                    final double jmlHeader    = rsperiksalab.getDouble("jml");
                    final double totalByrPA   = rsperiksalab.getDouble("total_byr");  // dipakai untuk PA

                    if ("PA".equalsIgnoreCase(kategori)) {
                        // === PA: harga di judul (total_byr). Tampilkan angka di header. ===
                        Double tambahan = (jmlHeader > 0) ? totalByrPA : null;
                        Double total    = (jmlHeader > 0) ? (jmlHeader * totalByrPA) : null;

                        tabModeRwJlDr.addRow(new Object[]{
                            true, "                           ",
                            nmPerawatan,
                            ":", 
                            null,           // Biaya (tidak dipakai)
                            jmlHeader,      // Jml
                            tambahan,       // Tambahan = total_byr
                            total,          // Total = Jml × total_byr
                            "Laborat"
                        });

                        if (total != null) subttl += total;
                        continue; // jangan render detail untuk PA
                    }

                    // === PK/MB (atau kategori lain): header tanpa nominal + render detail > 0 ===
                    tabModeRwJlDr.addRow(new Object[]{
                        true, "                           ",
                        nmPerawatan,
                        ":", 
                ...
                // TOTAL LABORAT (gabungan PA + PK/MB)
                if (subttl > 1) {
                    tabModeRwJlDr.addRow(new Object[]{ true, "", "Total Periksa Lab : " + Valid.SetAngka(subttl), "", null, null, null, null, "TtlLaborat" });
                }
            } catch (Exception e) {
    ...
    private void prosesCariOperasi(String norawat){
        try{            
            ...
            try {
                ...
                if(rincianoperasi.equals("Yes")){
                    while(rsoperasi.next()){
                        ...
                        if(rsoperasi.getDouble("biayasewaok")>0){
                           tabModeRwJlDr.addRow(new Object[]{true,"                           ","  Tarif  Kamar Operasi",":",rsoperasi.getDouble("biayasewaok"),1,0,rsoperasi.getDouble("biayasewaok"),"Operasi"}); 
                        }
                ...
                psobatoperasi=koneksi.prepareStatement(sqlpsobatoperasi_monitor);
                try {
                    psobatoperasi.setString(1,norawat);
                    rsobatoperasi=psobatoperasi.executeQuery();
                    if(centangobatranap.equals("Yes")){
    ...
    private void isSimpan() {
        try {  
            ...
            if(sukses==true){
                ...
                // 🔹 AUTO UPLOAD BILLING KE INACBG SETELAH BERHASIL SIMPAN
                try {
                    INACBGDataIDRG1 formBilling = new INACBGDataIDRG1(null, false); // sesuaikan ctor kalau perlu
                    formBilling.autoUploadBillingForNoRawat(TNoRw.getText());
                } catch (Exception e) {
                    System.out.println("Notifikasi autoUploadBillingForNoRawat : " + e);
                    // Optional: kalau mau kasih info ke user
                    // JOptionPane.showMessageDialog(null,"Billing tersimpan, tapi gagal auto upload ke INACBG.\nDetail: "+e.getMessage());
                }
...
// Capture halaman billing jadi beberapa JPG ukuran folio dan upload semuanya (kode berkas 144)
private void autoUploadBillingToBerkasDigital(String noRawat, String relativeUrl) {
    ...
    try {
        // Susun URL penuh ke billing (sama basis dengan upload.php)
        String baseUrl = "http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB()
                       + "/" + koneksiDB.HYBRIDWEB() + "/";
        String fullUrl = baseUrl + relativeUrl;   // contoh: .../webapps/billing/LaporanBillingUpload.php?...

        // Penamaan dasar file
        java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("ddMMyyyy_HHmmss");
        String timestamp = sdf.format(new java.util.Date());
        String fileBase = timestamp + "_" + noRawat.replace("/", "") + "_Billing";

        // Folder tmpJPG
        java.io.File tmpDir = new java.io.File("tmpJPG");
        if (!tmpDir.exists()) {
            tmpDir.mkdirs();
        }

        // 1) Render SATU gambar panjang penuh (tanpa batas tinggi)
        java.io.File fullImgFile = new java.io.File(tmpDir, fileBase + "_full.png");

        java.util.List<String> cmd = new java.util.ArrayList<>();
        cmd.add("C:\\Program Files\\wkhtmltopdf\\bin\\wkhtmltoimage.exe"); // SESUAIKAN path kalau beda
        cmd.add("--width");
        cmd.add("816");      // lebar ≈ 8.5 inch (folio) pada 96 dpi
        // TIDAK pakai --height, supaya tinggi mengikuti konten (multi page)
        cmd.add(fullUrl);
        cmd.add(fullImgFile.getAbsolutePath());

        ProcessBuilder pb = new ProcessBuilder(cmd);
        pb.redirectErrorStream(true);
        Process p = pb.start();

        // baca output wkhtmltoimage untuk debug (opsional)
        try (java.io.BufferedReader r = new java.io.BufferedReader(
                new java.io.InputStreamReader(p.getInputStream()))) {
            String line;
            while ((line = r.readLine()) != null) {
                System.out.println("[wkhtmltoimage] " + line);
            }
        }

        int exit = p.waitFor();
        if (exit != 0 || !fullImgFile.exists()) {
            System.out.println("autoUploadBillingToBerkasDigital: gagal membuat gambar, exit=" + exit);
            return;
        }

        // 2) Baca gambar penuh, lalu dipotong-potong per tinggi folio
        java.awt.image.BufferedImage fullImg = javax.imageio.ImageIO.read(fullImgFile);
        if (fullImg == null) {
            System.out.println("autoUploadBillingToBerkasDigital: gagal membaca gambar " + fullImgFile);
            return;
        }

        int pageHeight = 1248;                   // tinggi folio ≈ 13 inch * 96 dpi
        int totalHeight = fullImg.getHeight();
        int fullWidth = fullImg.getWidth();
        int totalPages = (int) Math.ceil(totalHeight / (double) pageHeight);

        java.util.List<String> pageFileBases = new java.util.ArrayList<>();

        for (int page = 0; page < totalPages; page++) {
            int y = page * pageHeight;
            int h = Math.min(pageHeight, totalHeight - y);

            // potong bagian gambar untuk halaman ini
            java.awt.image.BufferedImage slice = fullImg.getSubimage(0, y, fullWidth, h);

            // buat canvas putih tinggi pageHeight supaya konsisten
            java.awt.image.BufferedImage pageImage =
                    new java.awt.image.BufferedImage(fullWidth, pageHeight, java.awt.image.BufferedImage.TYPE_INT_RGB);
            java.awt.Graphics2D g2 = pageImage.createGraphics();
            g2.setColor(java.awt.Color.WHITE);
            g2.fillRect(0, 0, fullWidth, pageHeight);
            g2.drawImage(slice, 0, 0, null);
            g2.dispose();

            String pageBaseName = fileBase + "_p" + (page + 1);
            java.io.File pageFile = new java.io.File(tmpDir, pageBaseName + ".jpg");
            javax.imageio.ImageIO.write(pageImage, "jpg", pageFile);

            pageFileBases.add(pageBaseName);
        }

        // 3) Upload semua halaman & simpan ke berkas_digital_perawatan
        uploadBillingJPGMulti(pageFileBases, "berkasrawat/pages/upload/", noRawat);

    } catch (Exception ex) {
        System.out.println("autoUploadBillingToBerkasDigital error: " + ex);
    }
}

// Upload beberapa JPG billing (per halaman) dan simpan semuanya ke berkas_digital_perawatan (kode 144)
private void uploadBillingJPGMulti(java.util.List<String> fileBaseNames, String docpath, String no_rawat) {
    if (fileBaseNames == null || fileBaseNames.isEmpty()) {
        System.out.println("uploadBillingJPGMulti: tidak ada file yang akan diupload");
        return;
    }

    try {
        // Pastikan kode berkas Billing = 144
        String kodeberkas = Sequel.cariIsi(
                "SELECT kode FROM master_berkas_digital WHERE kode='144'"
        );
        if (kodeberkas.trim().equals("")) {
            kodeberkas = "144";
        }

        // Hapus semua billing lama utk no_rawat ini (supaya tidak dobel)
        Sequel.queryu2(
            "DELETE FROM berkas_digital_perawatan WHERE no_rawat='" + no_rawat + "' AND kode='" + kodeberkas + "'"
        );

        for (String fileBase : fileBaseNames) {
            java.io.File file = new java.io.File("tmpJPG/" + fileBase + ".jpg");
            if (!file.exists()) {
                System.out.println("uploadBillingJPGMulti: file tidak ditemukan: " + file.getAbsolutePath());
                continue;
            }
            ...
            reqEntity.addPart(
                    "file",
                    new org.apache.http.entity.mime.content.ByteArrayBody(data, fileBase + ".jpg")
            );
            postRequest.setEntity(reqEntity);
            httpClient.execute(postRequest);

            // lokasi_file yang disimpan di DB
            String lokasi_file = "pages/upload/" + fileBase + ".jpg";
            ...
            if (ok) {
                System.out.println("uploadBillingJPGMulti: tersimpan " + lokasi_file);
            } else {
                System.out.println("uploadBillingJPGMulti: GAGAL simpan metadata untuk " + lokasi_file);
            }
        }

    } catch (Exception e) {
        System.out.println("uploadBillingJPGMulti error : " + e);
    }
}
