...
public final class DlgCariPerawatanRanap extends javax.swing.JDialog {
    ...
    public  DlgCariPetugas petugas=new DlgCariPetugas(null,true);
    ...
    public DlgKtgPerawatan ktg=new DlgKtgPerawatan(null,true);
    ...
    private void BtnTambahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnTambahActionPerformed
        ...
        DlgJnsPerawatanRanap perawatan=new DlgJnsPerawatanRanap(null,true);
        perawatan.emptTeks();
        perawatan.isCek();
...
//            for(i=0;i<tbKamar.getRowCount();i++){
//                if(tbKamar.getValueAt(i,0).toString().equals("true")){
//                    String namaTindakan = tbKamar.getValueAt(i, 2).toString().toLowerCase();
//                    boolean containsRestrictedWord = namaTindakan.contains("echo") || namaTindakan.contains("usg doppler");
//                    if (containsRestrictedWord && 
//                        !(akses.getkode().equals("Admin Utama") || akses.getkode().equals("19951193"))) {
//                        continue;
//                    }
//                    jml++;
//                }
...
//              
//                    String namaPerawatan = list.path("NamaPerawatan").asText().toLowerCase();
//                    boolean containsRestrictedWord = namaPerawatan.contains("echo") || namaPerawatan.contains("usg doppler");
//
//                    if ((list.path("Kode").asText().toLowerCase().contains(TCari.getText().toLowerCase()) ||
//                         namaPerawatan.contains(TCari.getText().toLowerCase()) ||
//                         list.path("KategoriPerawatan").asText().toLowerCase().contains(TCari.getText().toLowerCase())) &&
//                        list.path("KategoriPerawatan").asText().toLowerCase().contains(NmKtg.getText().toLowerCase()) &&
//                        (!containsRestrictedWord || akses.getkode().equals("Admin Utama") || akses.getkode().equals("19951193"))) {
//
//                        tabMode.addRow(new Object[]{
//                            false,
//                            list.path("Kode").asText(),
//                            list.path("NamaPerawatan").asText(),
//                            list.path("KategoriPerawatan").asText(),
//                            list.path("Tarif").asDouble(),
//                            list.path("BagianRS").asDouble(),
//                            list.path("BHP").asDouble(),
//                            list.path("JMDokter").asDouble(),
//                            list.path("JMPerawat").asDouble(),
//                            list.path("KSO").asDouble(),
...
//        LCount.setText(""+tbKamar.getRowCount());
    ...
    try {
        ...
        String kategoriCari = NmKtg.getText().trim().toLowerCase();
        boolean adminBolehLihatRestricted = akses.getkode().equals("Admin Utama") || akses.getkode().equals("19951193");
        ...
        for (i = 0; i < tbKamar.getRowCount(); i++) {
            if (tbKamar.getValueAt(i, 0).toString().equals("true")) {
                ...
                if (containsRestrictedWord && !adminBolehLihatRestricted) {
                    continue;
                }
                jml++;
            }
        ...
        for (i = 0; i < jml; i++) {
            tabMode.addRow(new Object[]{
                pilih[i], kode[i], nama[i], kategori[i], totaltnd[i], bagianrs[i], bhp[i],
                jmdokter[i], jmperawat[i], kso[i], menejemen[i], kelastarif[i]
            });
        }

        String jsonTarif = new String(
            java.nio.file.Files.readAllBytes(java.nio.file.Paths.get("./cache/tarifranap.iyem")),
        ...
        jsonTarif = jsonTarif.replaceAll("[\\x00-\\x08\\x0B\\x0C\\x0E-\\x1F]", " ");

        root = mapper.readTree(jsonTarif);
        response = root.path("tarifranap");

        if (response.isArray()) {
            for (JsonNode list : response) {
                String kodeTindakan = list.path("Kode").asText();
                String namaPerawatan = list.path("NamaPerawatan").asText().toLowerCase();
                String kategoriPerawatan = list.path("KategoriPerawatan").asText().toLowerCase();
                boolean containsRestrictedWord = namaPerawatan.contains("echo") || namaPerawatan.contains("usg doppler");

                boolean cocokCari = cari.equals("")
                        || kodeTindakan.toLowerCase().contains(cari)
                        || namaPerawatan.contains(cari)
                        || kategoriPerawatan.contains(cari);

                boolean cocokKategori = kategoriCari.equals("")
                        || kategoriPerawatan.contains(kategoriCari);

                if (cocokCari && cocokKategori &&
                        (!containsRestrictedWord || adminBolehLihatRestricted)) {
