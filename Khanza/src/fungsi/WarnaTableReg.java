...
public class WarnaTableReg extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
        ...
        // Pewarnaan default selang-seling
        if (row % 2 == 1) {
            component.setBackground(new Color(238, 238, 238)); // Abu-abu muda
        } else {
            component.setBackground(new Color(255, 255, 255)); // Putih
        }

        // Pewarnaan berdasarkan jadwal dokter (kolom 1 dan 9)
        if (column == 1 || column == 9) {
            try {
                LocalDate today = LocalDate.now();
                String sql = "SELECT jam_mulai FROM jadwal WHERE kd_dokter = ? AND hari_kerja = ?";
                PreparedStatement ps = koneksi.prepareStatement(sql);
                ps.setString(1, table.getValueAt(row, 5).toString()); // kd_dokter di kolom 5
                ps.setInt(2, today.getDayOfWeek().getValue()); // hari sekarang

                ResultSet rs = ps.executeQuery();
                if (rs.next()) {
                    LocalTime jamMulai = LocalTime.parse(rs.getString("jam_mulai"));
                    long selisihMenit = Duration.between(LocalTime.now(), jamMulai).toMinutes();

                    if (selisihMenit > 30) {
                        component.setBackground(new Color(255, 250, 200)); // Kuning pucat
                    } else {
                        if (row % 2 == 1) {
        ...
        // Pewarnaan khusus untuk kolom 1 jika BPJS dan kondisi khusus
        if (column == 1) {
            try {
                if (table.getValueAt(row, 12).toString().equals("BPJS Kesehatan")) {
                    if (table.getValueAt(row, 25).toString().equals("Belum Sidik Jari")) {
                        component.setBackground(new Color(210, 255, 139)); // Hijau muda
                    } else if (table.getValueAt(row, 24).toString().equals("SEP Belum Terbit")) {
                        component.setBackground(new Color(219, 165, 7)); // Kuning tua
                    }
                }
            } catch (Exception e) {
                // Hindari error jika data null
            }
        }

        // Logika khusus untuk pasien geriatri di kolom umur (kolom index 10)
        if (column == 10) {
            try {
                String umurStr = table.getValueAt(row, 10).toString().replaceAll("[^\\d]", "");
                int umur = Integer.parseInt(umurStr);
                String jk = table.getValueAt(row, 9).toString(); // "L" atau "P"

                if (umur >= 60) {
                    // Atur font agar support emoji
                    component.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 12));

                    // Tambahkan ikon lansia sesuai gender
                    if (jk.equalsIgnoreCase("L")) {
                        setText("👴 " + table.getValueAt(row, 10).toString());
