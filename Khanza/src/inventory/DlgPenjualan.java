...
private void BtnGudangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnGudangActionPerformed
    ...
    */
    ...
    private String getKolomHargaJenisJual(){
        if(Jenisjual.getSelectedItem()==null){
            return "jualbebas";
        }

        String jenis=Jenisjual.getSelectedItem().toString();
        if(jenis.equals("Karyawan")){
            return "karyawan";
        }else if(jenis.equals("Jual Bebas")){
            return "jualbebas";
        }else if(jenis.equals("Beli Luar")){
            return "beliluar";
        }else if(jenis.equals("Rawat Jalan")){
            return "ralan";
        }else if(jenis.equals("Kelas 1")){
            return "kelas1";
        }else if(jenis.equals("Kelas 2")){
            return "kelas2";
        }else if(jenis.equals("Kelas 3")){
            return "kelas3";
        }else if(jenis.equals("Utama/BPJS")){
            return "utama";
    ...
    private double getHargaJenisJual(String kodeBrng,String noBatch,String noFaktur){
        double hargaJual=0;
        PreparedStatement psHarga=null;
        ResultSet rsHarga=null;
        String kolomHarga=getKolomHargaJenisJual();

        try{
            if(aktifkanbatch.equals("yes") &&
               noBatch!=null && !noBatch.trim().equals("") &&
               noFaktur!=null && !noFaktur.trim().equals("")){
                psHarga=koneksi.prepareStatement(
                    "select "+kolomHarga+" as harga from data_batch where kode_brng=? and no_batch=? and no_faktur=? limit 1"
                );
                psHarga.setString(1,kodeBrng);
                psHarga.setString(2,noBatch);
                psHarga.setString(3,noFaktur);
            }else{
                psHarga=koneksi.prepareStatement(
                    "select "+kolomHarga+" as harga from databarang where kode_brng=? limit 1"
                );
                psHarga.setString(1,kodeBrng);
            }
            ...
            if(rsHarga.next()){
                hargaJual=rsHarga.getDouble("harga");
            }
        }catch(Exception e){
            System.out.println("Notifikasi : "+e);
        }finally{
    ...
    private double getNilaiDouble(Object nilai){
        try{
            return Double.parseDouble(nilai.toString());
        }catch(Exception e){
            return 0;
        }
    }

    private void hitungUlangBarisObat(int baris){
        double jmlObat=getNilaiDouble(tabMode.getValueAt(baris,0));
        double hrgJual=getNilaiDouble(tabMode.getValueAt(baris,5));

        if(jmlObat>0){
            double subttl=jmlObat*hrgJual;
            tabMode.setValueAt(subttl,baris,6);

            double persenDiskon=getNilaiDouble(tabMode.getValueAt(baris,7));
            if(persenDiskon>0){
                tabMode.setValueAt(subttl*(persenDiskon/100),baris,8);
            }

            double totalBaris=subttl
                    - getNilaiDouble(tabMode.getValueAt(baris,8))
                    + getNilaiDouble(tabMode.getValueAt(baris,9))
                    + getNilaiDouble(tabMode.getValueAt(baris,10))
                    + getNilaiDouble(tabMode.getValueAt(baris,11));
            tabMode.setValueAt(totalBaris,baris,13);
        }else{
            tabMode.setValueAt(0,baris,6);
            tabMode.setValueAt(0,baris,13);
        }
    }

    private void hitungUlangBarisDetailRacikan(int baris){
        double jmlObat=getNilaiDouble(tbDetailObatRacikan.getValueAt(baris,8));
        double hrgJual=getNilaiDouble(tbDetailObatRacikan.getValueAt(baris,5));

        if(jmlObat>0){
            double subttl=jmlObat*hrgJual;
            tbDetailObatRacikan.setValueAt(subttl,baris,9);

            double persenDiskon=getNilaiDouble(tbDetailObatRacikan.getValueAt(baris,10));
            if(persenDiskon>0){
                tbDetailObatRacikan.setValueAt(subttl*(persenDiskon/100),baris,11);
            }

            double totalBaris=subttl
                    - getNilaiDouble(tbDetailObatRacikan.getValueAt(baris,11))
                    + getNilaiDouble(tbDetailObatRacikan.getValueAt(baris,12))
                    + getNilaiDouble(tbDetailObatRacikan.getValueAt(baris,13))
                    + getNilaiDouble(tbDetailObatRacikan.getValueAt(baris,14));
            tbDetailObatRacikan.setValueAt(totalBaris,baris,15);
        }else{
            tbDetailObatRacikan.setValueAt(0,baris,9);
            tbDetailObatRacikan.setValueAt(0,baris,15);
        }
    }

    private void hitungUlangGrandTotalPenjualan(){
        ttl=0;
        ttlhpp=0;
        y=0;
        z=0;
    ...
    private void refreshHargaJenisJualSemua(){
        double hargaBaru;
        String kodeBrng,noBatch,noFaktur;

        for(int r=0;r<tabMode.getRowCount();r++){
            try{
                kodeBrng=tabMode.getValueAt(r,1).toString();
                noBatch=tabMode.getValueAt(r,16)==null ? "" : tabMode.getValueAt(r,16).toString();
                noFaktur=tabMode.getValueAt(r,17)==null ? "" : tabMode.getValueAt(r,17).toString();
                hargaBaru=getHargaJenisJual(kodeBrng,noBatch,noFaktur);
                tabMode.setValueAt(hargaBaru,r,5);
                hitungUlangBarisObat(r);
            }catch(Exception e){
                System.out.println("Notifikasi : "+e);
            }
        }
        ...
        for(int r=0;r<tbDetailObatRacikan.getRowCount();r++){
            try{
                kodeBrng=tbDetailObatRacikan.getValueAt(r,1).toString();
                noBatch=tbDetailObatRacikan.getValueAt(r,18)==null ? "" : tbDetailObatRacikan.getValueAt(r,18).toString();
                noFaktur=tbDetailObatRacikan.getValueAt(r,19)==null ? "" : tbDetailObatRacikan.getValueAt(r,19).toString();
                hargaBaru=getHargaJenisJual(kodeBrng,noBatch,noFaktur);
                tbDetailObatRacikan.setValueAt(hargaBaru,r,5);
                hitungUlangBarisDetailRacikan(r);
            }catch(Exception e){
                System.out.println("Notifikasi : "+e);
            }
        }

        hitungUlangGrandTotalPenjualan();
    }
