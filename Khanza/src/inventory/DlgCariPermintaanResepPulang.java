    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            if(tbPemisahan.getValueAt(tbPemisahan.getSelectedRow(),0).toString().equals("")){
                ...
                String noPermintaan=tbPemisahan.getValueAt(tbPemisahan.getSelectedRow(),0).toString();
                Sequel.AutoComitFalse();
                Sequel.meghapus("permintaan_resep_pulang_racikan_detail","no_permintaan",noPermintaan);
                Sequel.meghapus("permintaan_resep_pulang_racikan","no_permintaan",noPermintaan);
                Sequel.meghapus("permintaan_resep_pulang","no_permintaan",noPermintaan);
                Sequel.Commit();
        ...
        try{  
            ...
            try{
                ...
                while(rs.next()){
                    ...
                    tampilRacikan(rs.getString("no_permintaan"));
                }
            } catch(Exception ex){
    ...
    private void tampilRacikan(String noPermintaan){
        ...
        try{
            psRacik=koneksi.prepareStatement(
                "select r.no_racik,r.nama_racik,ifnull(m.nm_racik,'') as metode,r.jml_dr,r.aturan_pakai,r.keterangan " +
            ...
            while(rsRacik.next()){
                String noRacik=rsRacik.getString("no_racik");
                tabMode.addRow(new String[]{"","RACIKAN "+noRacik,"Jumlah "+rsRacik.getString("jml_dr"),rsRacik.getString("aturan_pakai"),"Metode",rsRacik.getString("metode"),"",""});
                if(!rsRacik.getString("keterangan").trim().equals("")){
                    tabMode.addRow(new String[]{"","Keterangan","",rsRacik.getString("keterangan"),"","","",""});
                }
                tabMode.addRow(new String[]{"","Jumlah","Satuan","Komposisi","Kode Bahan","Nama Bahan","",""});

                PreparedStatement psDetail=null;
                ResultSet rsDetail=null;
                try{
                    psDetail=koneksi.prepareStatement(
                        "select d.kode_brng,db.nama_brng,db.kode_sat,d.p1,d.p2,d.kandungan,d.jml " +
                        "from permintaan_resep_pulang_racikan_detail d inner join databarang db on d.kode_brng=db.kode_brng " +
                        "where d.no_permintaan=? and d.no_racik=? order by db.nama_brng");
                    psDetail.setString(1,noPermintaan);
                    psDetail.setString(2,noRacik);
                    rsDetail=psDetail.executeQuery();
                    while(rsDetail.next()){
                        String komposisi=rsDetail.getString("p1")+"/"+rsDetail.getString("p2");
                        if(!rsDetail.getString("kandungan").trim().equals("")){
                            komposisi=komposisi+" ("+rsDetail.getString("kandungan")+")";
                        }
                        tabMode.addRow(new String[]{"",rsDetail.getString("jml"),rsDetail.getString("kode_sat"),komposisi,rsDetail.getString("kode_brng"),rsDetail.getString("nama_brng"),"",""});
                    }
                }finally{
                    try{ if(rsDetail!=null) rsDetail.close(); }catch(Exception e){}
                    try{ if(psDetail!=null) psDetail.close(); }catch(Exception e){}
                }
            }
        }catch(Exception e){
            System.out.println("Notifikasi tampil racikan permintaan resep pulang : "+e);
        }finally{
