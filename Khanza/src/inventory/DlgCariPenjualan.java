...
public class DlgCariPenjualan extends javax.swing.JDialog {
    ...
    public DlgCariPenjualan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row2={"No.","NIK","Nama Karyawan","Total"};
    ...
    private void initComponents() {
        ...
        Scroll1.setViewportView(tbDokter1);

        TabRawat.addTab("Total Penjualan Ke Karyawan", Scroll1);
        ...
        switch (TabRawat.getSelectedIndex()) {
            ...
            case 2:
                if(tabMode.getRowCount()==0){
                    ...
                    param.put("emailrs",akses.getemailrs());   
                    param.put("logo",Sequel.cariGambar("select setting.logo from setting")); 
                    Valid.MyReportqry("rptPenjualanKaryawan.jasper","report","::[ Transaksi Penjualan Barang ]::","select * from temporary where temporary.temp37='"+akses.getalamatip()+"' order by temporary.no",param);
                }  break;    
        }
        this.setCursor(Cursor.getDefaultCursor());
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
                  ...
                  if(rs.next()){
                      if(rs.getString("status").equals("Sudah Dibayar")){
                         ...
                         if(sukses==true){
                            ...
                            Sequel.menyimpan("tagihan_sadewa","'"+rs.getString("nota_jual")+"','"+rs.getString("no_rkm_medis")+"','"+rs.getString("nm_pasien").replaceAll("'","")+"','-',NOW(),'Pelunasan','"+Double.toString(ttljual)+"','"+Double.toString(ttljual)+"','Sudah','"+akses.getkode()+"'","No.Nota");    
                            Sequel.Commit();
                         }else{
                            sukses=false;
                            JOptionPane.showMessageDialog(null,"Terjadi kesalahan saat pemrosesan data, transaksi dibatalkan.\nPeriksa kembali data sebelum melanjutkan menyimpan..!!");
    ...
    */
    ...
    private void tampil3() {
        ...
        try{
            ttljual=0;
            ps=koneksi.prepareStatement("select pasien.nip,penjualan.nm_pasien, sum(detailjual.total)" 
                    + "from penjualan INNER JOIN pasien inner join petugas inner join bangsal inner join jenis  "+
                    " inner join detailjual inner join databarang inner join kodesatuan "+
                    " on detailjual.kode_brng=databarang.kode_brng "+
                    " and detailjual.kode_sat=kodesatuan.kode_sat "+
                    " and penjualan.kd_bangsal=bangsal.kd_bangsal "+
                    " and penjualan.nota_jual=detailjual.nota_jual "+
                    " and penjualan.nip=petugas.nip and databarang.kdjns=jenis.kdjns "+
                    " and penjualan.no_rkm_medis = pasien.no_rkm_medis "+
                    " where penjualan.jns_jual = 'Karyawan' and "+tanggal+statusbayar+datacari+
                    " group by penjualan.nm_pasien ");
            try {
                rs=ps.executeQuery();
                int nomor = 1;
                ttlsubttl=0;
                while(rs.next()){                    
                    tabMode2.addRow(new String[]{
                        String.valueOf(nomor), rs.getString(1),rs.getString(2),Valid.SetAngka4(rs.getDouble(3))
                    });
                    nomor = nomor + 1;
                    ttlsubttl = ttlsubttl + rs.getDouble(3);
                }
                tabMode2.addRow(new String[]{"","","Total",Valid.SetAngka4(ttlsubttl)});
            } catch (Exception e) {
                System.out.println("Notifikasi : "+e);
            } finally{
                if(rs!=null){
