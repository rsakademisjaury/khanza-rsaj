...
public class DlgPenggunaObat extends javax.swing.JDialog {
    ...
    public DlgPenggunaObat(java.awt.Frame parent, boolean modal) {
        ...
        // total kolom sekarang 9 (index 0..8)
        for (i = 0; i < 9; i++) {
            TableColumn column = tbDokter.getColumnModel().getColumn(i);
            if(i==0){
                column.setPreferredWidth(75);   // Kode Barang
            }else if(i==1){
                column.setPreferredWidth(450);  // Nama Barang
            }else if(i==2){
                column.setPreferredWidth(50);   // Jml
            }else if(i==3){
                column.setPreferredWidth(150);  // Asal Stok
            }else if(i==4){
                column.setPreferredWidth(150);  // Ruangan  (BARU)
            }else if(i==5){
                column.setPreferredWidth(50);   // Status
            }else if(i==6){
                column.setPreferredWidth(230);  // Alamat Pasien
            }else if(i==7){
                column.setPreferredWidth(80);   // No.Resep
            }else if(i==8){
                column.setPreferredWidth(170);  // Dokter Peresep
            }
        }
        ...
        kamar.bangsal.addWindowListener(new WindowListener() {
             ...
             @Override
             public void windowClosed(WindowEvent e) {
                 if(akses.getform().equals("DlgPenggunaObat")){
    ...
    private void initComponents() {
        ...
        BangsalCari.setBounds(970, 40, 235, 23);
        ...
        btnBangsalCari.setBounds(1210, 35, 30, 30);
...
private void prosesCari() {            
    try{   
        ...
        // --- filter teks (trim biar aman) ---
        final String fPoli   = CrPoli.getText().trim();
        final String fKamar  = BangsalCari.getText().trim();

        // --- query master barang (tetap seperti aslinya) ---
        if(nmjns.getText().equals("")&&nmkategori.getText().equals("")&&nmgolongan.getText().equals("")&&TCari.getText().equals("")){
            psbarang=koneksi.prepareStatement(
                "select databarang.kode_brng,databarang.nama_brng,kode_sat from databarang "+
                "inner join jenis on databarang.kdjns=jenis.kdjns "+
        ...
        // --- klausa filter Poli dan Kamar (dibatasi sesuai status_lanjut) ---
        String poliClause  = fPoli.isEmpty()  ? "" : " and rp.status_lanjut='Ralan' and ifnull(poliklinik.nm_poli,'') like '%"+fPoli+"%' ";
        String kamarClause = fKamar.isEmpty() ? "" : " and rp.status_lanjut='Ranap' and ifnull(b_kmr.nm_bangsal,'') like '%"+fKamar+"%' ";

        // --- SELECT detail (Asal Stok = b_asal, Ruangan = CASE Ralan/Ranap) ---
        String selectDetail =
            "select dpo.tgl_perawatan,dpo.jam,"+
            "dpo.no_rawat,rp.no_rkm_medis,pasien.nm_pasien,"+
            "concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab,', ',propinsi.nm_prop) as alamat,"+
            "dpo.jml,dpo.status, b_asal.nm_bangsal, "+
            "case when rp.status_lanjut='Ralan' then poliklinik.nm_poli "+
            "     when rp.status_lanjut='Ranap' then b_kmr.nm_bangsal "+
            "     else '' end as ruangan "+
            ...
            "inner join kabupaten on pasien.kd_kab=kabupaten.kd_kab "+
            "inner join propinsi on pasien.kd_prop=propinsi.kd_prop "+
            "inner join bangsal b_asal on dpo.kd_bangsal=b_asal.kd_bangsal "+ // asal stok (tetap)
            "left join poliklinik on rp.kd_poli=poliklinik.kd_poli "+         // untuk Ralan
            "left join kamar_inap ki on ki.no_rawat=rp.no_rawat "+            // untuk Ranap
            "left join kamar on kamar.kd_kamar=ki.kd_kamar "+
            "left join bangsal b_kmr on b_kmr.kd_bangsal=kamar.kd_bangsal "+  // nama bangsal kamar (ruangan)
            "inner join penjab on rp.kd_pj=penjab.kd_pj ";

        // --- WHERE dasar ---
        String whereDasar =
            "where dpo.tgl_perawatan between '"+tgl1+"' and '"+tgl2+"' "+
            "and dpo.kode_brng=? ";

        // --- filter penjab, asal stok, status (pakai alias b_asal) ---
        String filterPenjabAsalStatus =
            "and concat(rp.kd_pj,penjab.png_jawab) like '%"+kdpenjab.getText()+nmpenjab.getText()+"%' "+
            "and concat(dpo.kd_bangsal,b_asal.nm_bangsal) like '%"+kdasal.getText()+nmasal.getText()+"%' "+
            "and dpo.status like '%"+Status.getSelectedItem().toString().replaceAll("Semua","")+"%' ";

        // --- rakit final sqlsub (tambahkan poliClause & kamarClause) ---
        if(Status.getSelectedItem().toString().equals("Semua")&&nmpenjab.getText().equals("")&nmasal.getText().equals("")){
            sqlsub = selectDetail + whereDasar + poliClause + kamarClause +
                     " order by dpo.tgl_perawatan,dpo.jam";
        } else {
            sqlsub = selectDetail + whereDasar + poliClause + kamarClause + " " + filterPenjabAsalStatus +
                     " order by dpo.tgl_perawatan,dpo.jam";
        }
        ...
        try {
            ...
            i=1;
            while(rsbarang.next()){
                // header per barang (pastikan tabMode 9 kolom)
                tabMode.addRow(new Object[]{
                    rsbarang.getString("kode_brng"),
                    rsbarang.getString("nama_brng"),
                ...
                try {
                    ...
                    while(rspasien.next()){
                        ...
                        try {
                            ...
                            if(rsresep!=null){ rsresep.close(); }
                            if(psresep!=null){ psresep.close(); }
                        }
                        ...
                        tabMode.addRow(new Object[]{
                            ...
                            i+". "+rspasien.getString("tgl_perawatan")+" "+rspasien.getString("jam")+"  "+
                            rspasien.getString("no_rawat")+"  "+rspasien.getString("no_rkm_medis")+"  "+rspasien.getString("nm_pasien"),
                            rspasien.getDouble("jml"),
                            rspasien.getString("nm_bangsal"),     // Asal Stok (b_asal)
                            rspasien.getString("ruangan"),        // Ruangan (Poli/Ranap)
                            rspasien.getString("status"),
                    ...
                    if(pspasien!=null){ pspasien.close(); }
                }
            }
        } catch (Exception e) {
            System.out.println("Notif : "+e);
            ...
            if(psbarang!=null){ psbarang.close(); }
        }                
        this.setCursor(Cursor.getDefaultCursor());             
    }catch(Exception e){
        System.out.println("Catatan  "+e);
