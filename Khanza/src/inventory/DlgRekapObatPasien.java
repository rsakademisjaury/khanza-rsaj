...
public class DlgRekapObatPasien extends javax.swing.JDialog {
    ...
    private PreparedStatement psobat,psreg,pskamar,pslunas;
    private ResultSet rsobat,rsreg,rskamar,rslunas; 
    private DlgCariCaraBayar penjab=new DlgCariCaraBayar(null,true);
    private DlgCariJenis jenis = new DlgCariJenis(null, true);
    private DlgCariKategori kategori = new DlgCariKategori(null, true);
    private DlgCariGolongan golongan = new DlgCariGolongan(null, true);
    private DlgCariBangsal asalstok=new DlgCariBangsal(null,true);
    private int i=0,a=0;
    ...
    public DlgRekapObatPasien(java.awt.Frame parent, boolean modal) {
        ...
        tabMode=new DefaultTableModel(null,new Object[]{"No.","Tgl. Bayar","Tanggal","No. Rawat  /  No. Nota","Status","No.RM","Nama Pasien","Cara Bayar","Jml","Nama Obat","Biaya Obat","Embalase","Tuslah","Total"}){
        ...
        tabMode3=new DefaultTableModel(null,new Object[]{"No.","Tgl. Bayar","Tanggal","No. Rawat  /  No. Nota","Status","No.RM","Nama Pasien","Cara Bayar","Jml","Nama Obat","Biaya Obat","Embalase","Tuslah","Total"}){
              @Override public boolean isCellEditable(int rowIndex, int colIndex){return false;}
        };
        tbDokter3.setModel(tabMode3);
        ...
        statuspasien.setSelectedIndex(2); // Pasien Sudah Bayar
        isForm();
    ...
    private void initComponents() {
        ...
        label24.setText("Filter :");
        label24.setName("label24"); // NOI18N
        label24.setPreferredSize(new java.awt.Dimension(45, 23));
        panelisi1.add(label24);

        statuspasien.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Semua Pasien", "Pasien Belum Bayar", "Pasien Sudah Bayar", "Pasien Belum Pulang", "Pasien Sudah Pulang" }));
        statuspasien.setName("statuspasien"); // NOI18N
        statuspasien.setPreferredSize(new java.awt.Dimension(150, 23));
        panelisi1.add(statuspasien);
        ...
        label20.setBounds(528, 10, 70, 23);
        ...
        nmkategori.setBounds(1110, 10, 150, 23);
        ...
        nmgolongan.setBounds(1380, 10, 150, 23);
        ...
        BtnGolongan.setBounds(1530, 10, 28, 23);
        ...
        TabRawat1.setBackground(new java.awt.Color(255, 255, 255));
        TabRawat1.setBorder(new javax.swing.border.LineBorder(new java.awt.Color(241, 246, 236), 1, true));
        ...
        TabRawat1.addTab(" Pasien Umum     ", new javax.swing.ImageIcon(getClass().getResource("/picture/rkontrol.png")), scrollPane4); // NOI18N
        ...
        scrollPane6.setViewportView(tbDokter3);

        TabRawat1.addTab(" Pasien Instansi     ", new javax.swing.ImageIcon(getClass().getResource("/picture/liatdpjp.png")), scrollPane6); // NOI18N
        ...
        if(TabRawat1.getSelectedIndex()==0){
           this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            ...
            if(tabMode3.getRowCount()==0){
                ...
                for(int r=0;r<row;r++){  
                    Sequel.menyimpan("temporary","'"+r+"','"+
                                    tabMode3.getValueAt(r,0).toString().replaceAll("'","`")+"','"+
                                    tabMode3.getValueAt(r,1).toString().replaceAll("'","`")+"','"+
                                    ...
                                    tabMode3.getValueAt(r,5).toString().replaceAll("'","`")+"','"+
                                    tabMode3.getValueAt(r,6).toString().replaceAll("'","`")+"','"+
                                    tabMode3.getValueAt(r,7).toString().replaceAll("'","`")+"','"+
                                    tabMode3.getValueAt(r,8).toString().replaceAll("'","`")+"','"+
                                    tabMode3.getValueAt(r,9).toString().replaceAll("'","`")+"','','','','','','','','','','','','','','','','','','','','','','','','','','','"+akses.getalamatip()+"'","Rekap Obat Perdokter Poli"); 
                }
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    private void Tgl2KeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_Tgl2KeyPressed
        Valid.pindah(evt, Tgl1,statuspasien);
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgRekapObatPasien dialog = new DlgRekapObatPasien(new javax.swing.JFrame(), false);
    ...
    private void prosesCari() {
        prosesCariPasien(false, tabMode);
    }
    
    private void prosesCari3() {
        prosesCariPasien(true, tabMode3);
    }
    
    private String getTanggalRanap() {
        return "if(kamar_inap.tgl_keluar='0000-00-00',kamar_inap.tgl_masuk,kamar_inap.tgl_keluar)";
    }
    
    private String getFilterTanggalPasien(String tglBayar, String tglPelayanan) {
        if(statuspasien.getSelectedIndex()==1){
            return tglPelayanan+" between ? and ? ";
        }else if(statuspasien.getSelectedIndex()==2){
            return tglBayar+" between ? and ? ";
        }else{
            return "ifnull("+tglBayar+","+tglPelayanan+") between ? and ? ";
        }
    }
    
    private String getOrderTanggalPasien(String tglBayar, String tglPelayanan) {
        if(statuspasien.getSelectedIndex()==1){
            return tglPelayanan;
        }else if(statuspasien.getSelectedIndex()==2){
            return tglBayar;
        }else{
            return "ifnull("+tglBayar+","+tglPelayanan+")";
        }
    }
    
    private String getFilterBayarPasien(String kolomBayar) {
        if(statuspasien.getSelectedIndex()==1){
            return " and "+kolomBayar+" is null ";
        }else if(statuspasien.getSelectedIndex()==2){
            return " and "+kolomBayar+" is not null ";
        }else{
            return " ";
        }
    }
    
    private String getFilterPulangRalan() {
        if(statuspasien.getSelectedIndex()==3){
            return " and reg_periksa.stts='Belum' ";
        }else if(statuspasien.getSelectedIndex()==4){
            return " and reg_periksa.stts<>'Belum' ";
        }else{
            return " ";
        }
    ...
    private String getFilterPulangRanap() {
        if(statuspasien.getSelectedIndex()==3){
            return " and kamar_inap.stts_pulang='-' ";
        }else if(statuspasien.getSelectedIndex()==4){
            return " and kamar_inap.stts_pulang<>'-' ";
        }else{
            return " ";
    ...
    private String getStatusObat() {
        return status.getSelectedItem().toString().replaceAll("Obat Rawat Jalan","Ralan").replaceAll("Obat Rawat Inap","Ranap").replaceAll("Semua Status","");
    }
    
    private PreparedStatement prepareObatPasien(boolean instansi) throws Exception {
        String tuslah="sum(detail_pemberian_obat.tuslah) as tuslah";
        String joinPenjab=" ";
        if(instansi){
            tuslah="(SELECT CASE WHEN penjab.kd_pj='bpj' THEN sum(detail_pemberian_obat.tuslah)*0.3 ELSE sum(detail_pemberian_obat.tuslah) END ) AS tuslah";
            joinPenjab=" inner join penjab on reg_periksa.kd_pj = penjab.kd_pj ";
        }
        if((status.getSelectedIndex()==0)&&nmjns.getText().equals("")&&nmkategori.getText().equals("")&&nmgolongan.getText().equals("")&&nmasal.getText().equals("")){
            return koneksi.prepareStatement(
                ...
                "sum(detail_pemberian_obat.embalase) as embalase,"+tuslah+","+
    ...
    private void setParameterObat(String noRawat) throws Exception {
        if((status.getSelectedIndex()==0)&&nmjns.getText().equals("")&&nmkategori.getText().equals("")&&nmgolongan.getText().equals("")&&nmasal.getText().equals("")){
            psobat.setString(1,noRawat);
        }else{
            psobat.setString(1,noRawat);
            psobat.setString(2,"%"+getStatusObat()+"%");
    ...
    private void tampilkanObatPasien(ResultSet rsPasien, DefaultTableModel model, boolean instansi, boolean rawatInap) throws Exception {
        while(rsPasien.next()){
            psobat=prepareObatPasien(instansi);
            try {
                setParameterObat(rsPasien.getString("no_rawat"));
                    ...
                    do{
                        if(a==1){
                            model.addRow(new String[]{
                                i+"",rsPasien.getString("tgl_bayar"),rawatInap?rsPasien.getString("tgl_keluar"):rsPasien.getString("tgl_registrasi"),rsPasien.getString("no_rawat")+"  -  "+rsPasien.getString("no_nota"),rsPasien.getString("status_lanjut"),rsPasien.getString("no_rkm_medis"),rsPasien.getString("nm_pasien"),rsPasien.getString("instansi"),
    ...
    private void setParameterPasien(PreparedStatement ps) throws Exception {
    ...
    private void prosesCariPasien(boolean instansi, DefaultTableModel model) {             
        try{   
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR)); 
            Valid.tabelKosong(model);
            String tabelBayar=instansi?"bayar_piutang":"tagihan_sadewa";
            String kolomBayar=instansi?"bayar.no_rawat":"bayar.no_nota";
            String joinBayar="left join "+tabelBayar+" bayar on reg_periksa.no_rawat="+kolomBayar+" ";
            String filterUmum=instansi?" ":" and reg_periksa.kd_pj='001' ";
            String filterKeyword=" ";
            if(!(nmpenjab.getText().equals("")&&TCari.getText().equals(""))){
                filterKeyword=" and concat(reg_periksa.kd_pj,penjab.png_jawab) like ? and (reg_periksa.no_rkm_medis like ? or pasien.nm_pasien like ?) ";
            }
            ...
            try {
                ...
                if((status.getSelectedIndex()==0)||(status.getSelectedIndex()==1)){
                    psreg=koneksi.prepareStatement(
                       ...
                       "ifnull(nota_jalan.no_nota,'-') as no_nota,"+        
                       "reg_periksa.no_rkm_medis,"+
                       "penjab.png_jawab AS instansi,"+        
                       "pasien.nm_pasien,"+
                       "ifnull(bayar.tgl_bayar,'Belum Bayar') as tgl_bayar "+
                       "from "+
                       "reg_periksa "+                       
                       ...
                       filterUmum+" and "+
                       getFilterTanggalPasien("bayar.tgl_bayar","reg_periksa.tgl_registrasi")+
                       getFilterBayarPasien(kolomBayar)+
                       getFilterPulangRalan()+
                       filterKeyword+
                       "group by reg_periksa.no_rawat order by "+
                       getOrderTanggalPasien("bayar.tgl_bayar","reg_periksa.tgl_registrasi")
                    );
                    setParameterPasien(psreg);
                    rsreg=psreg.executeQuery();
                    tampilkanObatPasien(rsreg, model, instansi, false);
                }
                
                if((status.getSelectedIndex()==0)||(status.getSelectedIndex()==2)){
                    pskamar=koneksi.prepareStatement(
                       "select "+
                       getTanggalRanap()+" as tgl_keluar,"+
                       "reg_periksa.no_rawat,"+
                       "reg_periksa.status_lanjut,"+        
                       "ifnull(nota_inap.no_nota,'-') as no_nota,"+
                       "reg_periksa.no_rkm_medis,"+
                       "penjab.png_jawab AS instansi,"+
                       "pasien.nm_pasien,"+
                       "ifnull(bayar.tgl_bayar,'Belum Bayar') as tgl_bayar "+
                       ...
                       filterUmum+" and "+
                       getFilterTanggalPasien("bayar.tgl_bayar",getTanggalRanap())+
                       getFilterBayarPasien(kolomBayar)+
                       getFilterPulangRanap()+
                       filterKeyword+
                       "group by reg_periksa.no_rawat order by "+
                       getOrderTanggalPasien("bayar.tgl_bayar",getTanggalRanap())
                    );
                    setParameterPasien(pskamar);
                    rskamar=pskamar.executeQuery();
                    tampilkanObatPasien(rskamar, model, instansi, true);
                }
                
                if(ttltotal>0){
                    model.addRow(new Object[]{">>","Total ",":","","","","","","","",Valid.SetAngka(ttlbiaya),Valid.SetAngka(ttlembalase),Valid.SetAngka(ttltuslah),Valid.SetAngka(ttltotal)});
